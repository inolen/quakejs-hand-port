 ...
 */
function GetWaterLevel(origin, viewheight, pointContents) {
	var waterlevel = 0;

	var point = vec3.create(origin);
	point[2] += MINS_Z + 1;

	var contents = pointContents(point, -1);
...
function SetWaterLevel() {
	...
	pm.waterlevel = GetWaterLevel(ps.origin, ps.viewheight, pm.pointContents);
...
function Damage(targ, inflictor, attacker, dir, point, damage, dflags, mod) {
	...
	if (g_gametype() === GT.CLANARENA && targ === attacker && inflictor.classname === 'rocket') {
		damage = 0;
	}
...
function RadiusDamage(origin, inflictor, attacker, damage, radius, ignore, mod) {
	...
	for (var e = 0; e < entityNums.length; e++) {
		...
		if (CanDamage(ent, origin)) {
			...
			var dir = vec3.subtract(ent.currentOrigin, origin, vec3.create());
...
function CanDamage(targ, origin) {
	...
	var midpoint = vec3.add(targ.absmin, targ.absmax, vec3.create());
	vec3.scale(midpoint, 0.5);

	var dest = vec3.create(midpoint);
	var tr = sv.Trace(origin, dest, QMath.vec3origin, QMath.vec3origin, ENTITYNUM_NONE, MASK.SOLID);
	...
	dest[0] += 15.0;
	dest[1] += 15.0;
	tr = sv.Trace(origin, dest, QMath.vec3origin, QMath.vec3origin, ENTITYNUM_NONE, MASK.SOLID);
	if (tr.fraction === 1.0) {
		return true;
	}
...
function PlayerDie(self, inflictor, attacker, damage, meansOfDeath) {
	...
	var contents = sv.PointContents(self.currentOrigin, -1);
	...
	// In CA, you are forced to spec until the match is over once you die.
	// TODO Spawning them at a forced spectator spawn is pretty abrupt, perhaps just
	// leave them speccing where they died.
	if (g_gametype() === GT.CLANARENA && !level.warmupTime) {
		self.client.sess.spectatorState = SPECTATOR.FREE;
		ClientRespawn(self);
	}
}
...
function SetOrigin(ent, origin) {
	...
	vec3.set(vec3.create(), ent.s.pos.trDelta);
...
function KillBox(ent) {
	var mins = vec3.add(ent.client.ps.origin, ent.mins, vec3.create());
	var maxs = vec3.add(ent.client.ps.origin, ent.maxs, vec3.create());
	var entityNums = sv.FindEntitiesInBox(mins, maxs);
...
function RunItem(ent) {
	...
	var tr = sv.Trace(ent.currentOrigin, origin, ent.mins, ent.maxs, ent.ownerNum, mask);
	...
	var contents = sv.PointContents(ent.currentOrigin, -1);
	if (contents & CONTENTS.NODROP) {
...
function BounceItem(ent, trace) {
	...
	vec3.add(ent.currentOrigin, trace.plane.normal);
 ...
 */
function LaunchItem(item, origin, velocity) {
	...
	dropped.item = item;
	vec3.set([-ITEM_RADIUS, -ITEM_RADIUS, -ITEM_RADIUS], dropped.mins);
	vec3.set([ITEM_RADIUS, ITEM_RADIUS, ITEM_RADIUS], dropped.maxs);
	dropped.contents = CONTENTS.TRIGGER;
...
function AimAtTarget(self) {
	var origin = vec3.add(self.absmin, self.absmax, vec3.create());
...
function ShotgunPellet(start, end, ent) {
	...
	for (var i = 0; i < 10; i++) {
		var tr = sv.Trace(start, end, null, null, ent.s.number, MASK.SHOT);
		var traceEnt = level.gentities[tr.entityNum];
...
function DoorSpawnNewTrigger(ent) {
	...
	var mins = vec3.create(ent.absmin);
	var maxs = vec3.create(ent.absmax);
...
function TraceThroughPatchCollide(tw, pc) {
	...
	for (var i = 0; i < pc.facets.length; i++) {
		...
		if (tw.sphere.use) {
			...
			t = vec3.dot(plane.normal, tw.sphere.offset);
			if (t > 0) {
...
function BoxTrace(start, end, mins, maxs, model, brushmask, capsule ) {
	return Trace(start, end, mins, maxs, model, vec3.create(), brushmask, capsule, null);
}
...
var ServerStatic = function () {
	...
	for (var i = 0; i < MAX_SNAPSHOT_ENTITIES; i++) {
		this.snapshotEntities[i] = new QShared.EntityState();
	}
};
...
var ServerEntity = function () {
	...
	this.baseline        = new QShared.EntityState();
...
ServerClient.prototype.reset = function () {
	...
	this.lastUserCmd             = new QShared.UserCmd();
...
function CreateListenServer(gamePort) {
	...
	if (!dedicated) {
		...
		var loopback = com.NetchanSetup(NS.SERVER, addr, {
...
function SendMasterHeartbeat() {
	...
	var netchan = com.NetchanSetup(NS.SERVER, addr);
...
function ClientAccept(msocket) {
	...
	var netchan = com.NetchanSetup(NS.SERVER, msocket, {
		onmessage: function (buffer) {
...
function ClientEnterWorld(client, cmd) {
	...
	if (cmd) {
		...
		client.lastUserCmd = new QShared.UserCmd();
	}
...
function UserMove(client, msg, delta) {
	...
	client.frames[client.messageAcknowledge % PACKET_BACKUP].messageAcked = svs.time;
...
function SendClientGameState(client) {
	...
	// Write the baselines.
	var nullstate = new QShared.EntityState();
	for (var i = 0; i < MAX_GENTITIES; i++) {
...
function CmdRestartMap(delayString) {
	...
	// Force a fake warmup if the game isn't going to do one on init.
...
function ClipHandleForEntity(ent) {
	...
	if (ent.svFlags & gm.SVF.CAPSULE) {
		com.Error('Capsules not supported.');
		return 0;
...
function PointContents(p, passEntityNum) {
	...
	for (var i = 0; i < touchlist.length; i++) {
		...
		if (hit.bmodel) {
			angles = hit.currentAngles;
		}

		var c2 = cm.TransformedPointContents(p, clipHandle, hit.currentOrigin, angles);
...
function (_, ByteBuffer, BspSerializer, MapSerializer, MapCompiler, QMath, QShared, Server, Client) {
	var MAX_GENTITIES  = QShared.MAX_GENTITIES;
 ...
 *
 **********************************************************/
var dummyUsercmd = new QShared.UserCmd();
...
var dummyPlayerState = new QShared.PlayerState();
...
function NetchanSendLoopPacket(netchan, buffer, length) {
	...
	var remote_src = netchan.src === NS.CLIENT ? NS.SERVER : NS.CLIENT;
	var handler = loopbackHandlers[remote_src];

	if (!handler) {
		error('No loopback handler for', remote_src === NS.CLIENT ? 'client' : 'server');
		return;
	}
