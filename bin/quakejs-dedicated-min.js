...
function TestEntityPosition(ent) {
	...
	var tr;
	if (ent.client) {
		tr = sv.Trace(ent.client.ps.origin, ent.client.ps.origin, ent.mins, ent.maxs, ent.s.number, mask);
	} else {
		tr = sv.Trace(ent.s.pos.trBase, ent.s.pos.trBase, ent.mins, ent.maxs, ent.s.number, mask);
	}
...
function LinkEntity(gent) {
	...
	} else if (gent.contents & (QS.CONTENTS.SOLID | QS.CONTENTS.BODY)) {
