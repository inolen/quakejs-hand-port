...
var ClientLocals = function () {
	...
	this.outPackets           = new Array(PACKET_BACKUP);  // information about each packet we have sent out
	...
	for (var i = 0; i < CMD_BACKUP; i++) {
		this.cmds[i] = new QShared.UserCmd();
	}

	for (var i = 0; i < MAX_GENTITIES; i++) {
		this.entityBaselines[i] = new QShared.EntityState();
	}

	for (var i = 0; i < MAX_PARSE_ENTITIES; i++) {
		this.parseEntities[i] = new QShared.EntityState();
	}
};
...
var ClientSnapshot = function () {
	...
	this.ps               = new QShared.PlayerState();          // complete information about the current player at this time
