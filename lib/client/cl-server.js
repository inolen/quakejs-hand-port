...
function WritePacket() {
	...
	for (var i = clc.reliableAcknowledge + 1; i <= clc.reliableSequence; i++) {
		var cmd = clc.reliableCommands[i % MAX_RELIABLE_COMMANDS];
	...
	var cmd;
	var oldcmd;
	var oldPacketNum = (clc.netchan.outgoingSequence - 1/* - cl_packetdup->integer*/) % PACKET_BACKUP;
	var count = cl.cmdNumber - cl.outPackets[oldPacketNum].cmdNumber;

	// Sanity check.. disable for UDP.
	if (count > 1) {
		com.error(ERR.DROP, 'Invalid cmd number.');
		return;
	}
	...
	if (count >= 1) {
		...
		for (var i = 0; i < count; i++) {
			var j = (cl.cmdNumber - count + i + 1) % CMD_BACKUP;
			var cmd = cl.cmds[j];
	...
	var packetNum = clc.netchan.outgoingSequence % PACKET_BACKUP;
...
function ParseSnapshot(msg) {
	...
	for (var i = 0; i < PACKET_BACKUP; i++) {
		var packetNum = (clc.netchan.outgoingSequence - 1 - i) % PACKET_BACKUP;
