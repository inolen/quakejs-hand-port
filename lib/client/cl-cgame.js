...
function GetServerCommand(serverCommandNum) {
	...
	if (serverCommandNum <= clc.serverCommandSequence - COM.MAX_RELIABLE_COMMANDS) {
		error('GetServerCommand: a reliable command was cycled out');
		return null;
	}

	if (serverCommandNum > clc.serverCommandSequence) {
		error('GetServerCommand: requested a command not received');
		return null;
	}
...
function GetUserCmd(cmdNumber, dest) {
	...
	if (cmdNumber > cl.cmdNumber) {
		error('GetUserCmd: ' + cmdNumber + ' >= ' + cl.cmdNumber);
	}
...
function SetCGameTime() {
	...
	if (!cl.snap.valid) {
		error('SetCGameTime: !cl.snap.valid');
	}
...
function GetSnapshot(snapshotNumber) {
	if (snapshotNumber > cl.snap.messageNum) {
		error('GetSnapshot: snapshotNumber > cl.snapshot.messageNum');
	}
