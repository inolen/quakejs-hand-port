...
function InitMenus() {
	...
	connecting_model = new UI.ConnectingModel();
	connecting_view  = UI.CreateView(connecting_model, UI.ConnectingTemplate);
}
...
function ConnectToServer(address) {
	...
	var socket = COM.NetConnect(clc.serverAddress, {
		...
		onclose: function () {
			...
			// AP - Let this fall through as an error since
			// we have no real main menu UI.
...
function UpdateScreen() {
	switch (clc.state) {
		...
		case CA.CONNECTED:
			...
			connecting_model.address(address);
			connecting_model.message('Awaiting gamestate...');

			UI.RenderView(connecting_view);
			break;
		case CA.LOADING:
		case CA.PRIMED:
		case CA.ACTIVE:
