...
function UpdateScreen() {
	switch (clc.state) {
		case CA.DISCONNECTED:
			if (!(UI.GetActiveMenu() instanceof UI.DefaultMenu)) {
				UI.ClearViews();

				UI.SetActiveMenu(UI.GetView(UI.DefaultMenu));
			}
			break;
		...
		case CA.LOADING:
		case CA.PRIMED:
			// Clear the disconnected menu if it exists.
			if (UI.GetActiveMenu()) {
				UI.ClearViews();
			}
			...
			var loading_view = UI.GetView(UI.LoadingView);
			loading_view.address(address);
			break;
