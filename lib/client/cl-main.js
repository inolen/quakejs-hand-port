...
function UpdateScreen() {
	switch (clc.state) {
		case CA.DISCONNECTED:
			...
			if (!UI.PeekMenu()) {
				var main_model = new UI.MainMenuModel();
				var main_view = UI.CreateView(main_model, UI.MainMenuModel.template);
				UI.PushMenu(main_view);
			}
			break;
		case CA.CONNECTING:
		case CA.CHALLENGING:
		...
		case CA.PRIMED:
			UI.PopAllMenus();
			var loading_model = new UI.LoadingViewModel();
			var loading_view = UI.CreateView(loading_model, UI.LoadingViewModel.template);
