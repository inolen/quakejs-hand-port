...
function CheckForResend() {
	...
	if (clc.netchan && !clc.netchan.ready) {
		...
		clc.netchan.onclose = function () {
			// This is a bit indirect, com.Error will result in a disconnect
			// and throw up an error box as we'd like.
			com.Error('Disconnected from server');
		};
...
function CGameExports() {
	return {
		sys: {
			...
			ReadFile:          sys.ReadFile,
			GetViewportWidth:  sys.GetViewportWidth,
			GetViewportHeight: sys.GetViewportHeight
		},
		com: {
			ERR:     com.ERR,
			Error:   com.Error,
