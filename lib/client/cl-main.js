...
function UpdateScreen() {
	var ensureViewRemoved = function (name) {
		if (cls[name]) {
			UI.RemoveView(cls[name]);
			cls[name] = null;
		}
	};
	...
	switch (clc.state) {
		case CA.DISCONNECTED:
			ensureViewRemoved('connecting_view');
		...
		case CA.CHALLENGING:
		case CA.CONNECTED:
			ensureViewRemoved('default_view');

			if (!cls.connecting_view) {
			...
			cls.connecting_model.message('Awaiting gamestate...');
			break;
		case CA.LOADING:
		case CA.PRIMED:
		case CA.ACTIVE:
