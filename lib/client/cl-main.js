...
function CheckForResend() {
	...
	switch (clc.state) {
		...
		case CA.CHALLENGING:
			...
			COM.NetchanOutOfBandPrint(clc.netchan, JSON.stringify({ type: 'connect', data: data }));
...
function CGameExports() {
	return {
		...
		CL: {
			...
			LoadBsp:                  COM.LoadBsp,
			LoadMap:                  COM.LoadMap,
			GetLoadingModel:          function () { return loading_model; },
			GetLoadingView:           function () { return loading_view; },
