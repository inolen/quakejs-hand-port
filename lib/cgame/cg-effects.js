...
function MakeExplosion(origin, dir, hModel, shader, msec, isSprite) {
	...
	var offset = QMath.irrandom(0, 64);

	if (isSprite) {
		le.leType = LE.SPRITE_EXPLOSION;

		// Randomly rotate sprite orientation.
		le.refent.rotation = QMath.rrandom(0, 360);
...
function SmokePuff(p, vel,
	...
	refent.shaderRGBA[0] = le.color[0];
	refent.shaderRGBA[1] = le.color[1];
	refent.shaderRGBA[2] = le.color[2];
...
function Bleed(origin, entityNum) {
	...
	vec3.set(origin, refent.origin);
	refent.reType = RT.SPRITE;
	refent.rotation = QMath.rrandom(0, 360);
	refent.radius = 24;
