...
function NewClientInfo(clientNum, callback) {
	...
	LoadClientInfo(clientNum, ci, function () {
		UpdateCurrentGame();
...
function PlayerSprites(cent) {
	// if (cent.currentState.eFlags & EF.CONNECTION) {
	// 	PlayerFloatSprite(cent, cgs.media.connectionShader);
	// 	return;
	// }

	// if (cent.currentState.eFlags & EF.TALK) {
	// 	PlayerFloatSprite(cent, cgs.media.balloonShader);
	// 	return;
	// }

	if (cent.currentState.eFlags & EF.AWARD_IMPRESSIVE) {
		PlayerFloatSprite(cent, cgs.media.medalImpressive);
		return;
	}

	if (cent.currentState.eFlags & EF.AWARD_EXCELLENT) {
		PlayerFloatSprite(cent, cgs.media.medalExcellent);
		return;
	}

	if (cent.currentState.eFlags & EF.AWARD_GAUNTLET) {
		PlayerFloatSprite(cent, cgs.media.medalGauntlet);
		return;
	}

	if (cent.currentState.eFlags & EF.AWARD_DEFEND) {
		PlayerFloatSprite(cent, cgs.media.medalDefend);
		return;
	}

	if (cent.currentState.eFlags & EF.AWARD_ASSIST) {
		PlayerFloatSprite(cent, cgs.media.medalAssist);
		return;
	}

	if (cent.currentState.eFlags & EF.AWARD_CAP) {
...
function ResetPlayerEntity(cent) {
	...
	cent.pe.legs.reset();
	cent.pe.legs.yawAngle = cent.lerpAngles[QMath.YAW];
	cent.pe.legs.yawing = false;
	cent.pe.legs.pitchAngle = 0;
	cent.pe.legs.pitching = false;

	cent.pe.torso.reset();
	cent.pe.torso.yawAngle = cent.rawAngles[QMath.YAW];
	cent.pe.torso.yawing = false;
	cent.pe.torso.pitchAngle = cent.rawAngles[QMath.PITCH];
