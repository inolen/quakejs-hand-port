...
function LoadClientInfo(clientNum, ci, callback) {
	//if (!RegisterClientModelname(ci, ci.modelName, ci.skinName, ci.headModelName, ci.headSkinName, teamname)) {
		RegisterClientAssets(ci, DEFAULT_MODEL, 'default', 'default', function (err) {
			if (err) {
...
function PlayerPowerups(cent, torso) {
	...
	if (powerups & (1 << PW.QUAD)) {
		re.AddLightToScene(cent.lerpOrigin, 200 + Math.floor(Math.random() * 32), 0.2, 0.2, 1);
	}
	...
	if (powerups & (1 << PW.REDFLAG)) {
		...
		// }
		re.AddLightToScene(cent.lerpOrigin, 200 + Math.floor(Math.random() * 32), 1.0, 0.2, 0.2);
	}
	...
	if (powerups & (1 << PW.BLUEFLAG)) {
		...
		// }
		re.AddLightToScene(cent.lerpOrigin, 200 + Math.floor(Math.random() * 32), 0.2, 0.2, 1.0);
	}
	...
	if (powerups & (1 << PW.NEUTRALFLAG)) {
		...
		// }
		re.AddLightToScene(cent.lerpOrigin, 200 + Math.floor(Math.random() * 32), 1.0, 1.0, 1.0);
	}
...
function HasteTrail(cent) {
	...
	if (anim !== ANIMS.LEGS_RUN && anim !== ANIMS.LEGS_BACK) {
		return;
	}
