...
function ParseClientInfo(clientNum, callback) {
	...
	// Only load assets if the model / team has changed.
	var dirtyAssets = !ci.infoValid || ci.team !== cs.team;
	...
	ci.rank = cs.rank;
	ci.score = cs.score;
	ci.powerups = cs.powerups;
	ci.spectatorNum = cs.spectatorNum;
	ci.ping = cs.ping;
	ci.time = cs.time;
	ci.accuracy = cs.accuracy;
	ci.impressiveCount = cs.impressive;
	ci.excellentCount = cs.excellent;
	ci.gauntletCount = cs.gauntlet;
	ci.defendCount = cs.defend;
	ci.assistCount = cs.assist;
	ci.perfect = cs.perfect;
	ci.captures = cs.captures;
	ci.eliminated = cs.eliminated;

	UpdateScoreboard();

	// Early out if we don't need to update our assets.
	if (!dirtyAssets) {
		log('ParseClientInfo2', ci.infoValid, ci.team, cs.team);
		if (callback) {
			callback();
		}
		return;
