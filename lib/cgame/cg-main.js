...
function log() {
	// Add to HUD events.
	var hud_view = UI.GetView(UI.HudView);
	hud_view.addEvent('print', arguments[0]);
...
function Init(serverMessageNum, serverCommandSequence, clientNum) {
	...
	loading_view = UI.GetView(UI.LoadingView);

	LoadAssets(function () {
		// These views don't have a visible state, so wait and
		// create after loading is over.
		hud_view = UI.GetView(UI.HudView);
		scoreboard_view = UI.GetView(UI.ScoreboardView);
		ingame_view = UI.GetView(UI.IngameMenu);
...
function LoadAssets(callback) {
	...
	loading_view.mapname(cgs.mapname);
