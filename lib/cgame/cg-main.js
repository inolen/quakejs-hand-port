...
function log() {
	...
	} else if (arguments[0] === 'chat' || arguments[0] === 'tchat') {
		view_hud.addEvent({
			type: arguments[0],
			name: arguments[1],
			text: arguments[2]
...
function Init(serverMessageNum, serverCommandSequence, clientNum) {
	...
	// Parse the serverinfo config string so we have the bare essential
	// config values (e.g. map name). We'll parse the rest after our
	// first snapshot (so we can get score values and such).
	ParseServerinfo();
...
/**
 * GetArenaConfigstring
 */
function GetArenaConfigstring(arenaNum, key) {
	return cgs.gameState[key] ? cgs.gameState[key][arenaNum] : null;
}
