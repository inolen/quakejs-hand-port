...
function log() {
	// Add to HUD events.
	if (view_hud) {
		view_hud.addEvent('print', arguments[0]);
	}
...
function Init(serverMessageNum, serverCommandSequence, clientNum) {
	...
	view_hud        = UI.GetView(UI.HudViewModel);
	view_ingame     = UI.GetView(UI.IngameMenuModel);
	view_loading    = UI.GetView(UI.LoadingViewModel);
	view_scoreboard = UI.GetView(UI.ScoreboardViewModel);
...
function LoadAssets(callback) {
	...
	view_loading.mapName(cgs.mapname);
