...
function LoadWorld(progress) {
	cl.LoadBsp(cgs.mapname, function (err, world) {
		if (err) {
			return error(err);
		}
		progress(0.25);
		...
		re.LoadWorld(world, function (err) {
			progress(0.75);
...
function RegisterWorldModels(progress) {
	...
	var registerInlineModel = function (i) {
		...
		re.RegisterModel(name, function (err, hModel) {
...
function RegisterItem(itemNum, progress) {
	...
	if (item.models) {
		_.each(item.models, function (path, key) {
			re.RegisterModel(path, function (err, hModel) {
	...
	if (item.shaders) {
		_.each(item.shaders, function (path, key) {
			re.RegisterShader(path, function (err, hShader) {
	...
	if (item.gfx) {
		_.each(item.gfx, function (path, key) {
			ui.RegisterImage(path, function (err, hImage) {
	...
	if (item.sounds) {
		_.each(item.sounds, function (path, key) {
			snd.RegisterSound(item.sounds[key], function (err, hSound) {
...
function RegisterSounds(progress) {
	...
	_.each(sounds, function (path, key) {
		snd.RegisterSound(sounds[key], function (err, hSound) {
