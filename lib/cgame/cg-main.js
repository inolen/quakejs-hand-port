...
function Init(serverMessageNum, serverCommandSequence, clientNum) {
	...
	hud_view          = UI.CreateView(hud_model, UI.HudViewModel.template);
	loading_model     = new UI.LoadingViewModel();
	loading_view      = UI.CreateView(loading_model, UI.LoadingViewModel.template);
	scoreboard_model  = new UI.ScoreboardViewModel();
	scoreboard_view   = UI.CreateView(scoreboard_model, UI.ScoreboardViewModel.template);
...
function Frame(serverTime) {
	...
	cg.renderingThirdPerson = cg_thirdPerson.getAsInt() || cg.snap.ps.pm_type === PM.DEAD;
...
function HandleEscape() {
	...
	tab_model.addTab('Current game', currentgame_model,          UI.CurrentGameMenuModel.template);
	tab_model.addTab('Settings',     new UI.SettingsMenuModel(), UI.SettingsMenuModel.template);

	var tab_view = UI.CreateView(tab_model, UI.TabMenuModel.template);
