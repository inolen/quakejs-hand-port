...
function LoadAssets() {
	...
	var updateProgressBar = function () {
		...
		var percent = (total / finished) * 100;
		cg_connect.setProgress(percent);
 ...
 * RegisterCollision
 */
function RegisterCollision(callback) {
 ...
 * RegisterWorld
 */
function RegisterWorld(callback) {
...
function RegisterWorldModels(callback) {
	...
	for (var i = 1; i < numInlineModels; i++) {
		// Wrap in a closure so we can access i in the
 ...
 * RegisterItems
 */
function RegisterItems(callback) {
	var total = bg.ItemList.length - 1;
	var loaded = 0;

	for (var i = 1; i < bg.ItemList.length; i++) {
		RegisterItem(i, function (f) {
			if (f >= 1) {
				loaded++;
				f = 0;
			}

			callback((loaded + f) / total);
		});
	}
}

function RegisterItem(itemNum, callback) {
	...
	if (item.shaders) {
		_.each(item.shaders, function (path, key) {
			itemInfo.shaders[key] = re.RegisterShader(path);
			callback(++loaded / total);
		});
	}
	...
	if (item.gfx) {
		_.each(item.gfx, function (path, key) {
			itemInfo.gfx[key] = ui.RegisterImage(path);
			callback(++loaded / total);
		});
	}
 ...
 * RegisterShaders
 */
function RegisterShaders(callback) {
	var shaders = {
		// Powerup shaders.
	...
	_.each(shaders, function (path, key) {
		cgs.media[key] = re.RegisterShader(path);
		callback(++loaded / total);
	});
 ...
 * RegisterClients
 */
function RegisterClients(callback) {
	var total = 0;
