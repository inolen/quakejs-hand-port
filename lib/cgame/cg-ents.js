...
function AddPacketEntities() {
	...
	// if (Math.abs(vec3.length(ps.velocity)) > 0) {
	// 	log('velocity', ps.velocity[0], ps.velocity[1], ps.velocity[2]);
	// }
	// if (Math.abs(vec3.length(cg.predictedPlayerEntity.currentState.pos.trDelta)) > 0) {
	// 	log('trDelta', cg.predictedPlayerEntity.currentState.pos.trDelta[0], cg.predictedPlayerEntity.currentState.pos.trDelta[1], cg.predictedPlayerEntity.currentState.pos.trDelta[2]);
	// }
...
function CalcEntityLerpPositions(cent) {
	...
	if (cent !== cg.predictedPlayerEntity) {
		AdjustPositionForMover(cent.lerpOrigin, cent.currentState.groundEntityNum,
 ...
 */
function AdjustPositionForMover(origin_in, moverNum, fromTime, toTime, origin_out, angles_in, angles_out) {
	if (moverNum <= 0 || moverNum >= ENTITYNUM_MAX_NORMAL) {
		vec3.set(origin_in, origin_out);
		vec3.set(angles_in, angles_out);
		return;
	}

	var cent = cg.entities[moverNum];
	if (cent.currentState.eType !== ET.MOVER) {
		vec3.set(origin_in, origin_out);
		vec3.set(angles_in, angles_out);
		return;
	}

	var oldOrigin = vec3.create(), origin = vec3.create(), deltaOrigin = vec3.create();
	var oldAngles = vec3.create(), angles = vec3.create(), deltaAngles = vec3.create();

	bg.EvaluateTrajectory(cent.currentState.pos, fromTime, oldOrigin);
	bg.EvaluateTrajectory(cent.ccurrentState.apos, fromTime, oldAngles);

	bg.EvaluateTrajectory(cent.currentState.pos, toTime, origin);
	bg.EvaluateTrajectory(cent.currentState.apos, toTime, angles);

	vec3.subtract(origin, oldOrigin, deltaOrigin);
	vec3.subtract(angles, oldAngles, deltaAngles);

	vec3.add(origin_in, deltaOrigin, origin_out);
	vec3.add(angles_in, deltaAngles, angles_out);
...
function AddMissile(cent) {
	...
	AddRefEntityWithPowerups(refent, es, TEAM.FREE);
}
