...
function AdjustPositionForMover(origin_in, moverNum, fromTime, toTime, origin_out, angles_in, angles_out) {
	...
	BG.EvaluateTrajectory(cent.currentState.apos, fromTime, oldAngles);
...
function SetEntitySoundPosition(cent) {
	if (cent.currentState.solid === SOLID_BMODEL) {
		var v = cgs.inlineModelMidpoints[cent.currentState.modelIndex];
		var origin = vec3.add(cent.lerpOrigin, v, vec3.create());
		SND.UpdateEntityPosition(cent.currentState.number, origin);
	} else {
...
function AddMissile(cent) {
	...
	if (itemInfo.missileSound) {
		...
		SND.AddLoopingSound(cent.currentState.number, cent.lerpOrigin, /*velocity*/ undefined, itemInfo.missileSound);
	}
