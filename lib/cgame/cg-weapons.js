...
//			AddLoopingSound( cent.currentState.number, cent.lerpOrigin, vec3_origin, weapon.readySound );
	...
	nonPredictedCent = cg.entities[cent.currentState.clientNum];
	...
	if (true || ps || cg.renderingThirdPerson || cent.currentState.number != cg.predictedPlayerState.clientNum) {
		// add lightning bolt
...
// 	}
	...
	trace = Trace(muzzlePoint, endPoint, QMath.vec3_origin, QMath.vec3_origin, cent.currentState.number, MASK.SHOT);
	...
	if (trace.fraction < 1.0) {
		...
		vec3.sub(beam.oldOrigin, beam.origin, dir);
		vec3.normalize(dir);
		...
		angles[0] = Math.floor(Math.random() * 360); //rand() % 360;
		angles[1] = Math.floor(Math.random() * 360); //rand() % 360;
		angles[2] = Math.floor(Math.random() * 360); //rand() % 360;
