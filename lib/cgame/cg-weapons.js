...
function AddPlayerWeapon(parent, ps, cent/*, team*/) {
	...
	var itemInfo = GetItemInfoForWeapon(weaponNum);
	if (!itemInfo) {
		return; // WP.NONE
	}
...
//			AddLoopingSound( cent.currentState.number, cent.lerpOrigin, vec3origin, itemInfo.sounds.firing);
			cent.pe.lightningFiring = true;
		} else if (itemInfo.sounds.ready) {
//			AddLoopingSound( cent.currentState.number, cent.lerpOrigin, vec3origin, itemInfo.sounds.ready);
		}
	}
	...
	// 	if ( weaponInfo.flashDlightColor[0] || weapon.flashDlightColor[1] || weapon.flashDlightColor[2] ) {
...
function AddViewWeapon(ps) {
	if (ps.persistant[PERS.SPECTATOR_STATE] !== SPECTATOR.NOT) {
		return;
	}
...
// 	}
	...
	var itemInfo = GetItemInfoForWeapon(ps.weapon);
...
function FireWeapon(cent) {
	...
	var itemInfo = GetItemInfoForWeapon(es.weapon);
...
function MissileHitWall(weapon, clientNum, origin, dir, soundType) {
	...
	var itemInfo = GetItemInfoForWeapon(weapon);
...
function LightningBolt(cent, origin) {
	...
	var itemInfo = GetItemInfoForWeapon(WP.LIGHTNING);
	...
	var trace = Trace(muzzlePoint, endPoint, QMath.vec3origin, QMath.vec3origin, cent.currentState.number, MASK.SHOT);
...
function PlasmaTrail(cent, wi) {
	...
	var waterScale = 1.0;
	var itemInfo = GetItemInfoForWeapon(WP.PLASMAGUN);
...
// 	le.leMarkType = LEMT_NONE;
	...
	if (PointContents(refent.origin, -1) & CONTENTS.WATER) {
		waterScale = 0.10;
	}
...
function RailTrail(ci, start, end) {
	...
	var itemInfo = GetItemInfoForWeapon(WP.RAILGUN);
