...
function AddPlayerWeapon(parent, ps, cent) {
	...
	if (!ps) {
		...
		if (cg.snap.ps.clientNum !== cent.currentState.number) {
			// var zz = 0;
			// cent.pe.foobar = 13;
			// log('AddPlayerWeapon', cent.currentState.number, cent.pe.lightningFiring, (cent.currentState.eFlags & EF.FIRING));
		}
	}
...
function FireWeapon(cent) {
	...
	if (es.weapon === WP.LIGHTNING) {
		// if (cg.snap.ps.clientNum !== es.number && cent.pe.lightningFiring) {
		// 	var zz = 0;
		// 	log('FireWeapon', es.number, cent.pe.lightningFiring, (cent.currentState.eFlags & EF.FIRING));
		// }
...
// 	}
	...
	if (cg.entities[trace.entityNum].currentState.eType == ET.PLAYER) {
		...
		if (trace.surfaceFlags & SURF.METALSTEPS) {
