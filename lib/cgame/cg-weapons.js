...
function AddPlayerWeapon(parent, ps, cent) {
	...
	if (!ps) {
		...
		if ((cent.currentState.eFlags & EF.FIRING) && itemInfo.sounds.firing) {
			...
			snd.AddLoopingSound(cent.currentState.number, cent.lerpOrigin, QMath.vec3origin, itemInfo.sounds.firing);
			cent.pe.lightningFiring = true;
		} else if (itemInfo.sounds.ready) {
			snd.AddLoopingSound(cent.currentState.number, cent.lerpOrigin, QMath.vec3origin, itemInfo.sounds.ready);
		}
	}
...
function ShotgunPellet(start, end, skipNum) {
	...
	var tr = Trace(start, end, null, null, skipNum, MASK.SHOT);
...
function LightningBolt(cent, origin) {
	...
	// CPMA  "true" lightning.
	if ((cent.currentState.number === cg.predictedPlayerState.clientNum) && cg_trueLightning.getAsFloat() !== 0) {
		...
		for (var i = 0; i < 3; i++) {
			...
			angle[i] = cg.refdefViewAngles[i] + a * (1.0 - cg_trueLightning.getAsFloat());
			if (angle[i] < 0) {
				angle[i] += 360;
			}
			if (angle[i] > 360) {
	...
	var trace = Trace(muzzlePoint, endPoint, QMath.vec3origin, QMath.vec3origin,
...
function RailTrail(ci, start, end) {
	...
	le.startTime = cg.time;
	le.endTime = cg.time + cg_railTrailTime.getAsInt();
