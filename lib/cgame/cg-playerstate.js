...
function CheckAmmo() {
	...
	var weapons = cg.snap.ps.stats[STAT.WEAPONS];
	var total = 0;

	for (var i = WP.MACHINEGUN; i < WP.NUM_WEAPONS; i++) {
		if (!(weapons & (1 << i))) {
			continue;
		}

		// Can't go low when you have unlimited.
	...
	var previous = cg.lowAmmoWarning;
	if (total === 0) {
		cg.lowAmmoWarning = 2;
	} else {
		cg.lowAmmoWarning = 1;
	...
	if (cg.lowAmmoWarning !== previous) {
		SND.StartLocalSound(cgs.media.noAmmoSound/*, CHAN_LOCAL_SOUND*/);
	}
}
...
function ArenaChanged() {
	cgs.arena = cgs.arenas[cg.snap.ps.arenaNum];
