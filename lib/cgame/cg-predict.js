 ...
 * Trace
 */
function Trace(start, end, mins, maxs, passEntityNum, mask) {
...
function PredictPlayerState() {
	...
	cg.pmove.pmove_fixed = pmove_fixed.getAsInt();
	cg.pmove.pmove_msec = pmove_msec.getAsInt();
	...
	for (var cmdNum = oldest; cmdNum <= latest; cmdNum++) {
		...
		if (cg.predictedPlayerState.commandTime === oldPlayerState.commandTime) {
			if (cg.thisFrameTeleport) {
				...
				// if (cg_showmiss.getAsInt() && !vec3.equal(oldPlayerState.origin, adjusted)) {
		...
		if (cg.pmove.pmove_fixed) {
			cg.pmove.cmd.serverTime = ((cg.pmove.cmd.serverTime + pmove_msec.getAsInt()-1) / pmove_msec.getAsInt()) * pmove_msec.getAsInt();
		}
