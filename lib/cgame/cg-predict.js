...
// 			cmodel = exp.CM_InlineModel(es.modelIndex);
...
// 			cmodel = exp.CM_TempBoxModel(bmins, bmaxs);
// 			vec3.set(angles, [0, 0, 0]);
// 			vec3.set(cent.lerpOrigin, origin);
// 		}

// 		var trace = exp.CM_TransformedBoxTrace(start, end, mins, maxs, cmodel, mask, origin, angles);
...
function Trace(start, end, mins, maxs, skipNumber, mask) {
	var trace = exp.CM_BoxTrace(start, end, mins, maxs, 0, mask);
...
function InterpolatePlayerState(grabAngles) {
	...
	if (grabAngles) {
		var cmdNum = exp.CL_GetCurrentUserCmdNumber();
		var cmd = exp.CL_GetUserCmd(cmdNum);
...
function PredictPlayerState() {
	...
	var latest = exp.CL_GetCurrentUserCmdNumber();
	var oldest = latest - CMD_BACKUP + 1;
	var oldestCmd = exp.CL_GetUserCmd(oldest);
	...
	var latestCmd = exp.CL_GetUserCmd(latest);
	...
	for (var cmdNum = oldest; cmdNum <= latest; cmdNum++) {
		// Get the command.
		cg.pmove.cmd = exp.CL_GetUserCmd(cmdNum);
		...
		// predict several commands to get to the point we want to compaexp.RE_
