...
function CalcViewValues() {
	...
	// Add error decay.
	if (cg_errorDecay.getAsInt() > 0) {
		var t = cg.time - cg.predictedErrorTime;
		var f = (cg_errorDecay.getAsInt() - t) / cg_errorDecay.getAsInt();
		if (f > 0 && f < 1) {
...
function OffsetFirstPersonView() {
	...
	angles[QMath.PITCH] += delta * cg_runpitch.getAsFloat();

	delta = vec3.dot(cg.predictedPlayerState.velocity, cg.refdef.viewaxis[1]);
	angles[QMath.ROLL] -= delta * cg_runroll.getAsFloat();
	...
	delta = cg.bobFracSin * cg_bobpitch.getAsFloat() * speed;
	if (cg.predictedPlayerState.pm_flags & PMF.DUCKED) {
		delta *= 3;  // crouching
	}

	angles[QMath.PITCH] += delta;
	delta = cg.bobFracSin * cg_bobroll.getAsFloat() * speed;
	if (cg.predictedPlayerState.pm_flags & PMF.DUCKED) {
	...
	// Add bob height.
	bob = cg.bobFracSin * cg.xyspeed * cg_bobup.getAsFloat();
	if (bob > 6) {
		bob = 6;
	}
...
function OffsetThirdPersonView() {
	...
	var forwardScale = Math.cos( cg_thirdPersonAngle.getAsInt() / 180 * Math.PI);
	var sideScale = Math.sin(cg_thirdPersonAngle.getAsInt() / 180 * Math.PI);

	vec3.add(view, vec3.scale(forward, -cg_thirdPersonRange.getAsInt() * forwardScale, vec3.create()));
	vec3.add(view, vec3.scale(right, -cg_thirdPersonRange.getAsInt() * sideScale, vec3.create()));

	// Trace a ray from the origin to the viewpoint to make sure the view isn't
	// in a solid block. Use an 8 by 8 block to prevent the view from near clipping anything.
	var trace = new QShared.TraceResults();
	...
	cg.refdefViewAngles[QMath.YAW] -= cg_thirdPersonAngle.getAsInt();
}
/**
 * CalcFov
 *
...
function CalcFov() {
	...
	if (cg.predictedPlayerState.pm_type === PM.INTERMISSION) {
		...
		// Account for zooms.
		var zoomFov = cg_zoomFov.getAsFloat();
		if (zoomFov < 1) {
			zoomFov = 1;
		} else if (zoomFov > 160) {
			zoomFov = 160;
