...
function AddEntityEvent(cent, position) {
	...
	switch (event) {
		case EV.FOOTSTEP:
			...
			imp.snd_StartSound(es.origin, es.number, 'player/footsteps/step' + Math.ceil(Math.random() * 4));
			break;
			
		case EV.JUMP:
			// imp.snd_StartSound(es.origin, es.number, 'sound/player/sarge/jump1');
			break;
		
		case EV.CHANGE_WEAPON:
			// imp.snd_StartSound(es.origin, es.number, 'sound/weapons/change');
			break;
