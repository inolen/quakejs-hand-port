...
function AddEntityEvent(cent, position) {
	...
	switch (event) {
		case EV.FOOTSTEP:
			...
			imp.snd_StartSound(es.origin, es.number, imp.snd_RegisterSound('sound/player/footsteps/step' + Math.ceil(Math.random() * 4)));
			break;
			
		case EV.JUMP:
			imp.snd_StartSound(es.origin, es.number, imp.snd_RegisterSound('sound/player/sarge/jump1'));
			break;
		
		case EV.CHANGE_WEAPON:
			imp.snd_StartSound(es.origin, es.number, imp.snd_RegisterSound('sound/weapons/change'));
			break;
