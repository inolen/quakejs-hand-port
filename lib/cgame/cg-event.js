...
// 			trap_S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*jump1" ) );
			exp.SND_StartSound(es.origin, es.number, 'player/sarge/jump1');
			break;
		
		case EntityEvent.CHANGE_WEAPON:
			exp.SND_StartSound(es.origin, es.number, 'weapons/change');
			break;
		
		case EntityEvent.FIRE_WEAPON:
			switch(es.weapon) {
				case Weapon.GAUNTLET: // gauntlet
					exp.SND_StartSound(es.origin, es.number, 'weapons/melee/fstatck');
					break;
				
				case Weapon.MACHINEGUN: // machine gun
					exp.SND_StartSound(es.origin, es.number, 'weapons/machinegun/machgf' + Math.ceil(Math.random() * 4) + 'b');
					break;
					
				case Weapon.SHOTGUN: // shotgun
					exp.SND_StartSound(es.origin, es.number, 'weapons/shotgun/sshotf1b');
					break;
					
				case Weapon.ROCKET_LAUNCHER: // rocket
					exp.SND_StartSound(es.origin, es.number, 'weapons/rocket/rocklf1a');
					break;
				
				case Weapon.LIGHTNING: // lightning
					exp.SND_StartSound(es.origin, es.number, 'weapons/lightning/lg_fire');
					break;
				
				case Weapon.RAILGUN: // railgun
					exp.SND_StartSound(es.origin, es.number, 'weapons/railgun/railgf1a');
					break;
					
				case Weapon.PLASMAGUN: // plasma
					exp.SND_StartSound(es.origin, es.number, 'weapons/plasma/plasmx1a');
					break;
			}
