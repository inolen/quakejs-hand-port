	...
	switch (event) {
		...
		case EV.RAILTRAIL:
			...
			if (es.clientNum === cg.snap.ps.clientNum && !cg.renderingThirdPerson) {
				if (cg_drawGun.get() === 2) {
					vec3.add(es.origin2, vec3.scale(cg.refdef.viewaxis[1], 8, vec3.create()));
				} else if (cg_drawGun.get() === 3) {
					vec3.add(es.origin2, vec3.scale(cg.refdef.viewaxis[1], 4, vec3.create()));
				}
			}
			...
			switch (es.eventParm) {
				...
				case GTS.RED_TAKEN:  // CTF: red team took blue flag, 1FCTF: blue team took the neutral flag
					...
					if (cg.snap.ps.powerups[PW.BLUEFLAG] || cg.snap.ps.powerups[PW.NEUTRALFLAG]) {
						...
						if (cg.snap.ps.persistant[PERS.TEAM] === TEAM.BLUE) {
							if (cgs.arena.gametype === GT.NFCTF) {
		...
		case EV.GENERAL_SOUND:
			if (cgs.gameSounds[es.eventParm] !== undefined) {
				SND.StartSound(null, es.number, /*CHAN_VOICE,*/ cgs.gameSounds[es.eventParm]);
			} else {
				var s = Configstring('sounds:' + es.eventParm);
				SND.StartSound(null, es.number, /*CHAN_VOICE,*/ CustomSound(es.number, s));
			}
			break;

		case EV.GLOBAL_SOUND:  // play from the player's head so it never diminishes
			if (cgs.gameSounds[es.eventParm] !== undefined) {
				SND.StartSound(null, cg.snap.ps.clientNum, /*CHAN_AUTO,*/ cgs.gameSounds[es.eventParm]);
			} else {
				var s = Configstring('sounds:' + es.eventParm);
				SND.StartSound(null, cg.snap.ps.clientNum, /*CHAN_AUTO,*/ CustomSound(es.number, s));
			}
			break;
...
function PrintObituary(ent) {
	...
	if (attackerNum === cg.snap.ps.clientNum) {
		var s;
		if (cgs.arena.gametype < GT.TEAM) {
			s = 'You fragged ' + targetName + '\n' +
				PlaceString(cg.snap.ps.persistant[PERS.RANK] + 1) + ' place with ' +
				cg.snap.ps.persistant[PERS.SCORE];
		} else {
			s = 'You fragged ' + targetName;
		}

		hud_model.setCenterPrint(s); /* SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH */
	...
	if (!attackerInfo) {
		...
		if (targetNum === cg.snap.ps.clientNum) {
			hud_model.setCenterPrint('Fragged by ' + attackerName);
		}
	}
...
function PlaceString(rank) {
	...
	if (rank === 1) {
		t += '^' + QS.COLOR.BLUE + '1st^' + QS.COLOR.WHITE;
	} else if (rank === 2) {
		t += '^' + QS.COLOR.RED + '2nd^' + QS.COLOR.WHITE;
	} else if (rank === 3) {
		t += '^' + QS.COLOR.YELLOW + '3rd^' + QS.COLOR.WHITE;
	} else if (rank === 11) {
		t += '11th';
	} else if (rank === 12) {
		t += '12th';
