 ...
 * ParseTeamInfo
 */
function ParseTeamInfo(key, info) {
	var split = key.split(':');
	var arenaNum = parseInt(split[1], 10);
	var teamNum = parseInt(split[2], 10);
...
function ParseScores(val) {
	...
	for (var i = 0; i < val.scores.length; i++) {
		...
		switch (clientInfo.team) {
			case TEAM.FREE:
				scoreboard_model.addFreeScore(score.spectatorNum, clientInfo.name, score.score, score.ping, score.time, score.eliminated);
				break;
			case TEAM.RED:
				scoreboard_model.addRedScore(score.spectatorNum, clientInfo.name, score.score, score.ping, score.time, score.eliminated);
				break;
			case TEAM.BLUE:
				scoreboard_model.addBlueScore(score.spectatorNum, clientInfo.name, score.score, score.ping, score.time, score.eliminated);
				break;
			case TEAM.SPECTATOR:
				scoreboard_model.addSpecScore(score.spectatorNum, clientInfo.name, score.score, score.ping, score.time, score.eliminated);
				break;
		}
	}
}
