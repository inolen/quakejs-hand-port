...
function ParseServerinfo() {
	...
	ingame_model.gametype(BG.GametypeNames[cgs.gametype]);
...
function ParseScores(val) {
	...
	for (var i = 0; i < val.scores.length; i++) {
		...
		switch (s1.team) {
			case TEAM.FREE:
				scoreboard_model.addFreeScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
			case TEAM.RED:
				scoreboard_model.addRedScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
			case TEAM.BLUE:
				scoreboard_model.addRedScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
			case TEAM.SPECTATOR:
				scoreboard_model.addSpecScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
		}
	}
}
