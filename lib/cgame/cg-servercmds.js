...
function ParseServerinfo() {
	...
	cgs.arenas = serverInfo['g_arenas'].split(',');
...
function ParseWarmup() {
	var warmup = GetArenaConfigstring(cg.snap.ps.arenaNum, 'warmup');

	log('ParseWarmup', warmup, ConfigString('warmup'));
 ...
 * ParseScore1
 */
function ParseScore1() {
	cgs.score1 = GetArenaConfigstring(cg.snap.ps.arenaNum, 'score1');
}

/**
 * ParseScore2
 */
function ParseScore2() {
	cgs.score2 = GetArenaConfigstring(cg.snap.ps.arenaNum, 'score2');
}
