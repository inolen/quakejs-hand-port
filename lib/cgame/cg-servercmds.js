...
function ParseArenaInfo(key, info) {
	...
	if (key === 'flagstatus') {
		if (arena.gametype === GT.CTF) {
			// format is rb where its red/blue, 0 is at base, 1 is taken, 2 is dropped
			arena.redflag = info[0];
			arena.blueflag = info[1];
		} else if (arena.gametype === GT.NFCTF) {
			arena.neutralflag = info[0];
		}
	} else {
		arena[split[2]] = info;
	}
...
function ParseScores(val) {
	...
	for (var i = 0; i < val.scores.length; i++) {
		...
		scoreboard_model.addScore(score.clientNum === cg.snap.ps.clientNum, clientInfo.team, clientInfo.name, score.score,
...
function MapRestart() {
	...
	// FIXME - Should the server reset configstrings on
	// MapRestart instead of us making assumptions?
