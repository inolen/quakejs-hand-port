...
function ParseServerinfo() {
	...
	view_ingame.gametype(BG.GametypeNames[cgs.gametype]);
	view_scoreboard.gametype(BG.GametypeNames[cgs.gametype]);
}
...
function ParseScores(val) {
	...
	view_scoreboard.score1(val.scoreRed);
	view_scoreboard.score2(val.scoreBlue);
	view_scoreboard.resetScores();
	...
	for (var i = 0; i < val.scores.length; i++) {
		...
		switch (s1.team) {
			case TEAM.FREE:
				view_scoreboard.addFreeScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
			case TEAM.RED:
				view_scoreboard.addRedScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
			case TEAM.BLUE:
				view_scoreboard.addRedScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
			case TEAM.SPECTATOR:
				view_scoreboard.addSpecScore(s1.clientInfo.name, s1.score, s1.ping, s1.time, s1.eliminated);
				break;
		}
	}
}
