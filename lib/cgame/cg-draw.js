...
function Draw2D() {
	hud_model.gametype(BG.GametypeNames[cgs.arena.gametype]);
...
function UpdateScores() {
	...
	var score1 = cgs.arena.score1;
	var score2 = cgs.arena.score2;

	var ci = cgs.clientinfo[cg.snap.ps.clientNum];

	if (cgs.arena.gametype === GT.ROCKETARENA) {
	...
	else if (cgs.arena.gametype >= GT.TEAM) {
...
function UpdateWarmup() {
	...
	var gamestate = cgs.arena.gamestate;
	var warmup = cgs.arena.warmupTime;
	...
	// Counting down.
		...
		if (sec < 0) {
			...
			if (cgs.arena.gametype === GT.TOURNAMENT) {
			...
			else if (cgs.arena.gametype === GT.ROCKETARENA) {
				var ci1, ci2;
			...
			else if (cgs.arena.gametype === GT.CLANARENA) {
				// Figure out current round based on team scores.
				var round = cgs.arena.score1.score + cgs.arena.score2.score + 1;

				str += 'Red Team vs Blue Team\n';
				str += 'Round ' + round + ' of ' + cgs.arena.roundlimit + '\n';
				str += '^' + QS.COLOR.YELLOW + (sec + 1);
	...
	// Let everyone know who won.
		...
		hud_model.warmup(cgs.arena.winningTeam);
	}
...
function UpdateFlagStatus() {
	// // FIXME Move to general UpdatePowerups
	// var pw;
	// if (cg.predictedPlayerState.powerups[PW.REDFLAG]) {
	// 	pw = PW.REDFLAG;
	// } else if (cg.predictedPlayerState.powerups[PW.BLUEFLAG]) {
	// 	pw = PW.BLUEFLAG;
	// } else if (cg.predictedPlayerState.powerups[PW.NEUTRALFLAG]) {
	// 	pw = PW.NEUTRALFLAG;
	// }
	// if (pw === undefined) {
	// 	return;
	...
	// var itemInfo = BG.FindItemForPowerup(pw);
	// if (!itemInfo) {
	// 	return;
	// }
	// hud_model.setPowerup(pw, itemInfo.gfx.icon);

	var redFlagInfo = FindItemInfo(IT.TEAM, PW.REDFLAG);
	if (!redFlagInfo) {
		error('No item information for PW.REDFLAG');
		return;
	}
	var redState = cgs.arena.redflag >= 0 && cgs.arena.redflag <= 2 ? cgs.arena.redflag : 0;
	hud_model.redFlagIcon(redFlagInfo.gfx['icon' + redState]);
	...
	var blueState = cgs.arena.blueflag >= 0 && cgs.arena.blueflag <= 2 ? cgs.arena.blueflag : 0;
	hud_model.blueFlagIcon(blueFlagInfo.gfx['icon' + blueState]);
}
