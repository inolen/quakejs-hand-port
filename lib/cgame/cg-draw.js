...
function UpdateScores() {
	...
	if (arena.gametype === GT.ROCKETARENA) {
		hud_model.score1.visible(!!score1);
		hud_model.score1.localplayer(ci.group === score1.group);
		hud_model.score1.count(score1.alive);
		hud_model.score1.name(score1.group);
		hud_model.score1.score(score1.score);

		hud_model.score2.visible(!!score1);
		hud_model.score2.localplayer(ci.group === score2.group);
		hud_model.score2.count(score2.alive);
		hud_model.score2.name(score2.group);
 ...
 * as well as when a client's userinfo changes.
 */
function UpdateGameTeams() {
	...
	for (var i = 0; i < cgs.clientinfo.length; i++) {
