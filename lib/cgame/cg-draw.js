...
function HudUpdateScores() {
	...
	if (cgs.gametype >= GT.TEAM) {
		...
		hud_model.score1.score(arena.score1);
		hud_model.score1.count(arena.count1);

		hud_model.score2.localplayer(ci.team === TEAM.BLUE);
		hud_model.score2.score(arena.score2);
		hud_model.score2.count(arena.count2);
	} else {
...
function HudUpdateCrosshairNames() {
	...
	hud_model.crosshairName(cg.crosshairName || '');
}
...
function LagometerUpdate() {
	if (cg_drawLagometer.getAsInt()) {
		hud_model.lagometerVisible(true);
	} else {
		hud_model.lagometerVisible(false);
	}
}
...
function IngameUpdateArenas() {
	...
	ingame_model.currentTeam(cg.snap.ps.persistant[PERS.TEAM]);
	ingame_model.currentArenaNum(cg.snap.ps.arenaNum);

	for (var i = 0; i < cgs.arenas.length; i++) {
		var arena = ingame_model.getArena(i);
