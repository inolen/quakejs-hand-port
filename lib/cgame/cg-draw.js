...
function HudUpdateFPS() {
	...
	if (previousIdx > FPS_FRAMES) {
		...
		view_hud.model.setFPS(parseInt(1000 * FPS_FRAMES / total, 10));
	}
}
...
function HudUpdateWeapons() {
	...
	for (var i = 2; i < MAX_WEAPONS; i++) {
		if (!(bits & (1 << i))) {
			view_hud.model.setWeapon(i, null);
			continue;
		}

		view_hud.model.setWeapon(i, GetItemInfoForWeapon(i));
	}

	view_hud.model.setWeaponSelect(cg.weaponSelect);
}
...
function HudUpdateAmmo() {
	// Don't render WP.GAUNTLET.
	for (var i = 2; i < MAX_WEAPONS; i++) {
		view_hud.model.setAmmo(i, cg.snap.ps.ammo[i]);
	}
}
...
function HudUpdateArmor() {
	view_hud.model.setArmor(cg.snap.ps.stats[STAT.ARMOR]);
}
...
function HudUpdateHealth() {
	view_hud.model.setHealth(cg.snap.ps.stats[STAT.HEALTH]);
}
...
function HudUpdateScores() {
	...
	view_hud.model.setScore1(score1);
	view_hud.model.setScore2(score2);
}
...
function HudUpdateCrosshairNames() {
	...
	view_hud.model.setCrosshairName(alpha === 0 ? null : name, alpha);
}
...
function LagometerUpdate() {
	if (cg_drawLagometer()) {
		view_hud.model.showLagometer();
	} else {
		view_hud.model.hideLagometer();
	}
}
...
function LagometerAddFrame() {
	...
	view_hud.model.addLagometerFrame(offset);
}
...
function LagometerAddSnapshotInfo(snap) {
	...
	view_hud.model.addSnapshotFrame(snap.ping, snap.snapFlags);
}
