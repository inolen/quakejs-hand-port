...
function ScanForCrosshairEntity() {
	...
	if (cg_crosshairShaders()) {
		var trace = Trace(start, end, QMath.vec3origin, QMath.vec3origin,
			cg.snap.ps.arenaNum, cg.snap.ps.clientNum, CONTENTS.SOLID | CONTENTS.BODY | CONTENTS.DETAIL);
		cg.crosshairName = trace.shaderName;
		return;
	}

	var trace = Trace(start, end, QMath.vec3origin, QMath.vec3origin,
		cg.snap.ps.arenaNum, cg.snap.ps.clientNum, CONTENTS.SOLID | CONTENTS.BODY);
