...
function HudUpdateScores() {
	...
	if (cgs.gametype >= GT.TEAM) {
		info1 = { score: arena.score1, count: arena.count1, localplayer: ci.team === TEAM.RED };
		info2 = { score: arena.score2, count: arena.count2, localplayer: ci.team === TEAM.BLUE };
	} else {
		var ci1 = cgs.clientinfo[arena.count1];
		var ci2 = arena.score2 !== SCORE_NOT_PRESENT ? cgs.clientinfo[arena.count2] : null;

		info1 = { rank: 1, name: ci1.name, score: arena.score1, localplayer: ci1 === ci };

		// If we're not in first, always draw our own score in score2.
		if (!info1.localplayer) {
			...
			info2 = { rank: rank, name: ci.name, score: score, localplayer: true };
		} else if (ci2) {
			var tied = cg.snap.ps.persistant[PERS.RANK] & RANK_TIED_FLAG;
			var rank = tied ? 1 : 2;  // render both as 1 when tied

			info2 = { rank: rank, name: ci2.name, score: arena.score2, localplayer: false };
		}
	}

	view_hud.setGametype(BG.GametypeNames[cgs.gametype]);
	view_hud.setInfo1(info1);
	view_hud.setInfo2(info2);
}
...
function IngameUpdateArenas() {
	...
	view_ingame.setTeam(BG.TeamNames[cg.snap.ps.persistant[PERS.TEAM]]);
	view_ingame.setArena(cg.snap.ps.arenaNum);
