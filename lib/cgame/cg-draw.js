...
function UpdateScores() {
	...
	var team1 = arena.teams[arena.team1];
	var team2 = arena.teams[arena.team2];

	if (arena.gametype >= GT.TEAM) {
		hud_model.score1.visible(true);
		hud_model.score1.localplayer(ci.team === arena.team1);
		if (team1) {
			hud_model.score1.score(team1.score);
			hud_model.score1.count(arena.gametype >= GT.CLANARENA ? team1.alive : team1.count);
		}

		hud_model.score2.visible(true);
		hud_model.score2.localplayer(ci.team === arena.team2);
		if (team2) {
			hud_model.score2.score(team2.score);
			hud_model.score2.count(arena.gametype >= GT.CLANARENA ? team2.alive : team2.count);
		}
	} else {
		var ci1 = cgs.clientinfo[arena.team1];
		var ci2 = cgs.clientinfo[arena.team2];

		hud_model.score1.visible(!!team1);
		if (team1) {
		...
		hud_model.score2.visible(!!team2);
		if (ci1 !== ci) {
...
function UpdateScoreboard() {
	...
	for (var i = 0; i < MAX_CLIENTS; i++) {
		var info = cgs.clientinfo[i];
		var infoModel = scoreboard_model.getScore(i);

		infoModel.name(info.name);
		infoModel.team(info.team);
		infoModel.rank((info.rank & ~RANK_TIED_FLAG));
		infoModel.score(info.score);
		infoModel.ping(info.ping);
		infoModel.time(info.time);
		infoModel.eliminated(info.eliminated);
		infoModel.spectatorNum(info.spectatorNum);
	}
}
/**********************************************************
 *
...
function UpdateArena(arenaNum) {
	...
	// Update other views that depend on the gametype.
