...
function Draw2D() {
	...
	hud_view.gametype(BG.GametypeNames[arena.gametype]);
	hud_view.alive(cg.snap.ps.pm_type !== PM.INTERMISSION &&
...
function UpdateFPS() {
	...
	if (previousIdx > FPS_FRAMES) {
		...
		hud_view.fps(parseInt(1000 * FPS_FRAMES / total, 10));
	}
}
...
function UpdateCounts() {
	...
	hud_view.countsVisible(!!cg_drawLagometer.get());
	hud_view.shaders(counts.shaders);
	hud_view.nodes(counts.nodes);
	hud_view.leafs(counts.leafs);
	hud_view.surfaces(counts.surfaces);
	hud_view.indexes(counts.indexes);
	hud_view.culledModelOut(counts.culledModelOut);
	hud_view.culledModelIn(counts.culledModelIn);
	hud_view.culledModelClip(counts.culledModelClip);
}
...
function UpdateWeapons() {
	...
	for (var i = 2; i < MAX_WEAPONS; i++) {
		if (!(bits & (1 << i))) {
			hud_view.setWeaponIcon(i, null);
			continue;
		}

		hud_view.setWeaponIcon(i, FindItemInfo(IT.WEAPON, i).gfx.icon);
	}

	hud_view.weaponSelect(cg.weaponSelect);
}
...
function UpdateAmmo() {
	...
	for (var i = 2; i < MAX_WEAPONS; i++) {
		...
		hud_view.setWeaponAmmo(i, cg.snap.ps.ammo[i]);
	}
}
...
function UpdateArmor() {
	hud_view.armor(cg.snap.ps.stats[STAT.ARMOR]);
}
...
function UpdateHealth() {
	hud_view.health(cg.snap.ps.stats[STAT.HEALTH]);
}
...
function UpdateScores() {
	...
	if (arena.gametype === GT.ROCKETARENA) {
		hud_view.score1.localplayer(!score1 ? false : ci.group === score1.group);
		hud_view.score1.count(!score1 ? 0 : score1.alive);
		hud_view.score1.name(!score1 ? '' : score1.group);
		hud_view.score1.score(!score1 ? '' : score1.score);

		hud_view.score2.localplayer(!score2 ? false : ci.group === score2.group);
		hud_view.score2.count(!score2 ? 0 : score2.alive);
		hud_view.score2.name(!score2 ? '' : score2.group);
		hud_view.score2.score(!score2 ? '' : score2.score);
	}
	else if (arena.gametype >= GT.TEAM) {
		hud_view.score1.localplayer(ci.team === TEAM.RED);
		hud_view.score1.count(score1.count);
		hud_view.score1.name('Red Team');
		hud_view.score1.score(score1.score);

		hud_view.score2.localplayer(ci.team === TEAM.BLUE);
		hud_view.score2.count(score2.count);
		hud_view.score2.name('Blue Team');
		hud_view.score2.score(score2.score);
	} else {
		var ci1 = cgs.clientinfo[score1.clientNum];
		var ci2 = score2 ? cgs.clientinfo[score2.clientNum] : null;

		hud_view.score1.visible(true);
		hud_view.score1.localplayer(ci1 === ci);
		hud_view.score1.score(score1.score);
		hud_view.score1.rank(1);
		hud_view.score1.name(ci1.name);
		...
		if (ci1 !== ci) {
			...
			hud_view.score2.visible(true);
			hud_view.score2.localplayer(true);
			hud_view.score2.score(score);
			hud_view.score2.rank(rank);
			hud_view.score2.name(ci.name);
		} else if (ci2) {
			var tied = cg.snap.ps.persistant[PERS.RANK] & RANK_TIED_FLAG;
			var rank = tied ? 1 : 2;  // render both as 1 when tied

			hud_view.score2.visible(true);
			hud_view.score2.localplayer(false);
			hud_view.score2.score(score2.score);
			hud_view.score2.rank(rank);
			hud_view.score2.name(ci2.name);
		} else {
			hud_view.score2.visible(false);
		}
	}
}
...
function UpdateCrosshairNames() {
	...
	if (cg.crosshairName) {
		hud_view.setCrosshairName(cg.crosshairName);
	}
}
...
function UpdateWarmup() {
	...
	hud_view.gamestate(gamestate);

	// Waiting for players.
	if (gamestate === GS.WAITING) {
		hud_view.warmup('Waiting for players');
		return;
	}
	...
	// Counting down.
		...
		if (sec < 0) {
			hud_view.setCenterPrint('^' + QS.COLOR.RED + 'FIGHT!');
		} else {
			var str = '';
	...
	// Let everyone know who won.
		...
		hud_view.warmup(arena.winningTeam);
	}
...
function UpdateLagometer() {
	hud_view.lagometerVisible(!!cg_drawLagometer.get());
}
...
function AddLagometerFrame() {
	...
	hud_view.addLagometerFrame(offset);
}
...
function AddSnapshotInfo(snap) {
	...
	hud_view.addSnapshotFrame(snap.ping, snap.snapFlags);
}
...
function UpdateScoreboard() {
	...
	scoreboard_view.visible(true);
	scoreboard_view.mapname(arena.name === 'default' ? message : arena.name);
	scoreboard_view.gametype(BG.GametypeNames[arena.gametype]);
	scoreboard_view.timelimit(cgs.timelimit);
	scoreboard_view.fraglimit(cgs.fraglimit);
	scoreboard_view.capturelimit(cgs.capturelimit);
}
...
function UpdateGameArenas() {
	...
	ingame_view.currentgame.currentArenaNum(cg.snap.ps.arenaNum);

	for (var i = 0; i < cgs.arenas.length; i++) {
		var arena = cgs.arenas[i];
		var arenaModel = ingame_view.currentgame.getArena(i);
...
function UpdateGameGroups() {
	...
	//
	var ci = cgs.clientinfo[cg.snap.ps.clientNum];
	ingame_view.currentgame.currentGroup(ci.group);

	var arenaModel = ingame_view.currentgame.getArena(cg.snap.ps.arenaNum);
