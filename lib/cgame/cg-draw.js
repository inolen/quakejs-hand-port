...
function UpdateScores() {
	...
	if (arena.gametype >= GT.TEAM) {
		var team1 = score1.team === TEAM.NONE ? null : arena.teams[score1.team];
		var team2 = score2.team === TEAM.NONE ? null : arena.teams[score2.team];

		hud_model.score1.visible(!!team1);
		if (team1) {
			hud_model.score1.localplayer(ci.team === score1.team);
			hud_model.score1.count(arena.gametype === GT.CLANARENA ? score1.alive : score1.count);
			hud_model.score1.name(team1.name);
			hud_model.score1.score(team1.score);
		}

		hud_model.score2.visible(!!team2);
		if (team2) {
			hud_model.score2.localplayer(ci.team === score2.team);
			hud_model.score2.count(arena.gametype === GT.CLANARENA ? score2.alive : score2.count);
			hud_model.score1.name(arena.teams[score2.team].name);
			hud_model.score1.score(arena.teams[score2.team].score);
		}
	} else {
 ...
 * UpdateGameTeams
 *
 * Called whenever new teams are received from the server,
 * as well as when the user respawns.
 */
function UpdateGameTeams() {
	// No snapshot on initial configstring update.
	if (!cg.snap) {
		return;
	}
	...
	for (var i = 0; i < arena.teams.length; i++) {
		var team = arena.teams[i];
		var teamModel = arenaModel.getTeam(i);
