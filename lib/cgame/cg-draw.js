...
function UpdateScores() {
	...
	// rendering the hud.
	if (!score1 || score1.score === SCORE_NOT_PRESENT) {
	...
	if (cgs.gametype >= GT.TEAM) {
		...
		hud_model.score1.count(cgs.gametype === GT.CLANARENA ? score1.alive : score1.count);

		hud_model.score2.visible(true);
		hud_model.score2.localplayer(ci.team === TEAM.BLUE);
		hud_model.score2.score(score2.score);
		hud_model.score2.count(cgs.gametype === GT.CLANARENA ? score2.alive : score2.count);
	} else {
		var ci1 = cgs.clientinfo[score1.clientNum];
		var ci2 = score2.score !== SCORE_NOT_PRESENT ? cgs.clientinfo[score2.clientNum] : null;
