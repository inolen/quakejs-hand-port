 ...
 */
function HudUpdateWeaponSelect() {
	...
	for (var i = 0; i < MAX_WEAPONS; i++) {
		...
		cg_hud.model.setWeapon(i, cg.weaponInfo[i], i === cg.weaponSelect);
	}
}
 ...
 * HudUpdateKills
 */
function HudUpdateKills() {
	cg_hud.model.setKills(cg.snap.ps.persistant[PERS.SCORE]);
}

/**
 * HudUpdateTeam
 */
function HudUpdateTeam() {
	cg_hud.model.setTeam(cg.snap.ps.persistant[PERS.TEAM]);
}
