...
function MarkFragments(points, projection) {
	...
	//
	var projectionDir = vec3.normalize(projection, vec3.create());
	...
	for (var i = 0; i < points.length; i++) {
		...
		vec3.add(points[i], vec3.scale(projectionDir, -20, vec3.create()), temp);
		QMath.AddPointToBounds(temp, mins, maxs);
	}
		...
		if (face.surfaceType === SF.FACE) {
			...
			for (var j = 0; j < face.meshVertCount; j += 3) {
				for (var k = 0; k < 3 ; k++) {
					...
					clipPoints[k] = vec3.create(v.pos);
				}
...
function BoxSurfaces_r(node, mins, maxs, dir, list) {
	...
	for (var i = 0; i < node.numLeafSurfaces; i++) {
		...
		if ((face.shader.surfaceFlags & (SURF.FLAGS.NOIMPACT | SURF.FLAGS.NOMARKS)) ||
			(face.shader.contentFlags & SURF.CONTENTS.FOG)) {
		...
		// Extra check for surfaces to avoid list overflows
			...
			if (s === QMath.SIDE_FRONT || s === QMath.SIDE_BACK) {
				face.viewCount = re.viewCount;
...
function ChopPolyBehindPlane(points, plane, epsilon) {
	...
	if (!counts[SIDE_BACK]) {
		for (var i = 0; i < points.length; i++) {
			out.push(vec3.set(points[i], vec3.create()));
		}
		return out;
	}
	...
	for (var i = 0; i < points.length; i++) {
		...
		if (sides[i] === SIDE_ON) {
			out.push(vec3.set(p1, vec3.create()));
			continue;
		}
		...
		out.push(
			vec3.add(
				vec3.scale(
					vec3.subtract(p2, p1, vec3.create()),
					dot
				),
			p1)
		);
