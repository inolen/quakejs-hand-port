...
function FindModelByName(name, bmodel) {
	...
	if (bmodel !== null) {
		...
		LoadMd3WithLod(mod, name, function () {
			...
			// Trigger all callbacks once the model and its shaders have loaded.
			var promises = [];
			for (var i = 0; i < mod.numLods; i++) {
				var md3 = mod.md3[i];
				for (var j = 0; j < md3.surfaces.length; j++) {
					var surf = md3.surfaces[j];
					for (var k = 0; k < surf.shaders.length; k++) {
						promises.push(surf.shaders[k].promise);
					}
				}
			}
		...
		(function (lod) {
			LoadMd3(lodFilename, function (err, md3) {
				...
				if (err) {
					...
					// The highest loaded lod will be our numLods,
