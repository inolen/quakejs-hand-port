...
function FindModelByName(name, bmodel) {
	...
	if (bmodel !== null) {
		...
		LoadMd3WithLod(mod, name, function () {
 ...
 * LoadMd3WithLod
 */
function LoadMd3WithLod(mod, name, callback) {
	var done = 0;
	...
	var checkComplete = function () {
		...
		for (var i = 0; i < mod.md3.length; i++) {
			if (mod.md3[i]) {
				mod.numLods = i + 1;
			}
		}
	...
	var loadLOD = function (lod) {
		...
		var md3 = LoadMd3(lodFilename);

		md3.promise.fail(function (err) {
			log(err.message);
		}).done(function (md3) {
			mod.md3[lod] = md3;
...
function BatchMd3Surfaces(md3) {
	...
	for (var i = 0; i < md3.surfaces.length; i++) {
		...
		// the diffuse colors in TessellateCompiledMd3.
		...
		// Overwrite the current backend cmd so TessellateMd3
		// writes to us.
		backend.tess = batch.cmd;

		var offset = index.elementCount;
		TessellateMd3(surface);
