...
function RegisterModel(name, bmodel, callback) {
	...
	FindModelByName(name, bmodel, function (mod) {
...
function FindModelByName(name, bmodel, callback) {
	...
	if (modelCallbacks[name]) {
		modelCallbacks[name].push(callback);
		return;
	}
	...
	if (bmodel !== null) {
		...
		// There's no queue of callbacks for these.
		return callback(mod);
	}

	// Initialize the callback stack.
	modelCallbacks[name] = [callback];

	// Assume this is an md3.
	...
	LoadMd3(filename, function (err, md3) {
		...
		var callbacks = modelCallbacks[name];
