...
function InitBackend() {
	...
	backend.tessFns[SF.FACE] = TessellateFace;
	backend.tessFns[SF.FACEBATCH] = TessellateFaceBatch;
	// backend.tessFns[SF.GRID] = TessellateFace;
	backend.tessFns[SF.TRIANGLES] = TessellateFace;
	backend.tessFns[SF.MD3] = TessellateMd3;
	backend.tessFns[SF.MD3BATCH] = TessellateMd3Batch;
	backend.tessFns[SF.ENTITY] = TessellateEntity;
}
...
function RenderDrawSurfaces(firstDrawSurf, lastDrawSurf) {
	...
	for (var i = firstDrawSurf; i < lastDrawSurf; i++) {
		...
		// If we're changing material or noving to a new, non-mergable entity,
		// break the batch.
		if (surf.batch !== oldBatch ||
			shader !== oldShader ||
			(entityNum !== oldEntityNum && !entityMergable)) {
			if (oldShader) {
				EndSurface();
			}
		...
		// NOTE: The important of this conditional is important, the same batch
		// may follow the current batch but from a different refent.
		else if (surf.batch && surf.batch === oldBatch) {
			continue;
		}

		// Even if we didn't break the batch, we may possibly need to
		// update the backend matrices for sprites to use when tessellating.
		...
		// Call the appropriate tessellation function.
		if (surf.batch) {
...
function StageIteratorGeneric() {
	...
	gl.drawElements(gl.TRIANGLES, indexCount, gl.UNSIGNED_SHORT, indexOffset * 2);  // offset is in bytes
