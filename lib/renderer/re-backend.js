...
function RenderDrawGeometry(first, last) {
	...
	for (var i = first; i < last; i++) {
		...
		var mergable = shader === oldShader;

		if (mergable && !shader.entityMergable) {
			mergable = false;

			// HACK Even if the entity isn't defined to be mergable, we can be presumptuous
			// and merge together non-models who have the same timeSecs (as they'll be on
			// the animation frame). This helps out with fast-firing weapons such as the
			// plasma gun as well as impact marks.
			//
			// FIXME Being that we're making all these guesses, perhaps this should be
			// refactored so that the individual Update* functions are informing us if
			// we need to break.
			if (backend.currentEntity) {
				var timeSecs = originalTime - backend.currentEntity.shaderTime;

				if (backend.currentEntity.reType !== RT.MODEL && backend.shaderTime === timeSecs) {
					mergable = true;
				}
			}
		}
...
function BindShader(shader) {
	...
	if (shader.cull) {
		var cull = shader.cull;

		// Flip culling in mirrors.
		if (backend.viewParms.isMirror) {
			cull = cull === gl.FRONT ? gl.BACK : gl.FRONT;
		}
...
function BindShaderStage(stage) {
	...
	// Bind textures.
	if (stage !== lastStage || backend.shaderTime !== lastShaderTime) {
		for (var i = 0; i < stage.textures.length; i++) {
			...
			var frame = 0;
			if (animFreq) {
				frame = Math.floor(backend.shaderTime * animFreq) % textures.length;
			}
		...
		lastShaderTime = backend.shaderTime;
	}
