...
function RenderDrawSurfaces(firstDrawSurf, lastDrawSurf) {
	...
	for (var i = firstDrawSurf; i < lastDrawSurf; i++) {
		...
		if (backend.forceRender || shader !== oldShader || (entityNum !== oldEntityNum && !shader.entityMergable)) {
			if (oldShader) {
				EndSurface();
			}
...
function BeginSurface(shader) {
	...
	// Reassign default buffers and reset counts.
...
function StageIteratorGeneric() {
	...
	for (var i = 0; i < shader.stages.length; i++) {
		...
		// Sanity check.
		// for (var j = indexOffset; j < indexOffset + numIndexes; j++) {
		// 	var idx = tess.index[j];
		// 	if ((tess.xyz.elementCount && idx > tess.xyz.elementCount) || 
		// 		(tess.normal.elementCount && idx > tess.normal.elementCount) ||
		// 		(tess.texCoord.elementCount && idx > tess.texCoord.elementCount) ||
		// 		(tess.lightCoord.elementCount && idx > tess.lightCoord.elementCount) ||
		// 		(tess.color.elementCount && idx > tess.color.elementCount)) {
		// 		debugger;
		// 	}
		// }
