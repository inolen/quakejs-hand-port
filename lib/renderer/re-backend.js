...
function DrawTris() {
	...
	if (!tess.xyz) {
		com.error(sh.Err.DROP, 'Can\'t draw triangles without xyz.');  // shouldn't happen
	}
...
function DrawNormals() {
	...
	if (!tess.xyz || !tess.normal) {
		com.error(sh.Err.DROP, 'Can\'t draw normal without xyz and normal.');  // shouldn't happen
	}
...
function SetShaderStage(shader, stage) {
	...
	if (isNaN(tess.shaderTime)) {
		com.error(sh.Err.DROP, 'Invalid time for shader');
	}
