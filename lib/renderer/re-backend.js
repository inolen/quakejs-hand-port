...
function RenderDrawGeometry(first, last) {
	...
	for (var i = first; i < last; i++) {
		...
		// Merge all RT.POLY into the same batch.
		var entityMergable = backend.currentEntity && backend.currentEntity.reType === RT.POLY;

		// We start a new batch for each geo, unless it's a mergable refent.
		if (!entityMergable || (entityMergable && oldEntityNum !== entityNum)) {
			if (oldEntityNum !== -1) {
				EndBatch();
			}
...
function BindBuffers(geo, shader) {
	...
	for (var name in attributes) {
		...
		// ignore it.
		if (attribute.type !== gl.ELEMENT_ARRAY_BUFFER &&
