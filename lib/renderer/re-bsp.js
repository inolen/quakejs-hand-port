...
function LoadMap(mapName, callback) {
	...
	sys.ReadFile('maps/' + mapName + '.cfg', 'utf8', function (err, data) {
		if (!err) {
			re.world.cfg = JSON.parse(data);
		}
		...
		sys.ReadFile('maps/' + mapName + '.bsp', 'binary', function (err, data) {
			...
			LoadFogs(data, header.lumps[sh.Lumps.FOGS]);
...
function LoadLightmaps(buffer, lightmapLump) {
	...
	re.lightmapTexture = CreateTexture('*lightmap', re.world.lightmaps, textureSize, textureSize);
}
...
function LoadSubmodels(buffer, modelLump) {
	...
	for (var i = 0; i < models.length; i++) {
		...
		if (i === 0) {
			continue;  // world model doesn't need anything else
		}

		model.firstSurface = firstSurface;
		model.numSurfaces = numSurfaces;

		// Register the brush model.
		FindModelByName('*' + i, model);
	}
}
...
function LoadLightGrid(buffer, gridLump) {
	...
	if (gridLump.filelen !== numGridPoints * 8) {
		log('WARNING: light grid mismatch', gridLump.filelen, numGridPoints * 8);
 ...
 * LoadFogs
 */
function LoadFogs(buffer, lump) {
	var bb = new ByteBuffer(buffer, ByteBuffer.LITTLE_ENDIAN);
	bb.index = lump.fileofs;

	var fogs = re.world.fogs = new Array(lump.filelen / sh.dfog_t.size);

	for (var i = 0; i < fogs.length; i++) {
		var fog = fogs[i] = new sh.dfog_t();
