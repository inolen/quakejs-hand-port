	...
	for (var rot = 0; rot < 2; rot++) {
		for (var j = 0; j + 2 < width; j += 2) {
			...
			for (var i = 0; i < height; i++) {
				...
				var dir = vec3.subtract(ctrl[i][j+2].pos, ctrl[i][j].pos, vec3.create());
				vec3.normalize(dir);

				var d = vec3.dot(midxyz, dir);
				var projected = vec3.scale(dir, d, vec3.create());
			...
			for (var i = 0; i < height; i++ ) {
				var prev = {pos: vec3.create(), lmCoord: [0,0], texCoord: [0, 0], color: [0, 0, 0, 0], normal: [0, 0, 1]};
				var next = {pos: vec3.create(), lmCoord: [0,0], texCoord: [0, 0], color: [0, 0, 0, 0], normal: [0, 0, 1]};
				var mid =  {pos: vec3.create(), lmCoord: [0,0], texCoord: [0, 0], color: [0, 0, 0, 0], normal: [0, 0, 1]};
		...
		// Transpose the array and tessellate in the other direction.
