...
function AddDrawSurf(surf, shader, fogIndex) {
	...
	backend.drawSurfs[index].sort = (shader.sortedIndex << QSORT_SHADERNUM_SHIFT) |
	...
	for (var i = 0; i < re.refdef.numRefEnts; i++) {
		...
		switch (refent.reType) {
			...
			case RT.RAIL_CORE:
				...
				AddDrawSurf(entitySurface, shader, 0);
				break;
			...
			default:
				com.Error('AddEntitySurfaces: Bad reType');
				break;
		}
	}
}
