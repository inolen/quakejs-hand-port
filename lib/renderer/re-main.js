...
function SetupFrustum() {
	...
	vec3.add(parms.frustum[0].normal, vec3.scale(parms.or.axis[1], xc, [0, 0, 0]));

	vec3.scale(parms.or.axis[0], xs, parms.frustum[1].normal);
	vec3.add(parms.frustum[1].normal, vec3.scale(parms.or.axis[1], -xc, [0, 0, 0]));
	...
	vec3.add(parms.frustum[2].normal, vec3.scale(parms.or.axis[2], xc, [0, 0, 0]));

	vec3.scale(parms.or.axis[0], xs, parms.frustum[3].normal);
	vec3.add(parms.frustum[3].normal, vec3.scale(parms.or.axis[2], -xc, [0, 0, 0]));
...
function SetFarClip() {
	...
	for (var i = 0; i < 8; i++) {
		...
		var vecTo = vec3.subtract(v, parms.or.origin, [0, 0, 0]);
...
function RenderView(parms) {
	...
	GenerateDrawSurfs();
	var lastDrawSurf = re.refdef.numDrawSurfs;

	// Only sort the surfaces we've added (e.g. because of a portal).
	SortDrawSurfaces(firstDrawSurf, lastDrawSurf);
	...
	for (var i = firstDrawSurf; i < lastDrawSurf; i++) {
		...
		if (MirrorViewBySurface(drawSurf, entityNum)) {
	...
	RenderDrawSurfaces(firstDrawSurf, lastDrawSurf);
}
 ...
 * AddDrawSurf
 */
function AddDrawSurf(surf, shader/*, fogIndex*/) {
	...
	backend.drawSurfs[index].surface = surf;
	...
	for (var i = 0; i < md3.surfaces.length; i++) {
		...
		if (refent.customShader) {
			...
			if (face.shaders.length <= 0) {
				...
				shader = face.shaders[refent.skinNum % face.shaders.length];
			}
		}
