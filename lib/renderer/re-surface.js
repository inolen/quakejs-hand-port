 ...
 * TessellateFaceBatch
 */
function TessellateFaceBatch(compiled) {
	var tess = backend.tess;
...
function TessellateMd3(face) {
	...
	for (var i = 0; i < numVerts; i++) {
		if (precompiling) {
			vec3.set(face.xyz[i], newXyz);
			vec3.set(face.normals[i], newNormal);
		} else {
			// Lerp xyz / normal.
			vec3.set(face.xyz[refent.oldFrame * numVerts + i], oldXyz);
			vec3.set(face.normals[refent.oldFrame * numVerts + i], oldNormal);

			vec3.set(face.xyz[refent.frame * numVerts + i], newXyz);
			vec3.set(face.normals[refent.frame * numVerts + i], newNormal);
		...
		texCoord.data[texCoord.offset++] = face.st[i][0];
		texCoord.data[texCoord.offset++] = face.st[i][1];
	...
	//
	for (var i = 0; i < face.indexes.length; i++) {
		index.data[index.offset++] = xyzOffset + face.indexes[i];
	}
 ...
 * TessellateMd3Batch
 */
function TessellateMd3Batch(compiled) {
	var tess = backend.tess;
	var refent = backend.currentEntity;
	var cmd = compiled.cmd;
	...
	var color = tess.color;
	var newColor = [0, 0, 0, 0];
	var numVerts = compiled.numVerts;

	// Use the original color offset so we match up with the index buffer.
	var offset = compiled.colorOffset;

	for (var i = 0; i < numVerts; i++) {
		var normal = compiled.normals[i];

		CalcDiffuseColor(refent, normal, newColor);
 ...
 * TessellateEntity
 */
function TessellateEntity(face) {
 ...
 * TessellateLightningBolt
 */
function TessellateLightningBolt() {
	var refent = backend.currentEntity;
