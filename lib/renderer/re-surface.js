 ...
 * CheckOverflow
 *
 * This feels very legacy, perhaps SHADER_MAX_VERTEXES
 * should just be 0xFFFF and leave this purely for sanity.
 */
function CheckOverflow(numVerts, numIndexes) {
	var tess = backend.tess;

	if (tess.xyz.elementCount + numVerts < SHADER_MAX_VERTEXES &&
		tess.index.elementCount + numIndexes < SHADER_MAX_INDEXES) {
		return;
	}
	...
	if (numVerts >= SHADER_MAX_VERTEXES) {
		error('CheckOverflow: numVerts > MAX (', numVerts, '>', SHADER_MAX_VERTEXES, ')');
	}
	if (numIndexes >= SHADER_MAX_INDEXES) {
		error('CheckOverflow: numIndexes > MAX (', numIndexes, '>', SHADER_MAX_INDEXES, ')');
	}

	BeginSurface(tess.shader);
}
...
function TesselateFace(face) {
	...
	CheckOverflow(face.vertCount, face.meshVertCount);
...
function TesselateCompiledFace(compiled) {
	...
	if (tess.elementCount !== 0) {
		error('Compiled surfaces should never be tesselated twice.');
	}
