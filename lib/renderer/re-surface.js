...
function TesselateLightningBolt () {
	var e = backend.currentEntity;
	var right;
	
	var end = vec3.set(e.oldOrigin, [0, 0, 0]);
	var start = vec3.set(e.origin, [0, 0, 0]);
	
	// compute variables
	var vec = vec3.subtract(end, start, [0, 0, 0]);
	var len = vec3.normalize(vec);
