...
function UpdateMd3Geometry(geo) {
	...
	for (var i = 0; i < numVerts; i++) {
		...
		// Get vertex color from lightgrid data.
		CalcDiffuseColor(refent, newNormal, newColor);

		color.array[color.offset++] = newColor[0];
		color.array[color.offset++] = newColor[1];
		color.array[color.offset++] = newColor[2];
		color.array[color.offset++] = newColor[3];
	}
