...
function TesselatePoly() {
	...
	for (var i = 0; i < refent.numPolyVerts; i++) {
		...
		bxyz.data[bxyz.offset++] = p.xyz[0];
		bxyz.data[bxyz.offset++] = p.xyz[1];
		bxyz.data[bxyz.offset++] = p.xyz[2];

		btexCoord.data[btexCoord.offset++] = p.st[0];
		btexCoord.data[btexCoord.offset++] = p.st[1];

		bcolor.data[bcolor.offset++] = p.modulate[0];
		bcolor.data[bcolor.offset++] = p.modulate[1];
		bcolor.data[bcolor.offset++] = p.modulate[2];
		bcolor.data[bcolor.offset++] = p.modulate[3];
	}

	// Generate fan indexes into the tess array.
	for (var i = 0; i < refent.numPolyVerts-2; i++) {
		bindex.data[bindex.offset++] = indexOffset + 0;
		bindex.data[bindex.offset++] = indexOffset + i + 1;
		bindex.data[bindex.offset++] = indexOffset + i + 2;
	}
