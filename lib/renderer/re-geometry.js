...
function MarkLeaves() {
	...
	if (re.viewCluster === -1) {
		for (var i = 0, numNodes = nodes.length; i < numNodes; i++) {
			if (nodes[i].contents !== SURF.CONTENTS.SOLID) {
...
function AddEntityGeometry() {
	for (var i = 0; i < re.refdef.numRefEnts; i++) {
		...
		switch (refent.reType) {
			...
			default:
				error('AddEntityGeos: BadGeometry');
				break;
		}
	}
}
...
function UpdateRailCore(geo) {
	...
	DoRailCore(geo, start, end, right, r_railCoreWidth.get(), refent.shaderRGBA);
}
