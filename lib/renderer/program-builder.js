...
/**
 * Generic helper for building GLSL programs programmatically
 */
...
ProgramBuilder.prototype.addDefine = function (name, data) {
	this.defines[name] = data;
};

ProgramBuilder.prototype.addAttrib = function(name, type) {
	this.attribs[name] = type;
};

ProgramBuilder.prototype.addVarying = function(name, type) {
	this.varyings[name] = type;
};

ProgramBuilder.prototype.addUniform = function (name, data, prefix, suffix) {
	this.uniforms[name] = {
		data: data,
		prefix: prefix,
		suffix: suffix
...
ProgramBuilder.prototype.addVertexFunction = function (name, src) {
	this.vertexFunctions[name] = src;
};

ProgramBuilder.prototype.addFragmentFunction = function (name, src) {
	this.fragmentFunctions[name] = src;
};

ProgramBuilder.prototype.addVertexLines = function (lines) {
	for (var i = 0; i < lines.length; i++) {
		this.vertexLines.push(lines[i]);
	}
};

ProgramBuilder.prototype.addFragmentLines = function (lines) {
	for (var i = 0; i < lines.length; i++) {
		this.fragmentLines.push(lines[i]);
	}
};

ProgramBuilder.prototype.createWaveform = function (name, wf, vertex) {
	if (!wf) {
...
ProgramBuilder.prototype.addSquareFunc = function(vertex) {
	var func = [
...
ProgramBuilder.prototype.addTriangleFunc = function(vertex) {
	var func = [
...
ProgramBuilder.prototype.createBlend = function(srcVar, destVar, srcFunc, destFunc) {
	var srcCode;
	var destCode;
...
ProgramBuilder.prototype.getVertexSource = function() {
	var src = '#ifdef GL_ES\n' +
...
ProgramBuilder.prototype.getFragmentSource = function() {
	var src = '#ifdef GL_ES\n' +
		'	precision highp float;\n' +
		'#endif\n';
