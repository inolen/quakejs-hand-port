...
function BuildSkyBuffers() {
	...
	var skyVec = [0, 0, 0];
	var worldMins = [-256, -256, -256];
	var worldMaxs = [256, 256, 256];

	for (var i = 0; i < verts.length; i++) {
		QMath.AddPointToBounds(verts[i].pos, worldMins, worldMaxs);
	}

	for (var i = 0; i < 6; i++) {
		var offset = xyz.elementCount;

		// Use the real world bounds to compute the xyz.
		for (var t = 0; t <= SKY_SUBDIVISIONS; t++) {
			for (var s = 0; s <= SKY_SUBDIVISIONS; s++) {
				MakeSkyVec(s / SKY_SUBDIVISIONS, t / SKY_SUBDIVISIONS, i, worldMins, worldMaxs, skyVec);
		...
		// Compute the ST with a constant set of mins / maxs.
		for (var t = 0; t <= SKY_SUBDIVISIONS; t++) {
			for (var s = 0; s <= SKY_SUBDIVISIONS; s++) {
