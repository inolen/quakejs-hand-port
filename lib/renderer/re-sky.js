...
function UpdateSkydomeGeometry(geo) {
	...
	InitBuffers(geo, vertCount, indexCount, {
		'index':    'uint',
		'position': 'vec3',
		'texCoord': 'vec2'
	});
	...
	for (var i = 0; i < 6; i++) {
		...
		for (var t = 0; t <= SKY_SUBDIVISIONS; t++) {
			for (var s = 0; s <= SKY_SUBDIVISIONS; s++) {
				...
				position.array[position.offset++] = skyVec[0];
				position.array[position.offset++] = skyVec[1];
				position.array[position.offset++] = skyVec[2];
		...
		for (var t = 0, height = SKY_SUBDIVISIONS+1; t < height-1; t++) {
			for (var s = 0, width = SKY_SUBDIVISIONS+1; s < width-1; s++) {
				index.array[index.offset++] = positionOffset + s + t * width;
				index.array[index.offset++] = positionOffset + s + ( t + 1 ) * width;
				index.array[index.offset++] = positionOffset + s + 1 + t * width;
				index.array[index.offset++] = positionOffset + s + ( t + 1 ) * width;
				index.array[index.offset++] = positionOffset + s + 1 + ( t + 1 ) * width;
				index.array[index.offset++] = positionOffset + s + 1 + t * width;
			}
		}
	}
