...
function LoadWorld(world, callback) {
	...
	LoadSurfaces(world.surfaces, world.verts, world.indexes, function (err) {
		// Load lightmaps after surfaces are done so we can
		// patch surface lm coords.
...
function ShaderForShaderNum(shaderNum, lightmapNum, callback) {
	...
	FindShaderByName(dsh.shaderName, lightmapNum, callback);
}
...
function LoadSurfaces(surfaces, verts, meshVerts, callback) {
	...
	surfaces.forEach(function (surface, i) {
		...
		steps.push(function (cb) {
			ShaderForShaderNum(surface.shaderNum, surface.lightmapNum, function (err, shader) {
...
function LoadBrushModels(models) {
	...
	for (var i = 0; i < models.length; i++) {
		...
		FindModelByName('*' + i, models[i], function (err) {});
	}
}
