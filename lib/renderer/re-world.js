 ...
 */
function LoadWorld(bsp, callback) {
	var world = re.world = new RenderWorld();

	world.shaders = bsp.shaders;
	world.planes = bsp.planes;
	world.numClusters = bsp.numClusters;
	world.clusterBytes = bsp.clusterBytes;
	world.vis = bsp.vis;

	// Load certain data into more appropriate structures.
	LoadNodesAndLeafs(bsp.nodes, bsp.leafs, bsp.leafSurfaces);
	LoadBrushModels(bsp.bmodels);
	LoadLightmaps(bsp.lightmaps);
	LoadLightGrid(bsp.lightGridData,
		bsp.lightGridOrigin,
		bsp.lightGridSize,
		bsp.lightGridInverseSize,
		bsp.lightGridBounds);

	// We need to wait for the surfaces to load (which involves
	// async shader requests).
	LoadSurfaces(bsp.surfaces, bsp.verts, bsp.indexes, function () {
		CreateWorldGeometry();

		for (var i = 1; i < world.bmodels.length; i++) {
			CreateBModelGeometry(world.bmodels[i]);
		}
...
function ColorShiftLightingBytes(color, offset) {
	...
	var shift = r_mapOverBrightBits.getAsInt() - re.overbrightBits;
...
function ShaderForShaderNum(shaderNum, lightmapNum, callback) {
	...
	if (shaderNum < 0 || shaderNum >= shaders.length) {
		error('ShaderForShaderNum: bad num ' + shaderNum);
	}
	var dsh = shaders[shaderNum];

	FindShaderByName(dsh.shaderName, lightmapNum, 0, callback);
}
...
function LoadSurfaces(surfaces, verts, meshVerts, callback) {
	...
	verts.forEach(function (vert) {
		...
		// Vertex colors are scaled to 0 - 1.
		vec4.scale(vert.color, 1 / 255.0);
	});
	...
	surfaces.forEach(function (surface, i) {
		...
		if (surface.surfaceType === BspSerializer.MST.PLANAR) {
			ParseFace(wsurface, surface);
		} else if (surface.surfaceType === BspSerializer.MST.TRIANGLE_SOUP) {
			ParseTriSurf(wsurface);
		} else if (surface.surfaceType === BspSerializer.MST.PATCH) {
			ParseMesh(wsurface, r_subdivisions.getAsInt());
		}
...
function LoadBrushModels(models) {
	...
	for (var i = 0; i < models.length; i++) {
		RegisterModel('*' + i, models[i], function () {});
	}
}
...
function LoadLightmaps(buffer) {
	...
	//
	CreateTexture('*lightmap', lightmaps, textureSize, textureSize);
}
...
function LoadLightGrid(lightGridData, lightGridOrigin, lightGridSize, lightGridInverseSize, lightGridBounds) {
	...
	// CM shouldn't care.
	// FIXME - Move to com-bsp?
	// FIXME2 - We should really update the logic in re-light
	// and scale the light data to 0 - 1 here.
...
function CreateWorldGeometry() {
	...
	var addFacesForNode = function (node) {
		for (var i = 0; i < node.numLeafSurfaces; i++) {
			...
			// Ignore nodraw surfaces.
			if (shader.surfaceFlags & QS.SURF.NODRAW) {
				continue;
			}

			// Add the surface to its batch.
			var shaderName = shader.name;
			if (shader.sort === ShaderCompiler.SS.PORTAL) {
				shaderName += portals++;
			}
