...
function BuildWorldGeometry() {
	...
	var addFacesForNode = function (node) {
		for (var i = 0; i < node.numLeafSurfaces; i++) {
			...
			// The surfaces with the sky shader are just to let us know
			// the shader, don't actuall render them.
			if (shader.sky) {
				continue;
			}

			// Add the surface to its batch.
			var geo = geometry[shader.name];

			if (!geo) {
				geo = geometry[shader.name] = new WorldGeometry();
				geo.shader = shader;
				geo.numVerts = 0;
				geo.numIndexes = 0;
				geo.faces = [];
			...
			geo.numVerts += face.vertCount;
			geo.numIndexes += face.meshVertCount;
			geo.faces.push(face);
	...
	// Lets create vbos for each geometry group.
