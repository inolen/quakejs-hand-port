...
function AddBrushModelSurfaces(refent) {
	...
	for (var i = 0; i < bmodel.numSurfaces; i++) {
		...
		AddWorldSurface(face/*, tr.currentEntity->needDlights*/, false);
	}
}
...
function RecursiveWorldNode(node, planeBits/*, dlightBits*/) {
	...
	for (var i = 0; i < node.numLeafSurfaces; i++) {
		...
		AddWorldSurface(face/*, dlightBits*/, true);
	}
}
 ...
 * AddWorldSurface
 */
function AddWorldSurface(face/*, dlightBits*/, useCompiled) {
	var check = useCompiled ? face.compiled : face;
