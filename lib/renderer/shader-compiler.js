...
var ShaderBuilder = require('renderer/shader-builder');
var ShaderParser = require('renderer/shader-parser');

var ShaderCompiler = {};

/**
 * Group stages that can be blended together in the fragment shader
 * together.
 */
ShaderCompiler.mergeStages = function (script) {
	var groups = [];
...
ShaderCompiler.buildProgram = function (script, stages) {
	...
	for (var i = 0; i < stages.length; i++) {
		ShaderCompiler.buildVertexPass(builder, script, stages[i], i);
	}
	...
	for (var i = 0; i < stages.length; i++) {
		ShaderCompiler.buildFragmentPass(builder, script, stages[i], i);
	}

	// Add dlights if enabled.
	ShaderCompiler.buildDlightPass(builder, script);
...
ShaderCompiler.buildDlightPass = function (builder, script) {
...
return {
	mergeStages:  ShaderCompiler.mergeStages,
	buildProgram: ShaderCompiler.buildProgram
};
