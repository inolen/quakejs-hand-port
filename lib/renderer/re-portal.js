...
function SurfIsOffscreen(drawSurf, clipDest) {
	...
	for (var i = 0; i < tess.xyz.elementCount; i++) {
		TransformModelToClip(tess.xyz[i], or.modelMatrix, parms.projectionMatrix, eye, clip);
	...
	var numTriangles = tess.index.elementCount / 3;

	for (var i = 0; i < tess.numIndexes; i += 3) {
		var normal = vec3.subtract(tess.xyz[tess.indexes[i]], parms.or.origin, [0, 0, 0]);

		var len = vec3.length(normal);  // TODO lose the sqrt
		if (len < shortest) {
			shortest = len;
		}

		if (vec3.dot(normal, tess.normal[tess.indexes[i]]) >= 0) {
			numTriangles--;
		}
	}
	if (!numTriangles) {
