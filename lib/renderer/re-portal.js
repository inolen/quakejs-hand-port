...
function RenderMirrorView(geo, entityNum) {
	...
	if (GeometryIsOffscreen(geo, entityNum)) {
		return false;
	}
	...
	var surface = new QS.Orientation();
	var camera = new QS.Orientation();
 ...
 */
function GeometryIsOffscreen(geo, entityNum) {
	...
	for (var i = 0; i < position.index; i += 3) {
		// Copy off the vert.
		pos[0] = position.array[i + 0];
		pos[1] = position.array[i + 1];
		pos[2] = position.array[i + 2];
	...
	for (var i = 0; i < index.index; i += 3) {
		...
		vec3.subtract(pos, or.origin, dir);

		var len = vec3.squaredLength(dir);
		if (len < shortest) {
			shortest = len;
		}

		n[0] = normal.array[offset + 0];
		n[1] = normal.array[offset + 1];
		n[2] = normal.array[offset + 2];
 ...
 * IsMirror
 */
function IsMirror(geo, entityNum) {
	...
	// origin will be the origin of the portal, oldOrigin will be
...
function GetPortalOrientations(viewParms, geo, entityNum, surface, camera) {
	...
	for (var i = 0; i < re.refdef.numRefEnts; i++) {
		...
		vec3.add(refent.origin, vec3.scale(surface.axis[0], -d, vec3.create()), surface.origin);

		// Now get the camera origin and orientation.
		vec3.set(refent.oldOrigin, camera.origin);
		QMath.AxisCopy(refent.axis, camera.axis);
		vec3.negate(camera.axis[0]);
		vec3.negate(camera.axis[1]);
