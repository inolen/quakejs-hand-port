...
function FindTextureByName(name, clamp, callback) {
	...
	sys.ReadFile(name, 'binary', function (err, data) {
		if (err) {
			...
			return callback(re.defaultTexture);
		}
...
function RegisterSkin(filename, callback) {
	...
	if (filename.indexOf('.skin') === -1) {
		...
		FindShaderByName(filename, SF.LIGHTMAP_GRID | SF.POSITION_LERP, function (err, shader) {
	...
	sys.ReadFile(filename, 'utf8', function (err, data) {
		...
		async.series(steps, function (err, cb) {
			if (callback) {
				callback(err, hSkin);
			}
		});
