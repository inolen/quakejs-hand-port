 ...
 * FindTextureByName
 */
function FindTextureByName(name, clamp) {
...
function RegisterSkin(filename, callback) {
	...
	if (filename.indexOf('.skin') === -1) {
		var surface = new SkinSurface();
		surface.shader = FindShaderByName(filename, LIGHTMAP.LIGHTGRID);
	...
	sys.ReadFile(filename, 'utf8', function (err, data) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			surface.shader = FindShaderByName(shaderName, LIGHTMAP.LIGHTGRID);
			skin.surfaces.push(surface);
