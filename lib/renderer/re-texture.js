...
function FindTextureByName(name, clamp, callback) {
	...
	texture.name = name;
	texture.texnum = re.defaultTexture.texnum;
	texture.promise = deferred.promise();
	texture.promise.then(callback);
...
function CreateTexture(name, buffer, width, height, clamp) {
	// Not actually used, just used to help support subsequent
	// calls to FindTextureByName.
	var deferred = $.Deferred();

	var texture = re.textures[name] = new Texture();
	texture.name = name;
	texture.texnum = BuildTexture(buffer, width, height, clamp);
	texture.promise = deferred.promise();

	// Set to a resolved state even thought we don't
	// currently have anything waiting on it.
	deferred.resolve(texture);
 ...
 * RegisterSkin
 */
function RegisterSkin(filename, callback) {
	if (!filename) {
	...
	var hSkin = re.skins.length;

	var skin = re.skins[hSkin] = new Skin();
	skin.name = filename;
	skin.promise = deferred.promise();
	skin.promise.then(callback);
