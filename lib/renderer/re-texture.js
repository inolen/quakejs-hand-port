...
function BuildDefaultTexture() {
	...
	re.defaultTexture = CreateTexture('*default', data, 64, 64);
}
...
function BuildWhiteTexture() {
	re.whiteTexture = CreateTexture('*white', new Uint8Array([255,255,255,255]), 1, 1);
}

/**
 * FindTextureByName
 *
 * Always returns a valid texture (possibly the default).
 */
...
function FindTextureByName(name, clamp, callback) {
	...
	if (re.textures[name]) {
		return callback(re.textures[name]);
	}

	// Append to the pending callback stack if it exists.
	if (textureCallbacks[name]) {
		textureCallbacks[name].push(callback);
		return;
	}

	// Initialize the callback stack.
	textureCallbacks[name] = [callback];
	...
	sys.ReadFile(name, 'binary', function (err, data) {
		...
		if (err) {
			...
			texture = re.defaultTexture;
		} else {
			texture = new Texture();
			texture.name = name;
			texture.texnum = BuildTexture(data, null, null, clamp);
			re.textures[name] = texture;
		}

		// Execute pending callbacks.
		var callbacks = textureCallbacks[name];
		for (var i = 0; i < callbacks.length; i++) {
			callbacks[i](texture);
		}

		// Clear the callback stack.
		delete textureCallbacks[name];
	}, 'renderer');
}
...
function CreateTexture(name, buffer, width, height, clamp) {
	...
	if (!(texture = re.textures[name])) {
		texture = re.textures[name] = new Texture();
	}
...
function RegisterSkin(name, callback) {
	...
	FindSkinByName(name, function (skin) {
		var hSkin = re.skins.indexOf(skin);
		callback(hSkin);
	});
}
...
function FindSkinByName(name, callback) {
	// Strip extension.
	name = name.replace(/\.[^\/.]+$/, '');

	// See if the skin is already loaded.
	for (var hSkin = 1; hSkin < re.skins.length; hSkin++) {
		var skin = re.skins[hSkin];

		if (skin.name == name) {
			return callback(hSkin);
		}
	}

	// Append to the pending callback stack if it exists.
	if (skinCallbacks[name]) {
		skinCallbacks[name].push(callback);
		return;
	}

	// Initialize the callback stack.
	skinCallbacks[name] = [callback];
	...
	sys.ReadFile(filename, 'utf8', function (err, data) {
		var complete = function (skin) {
			// Execute pending callbacks.
			var callbacks = skinCallbacks[name];
			for (var i = 0; i < callbacks.length; i++) {
				callbacks[i](skin);
			}

			// Clear the callback stack.
			delete skinCallbacks[name];
		};

		if (err) {
			log('Failed to load skin', name);
			return complete(re.skins[0]);
		} else {
			var skin = new Skin();
			skin.name = name;
