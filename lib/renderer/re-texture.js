...
function FindTextureByName(name, clamp, callback) {
	if (name.indexOf('bulletExplosion') !== -1) {
		debugger;
	}
	...
	sys.ReadFile(name, 'binary', function (err, data) {
		if (err) {
			return callback(err, re.defaultTexture);
		}
...
function RegisterSkin(filename, callback) {
	...
	if (filename.indexOf('.skin') === -1) {
		...
		FindShaderByName(filename, LIGHTMAP.LIGHTGRID, function (err, shader) {
	...
	sys.ReadFile(filename, 'utf8', function (err, data) {
		...
		lines.forEach(function (line) {
			...
			steps.push(function (cb) {
				FindShaderByName(shaderName, LIGHTMAP.LIGHTGRID, function (err, shader) {
					surface.shader = shader;
					cb(null, shader);
				});
			});
