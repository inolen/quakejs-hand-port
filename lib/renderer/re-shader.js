...
function RegisterShader(shaderName, callback) {
	...
	re.shaders.load(shaderName, 0, callback);
}
...
function FindShaderByName(shaderName, flags, callback) {
	re.shaders.findByName(shaderName, flags, callback);
}
 ...
 */
function LoadShader(shaderName, flags, callback) {
...
function BuildProgramFromScript(script) {
	...
	if (dlights) {
		...
		var dlightFunc = 'vec4 dlightPass() {\n' +
						 ...
						 '		vec3 dir = vPosition.xyz - dlightPositions[i].xyz;\n' +
						 '		float distance = 0.0;\n' +
