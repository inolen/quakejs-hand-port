...
function InitDefaultShaders() {
	...
	var shader = re.debugShader = new Shader();
...
function BuildNoLightmapProgram() {
	...
	var fs = '#ifdef GL_ES\n' +
			...
			'void main(void) {\n' +
				'vec4 diffuseColor = texture2D(texSampler0, vTexCoord);\n' +
...
function FindShaderByName(shaderName, lightmapIndex) {
	...
	if (re.shaderBodies[shaderName]) {
		...
		var script = ParseShaderScript(shaderText, lightmapIndex);

		shader = re.shaders[hShader] = CreateShaderFromScript(script);
	}
...
function CreateDefaultShader(shaderName, lightmapIndex, callback) {
	var script = new ShaderScript();

	script.name = shaderName;
	script.sort = SS.OPAQUE;

	var stage1 = new ShaderScriptStage();
	stage1.maps[0] = shaderName;
	script.stages.push(stage1);

	// Assign a vertex / fragment shader program.
	if (lightmapIndex < 0) {
		stage1.rgbGen = 'vertex';
	} else {
		var stage2 = new ShaderScriptStage();
		stage2.maps[0] = '*lightmap';
		stage2.isLightmap = true;
		stage2.blendSrc = 'GL_DST_COLOR';
		stage2.blendDest = 'GL_ZERO';
		script.stages.push(stage2);
	}
...
function CreateShaderFromScript(script) {
	...
	shader.surfaceFlags = TranslateSurfaceParmsToSurfaceFlags(script.surfaceParms);
	shader.contentFlags = TranslateSurfaceParmsToContentFlags(script.surfaceParms);
	...
	shader.portalRange = script.portalRange;
	...
	for (var i = 0; i < script.stages.length; i++) {
		...
		stage.isLightmap = scriptStage.isLightmap;
		stage.hasBlendFunc = scriptStage.hasBlendFunc;
		stage.blendSrc = TranslateBlend(scriptStage.blendSrc);
		stage.blendDest = TranslateBlend(scriptStage.blendDest);
		stage.depthFunc = TranslateDepthFunc(scriptStage.depthFunc);
		stage.depthWrite = scriptStage.depthWrite;

		shader.stages.push(stage);
	}

	var firstStage = shader.stages[0];

	if (firstStage && firstStage.hasBlendFunc) {
		shader.hasBlendFunc = true;
		shader.blendSrc = firstStage.blendSrc;
		shader.blendDest = firstStage.blendDest;
	}

	if (firstStage && firstStage.depthWrite) {
		shader.depthWrite = true;
		shader.depthFunc = firstStage.depthFunc;
	}

	shader.program = BuildProgramFromScript(script);
...
function TranslateSurfaceParmsToSurfaceFlags(surfaceParms) {
	var surfaceFlags = 0;
...
function TranslateSurfaceParmsToContentFlags(surfaceParms) {
	var contentFlags = 0;
...
function BuildProgramFromScript(script) {
	...
	// Skies don't have vertex colors.
	if (!script.sky) {
	...
	for (var i = 0; i < script.stages.length; i++) {
		...
		BuildVertexPass(builder, script, stage);
	}
	...
	for (var i = 0; i < script.stages.length; i++) {
		var stage = script.stages[i];
		BuildFragmentPass(builder, script, stage);
	}

	builder.addFragmentLines(['gl_FragColor = fragColor;']);

	// Compile it.
	return CompileShaderProgram(builder.getVertexSource(), builder.getFragmentSource());
}

/**
 * BuildVertexPass
 */
function BuildVertexPass(builder, script, stage) {
	...
	switch (stage.alphaGen) {
		case 'portal':
			builder.addVarying('vPortalScale', 'float');
			builder.addVertexLines([ 'vPortalScale = length(position - viewPosition) / ' + script.portalRange.toFixed(1) + ';' ]);
			break;
		case 'lightingspecular':
			builder.addAttrib('lightCoord', 'vec2');
			builder.addVarying('vLightCoord', 'vec2');
			builder.addVertexLines([ 'vLightCoord = lightCoord;' ]);
			break;
		default:
			break;
	}
 ...
 * BuildFragmentPass
 */
function BuildFragmentPass(builder, script, stage) {
	...
	switch (stage.alphaGen) {
		...
		case 'entity':
			...
			break;
		case 'wave':
			passFunc += builder.createWaveform('alpha', stage.alphaWaveform, false);
			break;
		case 'portal':
			passFunc += '	float alpha = vPortalScale;\n';
			break;
		case 'lightingspecular':
			// For now this is VERY special cased. May not work well with all instances of lightingSpecular
			builder.addUniform('lightmap', 'sampler2D');
			passFunc += [
	...
	if (stage.stageId === 0) {
		builder.addFragmentLines([
			'fragColor = ' + passName + '(' + samplerVar + ', ' + texCoordVar + ');'
		]);
	} else {
		builder.addFragmentLines([
 ...
 * ShaderBuilder
 *
 * Compile shader scripts into GLSL program source.
 */
 ...
 * ParseShaderScript
 */
function ParseShaderScript(shaderText, lightmapIndex) {
	...
	while (!tokens.EOF()) {
		...
		switch (token) {
			case '{': {
				var stage = ParseShaderScriptStage(script, tokens);
			...
			// TODO Merge all surfaceparms into script.surfaceFlagsand script.contentFlags.
			case 'surfaceparm':
...
/**
 * ParseShaderScriptStage
 */
function ParseShaderScriptStage(script, tokens) {
	var scriptStage = new ShaderScriptStage();
