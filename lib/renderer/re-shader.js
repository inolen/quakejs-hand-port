...
function CreateShaderFromScript(script, callback) {
	...
	for (var i = 0; i < script.stages.length; i++) {
		...
		var vs = GenerateVertexShader(script, scriptStage);
		var fs = GenerateFragmentShader(script, scriptStage);
 ...
 */
function GenerateVertexShader(script, stage) {
	...
	switch (stage.alphaGen) {
		case 'portal':
			...
			builder.addLines(['vPortalScale = length(position - viewPosition) / ' + script.portalRange.toFixed(1) + ';']);
			break;
 ...
 */
function GenerateFragmentShader(script, stage) {
	var builder = new ShaderBuilder();
	...
	while (!tokens.EOF()) {
		...
		switch (token.toLowerCase()) {
			...
			case 'map':
				var map = tokens.next();
				if (!map) {
					com.Error(ERR.DROP, 'WARNING: missing parameter for \'map\' keyword in script \'' + script.name + '\'');
				}
				if (map === '$whiteimage') {
					map = '*white';
			...
			case 'blendfunc':
				scriptStage.blendSrc = tokens.next();
				...
				switch (scriptStage.blendSrc) {
					...
					default:
						scriptStage.blendDest = tokens.next();
						break;
				}
				break;
