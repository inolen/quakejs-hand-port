...
function RegisterShader(shaderName, callback) {
	...
	FindShaderByName(shaderName, LIGHTMAP.NONE, function (err, shader) {
...
function FindShaderByName(shaderName, lightmapIndex, callback) {
	...
	// Called once the shader has finished loading.
	var finish = function (err, shader) {
		re.shaders.push(shader);
	...
	if (re.scriptBodies[shaderName]) {
		...
		shader = CreateShaderFromScript(script, finish);
	}
	// There is no shader for this name, let's create a default.
	else {
		shader = CreateShaderFromTexture(shaderName, lightmapIndex, finish);
	}
}
 ...
 */
function CreateShaderFromTexture(textureName, lightmapIndex, callback) {
	var script = new Script();
	...
	if (lightmapIndex === LIGHTMAP.VERTEX) {
		stage.rgbGen = 'vertex';
	} else if (lightmapIndex === LIGHTMAP.LIGHTGRID) {
	...
	CreateShaderFromScript(script, callback);
}
...
function BuildProgramFromScript(script) {
	...
	// Add special properties based on cgen / agen states.
	var vColor = false;
	...
	if (vDiffuseColor) {
		...
		builder.addVertexLines([
			'vDiffuseScale = max(0.0, dot(normal, lightDir));'
		]);
	}
