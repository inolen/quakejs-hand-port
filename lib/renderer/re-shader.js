...
function InitDefaultShaders() {
	...
	stage.texture = FindTextureByName('*default');
	shader.stages.push(stage);
	SortShader(shader);
}
...
function BuildDebugProgram() {
	...
	var fs = '#ifdef GL_ES\n' +
			 ...
			 '#endif\n' +
			 'void main(void) {\n' +
			     'float w = 1.0 - (gl_FragCoord.z / gl_FragCoord.w) / 1024.0;\n' +
 ...
 * FindShaderByName
 */
function FindShaderByName(shaderName, lightmapIndex, callback) {
	if (!shaderName) {
		// TODO Fix FindShaderByName in re-model and make this not optional.
		callback && callback(re.shaders[0]);
		return re.shaders[0];
	}
	...
	if (re.shaderBodies[shaderName]) {
		console.log('TRANSLATING', shaderName, hShader);
		var shaderText = re.shaderBodies[shaderName];
		var q3shader = ParseShader(shaderText, lightmapIndex);

		shader = re.shaders[hShader] = TranslateShader(q3shader, function () {
			deferred.resolve(shader);
		});
	}
	// There is no shader for this name, let's create a default.
	else {
		shader = re.shaders[hShader] = CreateDefaultShader(shaderName, lightmapIndex, function () {
			deferred.resolve(shader);
		});
	}

	shader.promise = deferred.promise();
	shader.promise.then(callback);
...
function RegisterShader(shaderName, callback) {
	...
	var shader = FindShaderByName(shaderName, LIGHTMAP.TWOD, function (shader) {
		return callback(re.shaders.indexOf(shader));
	});

	return re.shaders.indexOf(shader);
}
...
function CreateDefaultShader(shaderName, lightmapIndex, callback) {
	var texture = FindTextureByName(shaderName, false, function (texture) {
		callback();
	});
...
function TranslateShader(q3shader, callback) {
	...
	for (var i = 0; i < q3shader.stages.length; i++) {
		totalTextures += q3shader.stages[i].maps.length;
	}
	...
	for (var i = 0; i < q3shader.stages.length; i++) {
		...
		for (var j = 0; j < q3stage.maps.length; j++) {
			stage.textures[j] = FindTextureByName(q3stage.maps[j], q3stage.clamp, function (texture) {
