...
function RegisterShader(shaderName, callback) {
	...
	FindShaderByName(shaderName, 0, function (shader) {
...
function FindShaderByName(shaderName, flags, callback) {
	if (!shaderName) {
		return callback(re.defaultShader);
	}

	shaderName = shaderName.replace(/\.[^\.]+$/, '');

	// Try to find the shader in our cache.
	...
	if (shaderCallbacks[shaderName]) {
		shaderCallbacks[shaderName].push(callback);
		return;
	}

	// Initialize the callback stack.
	shaderCallbacks[shaderName] = [callback];
	...
	// Conver the script into a shader.
	CreateShaderFromScript(script, function (err, shader) {
		if (err) {
		...
		var callbacks = shaderCallbacks[shaderName];
		for (var i = 0; i < callbacks.length; i++) {
			callbacks[i](shader);
		}

		// Clear the callback stack.
		delete shaderCallbacks[shaderName];
	});
}
...
function CreateShaderFromScript(script, callback) {
	// Error out on zero-stage shaders (e.g. caulk surfaces
	// that make it into the bsp somehow).
	if (!script.stages.length) {
		return callback(new Error('Shader script has no stages'));
	}
