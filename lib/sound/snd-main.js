...
function StartBackgroundTrack(hSound, loop) {
	...
	var sound = GetSoundByHandle(hSound);
	// Early out if there is no sound for this handle, or it failed to load.
	if (!sound || (sound.promise.state() !== 'pending' && !sound.buffer)) {
		return;  // fail silently
	}

	// If the sound is still loading, set this call to re-execute once it's loaded.
	if (sound.promise.state() === 'pending') {
		sound.promise.then(function () {
			StartBackgroundTrack(hSound, loop);
		});
		return;
	}
