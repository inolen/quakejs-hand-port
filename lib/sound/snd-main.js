...
function RegisterSound(name, callback) {
	...
	FindSoundByName(name, function (sound) {
...
function FindSoundByName(name, callback) {
	...
	if (soundCallbacks[name]) {
		soundCallbacks[name].push(callback);
		return;
	}

	// Initialize the callback stack.
	soundCallbacks[name] = [callback];

	sys.ReadFile(name + '.ogg', 'binary', function (err, data) {
		var complete = function (sound) {
			// Execute pending callbacks.
			var callbacks = soundCallbacks[name];
			for (var i = 0; i < callbacks.length; i++) {
				callbacks[i](sound);
			}

			// Clear the callback stack.
			delete soundCallbacks[name];
		};

		if (err) {
			log('Failed to load sound \'' + name + '\'');
			complete(snd.sounds[0]);
		} else {
