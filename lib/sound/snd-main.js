...
//			log('removing sound', i, source.hSound);
		...
		// stop looping sound if "loopAddedThisFrame" wasn't set
		if (source.looping && !source.loopAddedThisFrame) {
//			log('kill looping sound', source.hSound);
			source.bufsrc.noteOff(0);
		}
...
function RegisterSound(name, callback) {
	...
	FindSoundByName(name, function (err, sound) {
		var hSound = snd.sounds.indexOf(sound);
		callback(err, hSound);
	});
}
 ...
 * FindSoundByName
 */
function FindSoundByName(name, callback) {
	// Search the currently loaded sounds.
...
function StartSound(origin, entityNum, hSound, looping) {
	...
	if (looping) {
		for (var i = 0; i < snd.sources.length; i++) {
			if (snd.sources[i].hSound === hSound) {
//				log('return currently looping sound');
 ...
 * AddLoopingSound
 */
function AddLoopingSound(entityNum, origin, velocity, hSound) {
	// S_AL_AddLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx)
	// (cent.currentState.number, cent.lerpOrigin, QMath.vec3origin, itemInfo.sounds.firing)
	// function StartSound(origin, entityNum, hSound, looping) {
	
	var source = StartSound(undefined, entityNum, hSound, /* looping = */ true);
	
	source.loopAddedThisFrame = true;
	source.looping = true;
	
//	log('AddLoopingSound', source.hSound);
}
