...
function S_Init () {
	...
	snd_volume_sfx.gain.value = 0.6; // lowering sound effects in the mix
}
...
//		snd_ctx.listener.setPosition(0, 0, 0); // listener is at origin by default
	...
	// Read from memo if sound beffer data already loaded before
	if (snd_memo_sfx.hasOwnProperty(sfx)) {
		console.log('SOUND MEMO READ');
	...
	// Else, read the sound buffer from file
		...
		sys.ReadFile('sound/' + sfx + '.wav', 'binary', function (err, data) {
			...
			snd_memo_sfx[sfx] = snd_ctx.createBuffer(data, false);
