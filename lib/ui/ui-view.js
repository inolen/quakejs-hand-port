...
function GetViewConstructor(name) {
	switch (name) {
		...
		case 'hudnew':       return HudViewNewModel;
...
function RegisterView(name) {
	...
	if (ctor === HudViewNewModel) {
		view = uil.views[name] = new HudViewNewModel();
		$viewportUI.append(hudViewNewTemplate);
		ko.applyBindings(view, document.getElementById('hud-new'));
	} else {
...
function ScaleElements() {
	...
	for (var name in views) {
		...
		if (!(views[name] instanceof HudViewNewModel)) {
