...
function HudModel() {
	...
	self.redFlagIcon = ko.observable(0);
	self.blueFlagIcon = ko.observable(0);
	...
	self.powerups = ko.observableArray([]);
	...
	self.setPowerup = function (i, icon) {
		if (!self.powerups()[i]) {
			self.powerups.valueWillMutate();
			self.powerups()[i] = new HudPowerup(icon, 0);
			self.powerups.valueHasMutated();
		} else {
			self.powerups()[i].icon(icon);
		}
	};
