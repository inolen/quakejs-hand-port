	...
	var SettingsMenu = Backbone.View.extend({
		...
		initialize: function (opts) {
			...
			for (var i = 0; i < this.modelCvars.length; i++) {
				...
				this.model[cvar] = exp.COM_GetCvarVal(cvar);
			}
			...
			for (var key in this.modelKeys) {
				...
				var keys = exp.CL_GetKeyNamesForCmd(key);
		...
		saveConfig: function () {
			...
			for (var key in this.modelKeys) {
				...
				var mval = this.modelKeys[key];
				if (mval !== '') {
					exp.CL_Bind(this.model[mval], key);
				}
			}

			exp.COM_SaveConfig(function (err) {
				if (err) {
