var SettingsMenu = UIView.extend({
	template: _.template('{{ include ../templates/settings-menu.tpl }}'),
	model: {},
	cvars: {
	...
	events: {
		'click #current-game': 'openIngameMenu',
	},
	initialize: function () {
		var name, evt;
	...
	formChanged: function (ev) {
		...
		var settingValue = ev.target.getAttribute('data-value');

		this.model[settingName] = settingValue;

		// Update the cvar or bind.
		if (this.cvars[settingName]) {
			var cvarName = this.cvars[settingName];
