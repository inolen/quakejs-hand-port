...
var HudView = UIView.extend({
	...
	initialize: function () {
		...
		this.model.on('change:info1', this.updateScores, this);
		this.model.on('change:info2', this.updateScores, this);
	...
	setInfo1: function (info) {
		this.model.set('info1', info);
	},
	setInfo2: function (info) {
		this.model.set('info2', info);
	},
	addEvent: function (ev) {
		...
		if (events.length >= MAX_HUD_EVENTS) {
			events.shift();
		}
	...
	updateAmmo: function (model, i, ammo) {
		this.els.ammo[i].innerHTML = ammo > 0 ? ammo : '&nbsp;';
	},
	...
	renderView: function () {
		...
		_.extend(data, { colorize: QS.colorize });
