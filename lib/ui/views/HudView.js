var HudView = UIView.extend({
	...
	setCrosshairName: function (name, alpha) {
		if (this.model.crosshairName !== name) {
			this.model.crosshairName = name;
			this.crosshairNameEl.text(name).trigger('recenter');  // force recentering
		}

		if (this.model.alpha !== alpha) {
			this.model.alpha = alpha;
			this.crosshairNameEl.css('opacity', alpha);
		}
	},
	...
	setAmmo: function (ammo) {
		...
		if (render) {
			...
			for (i = 0; i < this.model.weapons.length; i++) {
	...
	setKills: function (kills) {
		if (this.model.kills !== kills) {
			this.model.kills = kills;
			this.killsEl.text(kills);
		}
	},
	setTeam: function (team) {
		if (this.model.team !== team) {
			this.model.team = team;
			
			switch (this.model.team) {
			case 1:
				this.teamEl.text('RED');
				break;
			case 2:
				this.teamEl.text('BLUE');
				break;
			case 3:
				this.teamEl.text('SPECTATOR');
				break;
			default:
				this.teamEl.text('FREE');
				break;
			}
		}
	},
	renderView: function () {
		...
		this.crosshairNameEl = this.$el.find('.crosshair-name');
		...
		this.killsEl = this.$el.find('.kills');
		this.teamEl = this.$el.find('.team');
