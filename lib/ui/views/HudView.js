...
var HudModel = Backbone.Model.extend({
	...
	setAmmo: function (i, ammo) {
		...
		if (currentAmmo[i] !== ammo) {
			currentAmmo[i] = ammo;
			this.trigger('change:ammo', this, currentAmmo);
		}
	},
...
var HudView = UIView.extend({
	...
	initialize: function () {
		...
		this.model.on('change:kills', this.updateKills, this);
		this.model.on('change:team', this.updateTeam, this);
	...
	updateAmmo: function (model, ammo) {
		for (var i = 0; i < this.els.ammo.length; i++) {
			var $ammo = $(this.els.ammo.get(i));
			var index = $ammo.data('index');
			$ammo.text(index);
		}
	},
	...
	updateKills: function (model, kills) {
		this.els.kills.text(kills);
	},
	updateTeam: function (model, team) {
		var name = 'FREE';
	...
	renderView: function () {
		...
		this.els.ammo = this.$el.find('.ammo');
		...
		this.els.kills = this.$el.find('.kills');
		this.els.team = this.$el.find('.team');
