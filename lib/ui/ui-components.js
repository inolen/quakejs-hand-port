...
TextInput.prototype.onFocus = function (ev) {
	this._tempValue = this._originalValue = this.val();
};
...
TextInput.prototype.onKeyPress = function (ev) {
	...
	// Update element text.
	this.el.innerHTML = this._tempValue;
};
...
RadioInput.prototype.enabled = function (enabled) {
	if (arguments.length) {
		...
		this.el.setAttribute('data-enabled', enabled ? 'true' : 'false');

		// Trigger changed event.
		log('radio input changed!');
		var ev = document.createEvent('Event');
		ev.initEvent('change', true, false);
		this.el.dispatchEvent(ev);
	} else {
		return this.el.getAttribute('data-enabled') === 'true';
	}
};
