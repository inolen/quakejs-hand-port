...
function Init() {
	...
	//
	var context = exp.SYS_GetUIContext();
	$viewportUI = $(context);
	ScaleUI();
...
function PushMenu(name) {
	if (!uil.activeMenus.length) {
		...
		exp.CL_CaptureInput(KeyPressEvent, MouseMoveEvent);
	}
...
function RegisterView(name) {
	var exports = {
		SYS_ReadFile:         exp.SYS_ReadFile,
		
		COM_GetCvarVal:       exp.COM_GetCvarVal,
		
		CL_Bind:              exp.CL_CmdBind,
		CL_UnbindAll:         exp.CL_CmdUnbindAll,
		CL_GetKeyNamesForCmd: exp.CL_GetKeyNamesForCmd,
		CL_CaptureInput:      exp.CL_CaptureInput,
		CL_Disconnect:        exp.CL_Disconnect,
