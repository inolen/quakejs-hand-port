...
function RegisterDOMHooks() {
	...
	// Any clear event that's bubbled up should cause a focus clear.
	$viewportUI.on('click', function (ev) {
		ev.stopPropagation();

		FocusElement(ev.relatedTarget);
	});
}
...
function KeyPressEvent(keyName) {
	// Simulate click events for mouse keys.
	if (uil.hoverEls && keyName.indexOf('mouse') === 0) {
		var offset = $ptr.offset();
		var bottomMost = uil.hoverEls[0];
		var button = parseInt(keyName.substr(5), 10);

		// FIXME - Should happen on mouseup.
		bottomMost.dispatchEvent(new MouseEvent('click', {
