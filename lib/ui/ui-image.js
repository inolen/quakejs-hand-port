...
function RegisterImage(name, callback) {
	FindImageByName(name, function (img) {
		var hImage = uil.images.indexOf(img);
		callback(hImage);
	});
}
...
function FindImageByName(name, callback) {
	...
	for (var hImage = 1; hImage < uil.images.length; hImage++) {
		var img = uil.images[hImage];

		if (img.name === name) {
			// Append to the pending callback stack if it exists.
			if (imageCallbacks[name]) {
				imageCallbacks[name].push(callback);
				return;
			}
	...
	imageCallbacks[name] = [callback];

	// Start loading the image.
	sys.ReadFile(name, 'binary', function (err, data) {
		if (err) {
			log('Failed to load image \'' + name + '\'');
			return callback(uil.images[0]);
		}

		img.data = 'data:image/png;base64,' + QShared.atob64(new Uint8Array(data));

		// Execute pending callbacks.
		var callbacks = imageCallbacks[name];
		for (var i = 0; i < callbacks.length; i++) {
			callbacks[i](img);
		}

		// Clear the callback stack.
		delete imageCallbacks[name];
	});
}