 ...
 */
function LoadImagesInElement($el) {
	// First, replace any inline image paths with image handles.
	$el.find('[data-image]').each(function () {
		var $img = $(this),
			path = $img.data('image'),
			hImage = RegisterImage(path);

		$img.attr('data-himage', hImage);
	});

	// Next, process all image handle attributes.
	$el.find('[data-himage]').each(function () {
		var $img = $(this),
			hImage = $img.data('himage'),
		...
		if (!img.data) {
			...
			$img.css({
				...
				'background-image': 'url(\'' + FindImageByName('*default').data + '\')'
			});

			// Once the image has loaded, swap it out.
			img.load(function () {
				$img.css({
					'background-size': 'cover',
		...
		// It's possible the image is already loaded.
		else {
			$img.css({
				'background-size': 'cover',
				'background-image': 'url(\'' + img.data + '\')'
