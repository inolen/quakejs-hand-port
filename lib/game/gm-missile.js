...
function MissileImpact(ent, trace ) {	
	...
	if (other.takeDamage) {
		...
		if (ent.damage) {
			...
			// if (LogAccuracyHit(other, &g_entities[ent.ownerNum])) {
			...
			// }
			// G_Damage(other, ent, &g_entities[ent.ownerNum], velocity,
	...
	if (other.takeDamage && other.client) {
		AddEvent(ent, EV.MISSILE_HIT, DirToByte(trace.plane.normal));
	...
	if (ent.splashDamage) {
		if (RadiusDamage(trace.endPos, ent.parent, ent.splashDamage, ent.splashRadius, other, ent.splashMethodOfDeath)) {
			if (!hitClient) {
				//level.entities[ent.ownerNum].client.accuracy_hits++;
			}
		}
	}
