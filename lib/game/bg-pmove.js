...
function SetWaterLevel() {
	...
	var contents = pm.pointContents(point, ps.clientNum);

	if (contents & MASK.WATER) {
		var sample2 = ps.viewheight - MINS_Z;
		...
		contents = pm.pointContents(point, ps.clientNum);
...
function CheckWaterJump() {
	...
	var flatforward = [pml.forward[0], pml.forward[1], 0];
	vec3.normalize(flatforward);

	var spot = vec3.add(vec3.scale(flatforward, 30, [0, 0, 0]), ps.origin);
	spot[2] += 4;
...
function NoclipMove() {
	...
	// Move.
	vec3.add(ps.origin, vec3.scale(ps.velocity, pml.frameTime, [0, 0, 0]));
}
