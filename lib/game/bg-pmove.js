...
function SetWaterLevel() {
	...
	pm.waterlevel = GetWaterLevel(ps.origin, ps.viewheight, ps.arenaNum, ps.clientNum, pm.pointContents);

	var point = vec3.createFrom(ps.origin[0], ps.origin[1], ps.origin[2] + MINS_Z + 1);
	var contents = pm.pointContents(point, ps.arenaNum, -1);
...
function CheckDuck() {
	...
	if (pm.cmd.upmove < 0) {
		...
		if (ps.pm_flags & PMF.DUCKED) {
			// Try to stand up.
			pm.maxs[2] = 32;
			var trace = pm.trace(ps.origin, ps.origin, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
			if (!trace.allSolid) {
				ps.pm_flags &= ~PMF.DUCKED;
			}
		}
...
function GroundTrace() {
	...
	var trace = pm.trace(ps.origin, point, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
...
function CheckWaterJump() {
	...
	var contents = pm.pointContents(spot, ps.arenaNum, ps.clientNum);
	if (!(contents & CONTENTS.SOLID)) {
		return false;
	}

	spot[2] += 16;
	contents = pm.pointContents(spot, ps.arenaNum, ps.clientNum);
	...
	for (var i = -1; i <= 1; i++) {
		for (var j = -1; j <= 1; j++) {
			for (var k = -1; k <= 1; k++) {
				...
				point[1] += j;
				point[2] += k;
				tr = pm.trace(point, point, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
...
function SlideMove(gravity) {
	...
	for (var bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		...
		var trace = pm.trace(ps.origin, end, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
...
function StepSlideMove(gravity) {
	...
	var trace = pm.trace(start_o, down, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
	...
	trace = pm.trace(start_o, up, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
	if (trace.allSolid) {
	...
	down[2] -= stepSize;
	trace = pm.trace(ps.origin, down, pm.mins, pm.maxs, ps.arenaNum, ps.clientNum, pm.tracemask);
	if (!trace.allSolid) {
		vec3.set(trace.endPos, ps.origin);
	}
	if (trace.fraction < 1.0) {
