...
function ClientCommand(clientNum, cmd) {
	...
	// 	Cmd_FollowCycle_f (ent, -1);
		...
		ClientCmdCreateTeam(ent, args[0]);
	}
	else if (name === 'team') {
...
function Say(ent, target, mode, text) {
	...
	if (g_gametype.at(level.arena.arenaNum).get() < GT.TEAM && mode === SAY.TEAM) {
		mode = SAY.ALL;
	}
	...
	switch (mode) {
		...
		// 	break;
		// case SAY.TELL:
		// 	if (target && g_gametype.at(level.arena.arenaNum).get() >= GT.TEAM &&
 ...
 * ClientCmdCreateTeam
 */
function ClientCmdCreateTeam(ent, teamName) {
	var team = level.arena.teams[ent.client.ps.clientNum];
	team.name = teamName;

	log('Setting teamname for', ent.client.ps.clientNum, 'to', teamName);

	ent.client.sess.team = -1;
	ent.client.sess.raTeam = ent.client.ps.clientNum;

	SetTeam(ent.client, 'f', true);
}
...
function ClientCmdTeam(ent, teamName) {
	if (teamName === undefined) {
		var oldTeam = ent.client.sess.team;
		switch (oldTeam) {
			case TEAM.BLUE:
	...
	if (g_gametype.at(level.arena.arenaNum).get() === GT.TOURNAMENT && ent.client.sess.team === TEAM.FREE) {
