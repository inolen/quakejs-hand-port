...
function Say(ent, target, mode, text) {
	...
	if (g_gametype.getAsInt() < GT.TEAM && mode === SAY.TEAM) {
		mode = SAY.ALL;
	}
	...
	switch (mode) {
		...
		// 	break;
		// case SAY.TELL:
		// 	if (target && g_gametype.getAsInt() >= GT.TEAM &&
...
function SendScoreboardMessage(ent) {
	...
	for (var i = 0; i < arena.numConnectedClients; i++) {
		...
		val.scores.push({
			...
			dead: client.pers.teamState.state === TEAM_STATE.ELIMINATED
		});
	}
...
function ClientCmdTeam(ent, teamName) {
	...
	if (g_gametype.getAsInt() === GT.TOURNAMENT && ent.client.sess.sessionTeam === TEAM.FREE) {
