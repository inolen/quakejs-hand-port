...
function MoverPush(pusher, move, amove) {
	...
	var entityNums = sv.FindEntitiesInBox(totalMins, totalMaxs, pusher.s.arenaNum);
...
function TestEntityPosition(ent) {
	...
	var tr;
	if (ent.client) {
		tr = sv.Trace(ent.client.ps.origin, ent.client.ps.origin, ent.r.mins, ent.r.maxs, ent.s.arenaNum, ent.s.number, mask);
	} else {
		tr = sv.Trace(ent.s.pos.trBase, ent.s.pos.trBase, ent.r.mins, ent.r.maxs, ent.s.arenaNum, ent.s.number, mask);
	}
