	...
	if (g_gametype() === GT.TOURNAMENT) {
		...
		if (level.warmupTime < 0) {
			if (level.numPlayingClients === 2) {
				...
				if (g_warmup() > 1) {
					level.warmupTime = level.time + (g_warmup() - 1) * 1000;
				} else {
					level.warmupTime = 0;
				}
		...
		if (g_gametype() > GT.TEAM) {
			counts[TEAM.BLUE] = TeamCount(null, TEAM.BLUE);
			counts[TEAM.RED] = TeamCount(null, TEAM.RED);
...
function RestartMatch() {
	...
	// Start the warmup.
	// NOTE: This must happen before we spawn everybody back in,
	// so checks for level.warmupTime are valid.
	StartWarmup();
