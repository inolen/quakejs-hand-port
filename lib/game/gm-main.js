...
function ChangeWarmupState(warmupTime) {
	...
	// End the current match if back in waiting state in CA.
	if (warmupTime === WARMUP_WAITING && g_gametype() === GT.CLANARENA) {
		EndMatch(null);
	}
}
...
function EndMatch(winningTeam) {
	...
	// No team won, we just entered warmup and should
	// spawn dead specs.
	if (winningTeam === null) {
		SpawnSpectatingPlayers();
		return;
	}
 ...
 * SpawnSpectatingPlayers
 */
function SpawnSpectatingPlayers() {
	for (var i = 0; i < level.maxclients; i++) {
		var ent = level.gentities[i];
		if (!ent.inuse) {
			continue;
		...
		// Don't re-spawn spectators or people who aren't dead.
		var client = ent.client;
		if (client.sess.sessionTeam === TEAM.SPECTATOR ||
