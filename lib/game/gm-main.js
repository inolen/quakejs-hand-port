...
function Frame(levelTime) {
	...
	for (var i = 0; i < level.matches.length; i++) {
		CheckMatchRules(level.matches[i]);
	}
 ...
 * InitMatches
 *
 * Initializes a single default match, as well as one per arena.
 */
function InitMatches() {
	...
	for (var i = 0; i < points.length; i++) {
		...
		if (arenas.indexOf(point.arena) === -1) {
			arenas.push(point.arena);
		}
	}

	for (var i = 0; i < arenas.length; i++) {
		match = new MatchLocals();
		match.arena = arenas[i];
		level.matches.push(match);
	}
}
...
function MoveClientToIntermission(ent) {
	...
	SelectIntermissionSpawnPoint(origin, angles, ent.match.arena);
