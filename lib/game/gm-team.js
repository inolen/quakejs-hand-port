 ...
 * PickTeam
 */
function PickTeam(arenaNum, ignoreClientNum) {
	var arena = level.arenas[arenaNum];

	var team;
	if (g_gametype() >= GT.TEAM) {
		// Always TEAM.FREE in lobby of CA.
		if (g_gametype() === GT.CLANARENA && arenaNum === 0) {
			team = TEAM.FREE;
		} else {
			// Find the team with the least amount of players.
			var counts = new Array(TEAM.NUM_TEAMS);

			counts[TEAM.RED] = TeamCount(TEAM.RED, arenaNum, ignoreClientNum);
			counts[TEAM.BLUE] = TeamCount(TEAM.BLUE, arenaNum, ignoreClientNum);
 ...
 */
function TeamCount(team, arenaNum, ignoreClientNum) {
	var count = 0;
 ...
 * TeamAliveCount
 */
function TeamAliveCount(team, arenaNum) {
	var count = 0;
...
function Team_TouchOurFlag(ent, other, team) {
	...
	Team_AddScore(other.client.sess.sessionTeam, other.client.arenaNum, ent.s.pos.trBase, 1);
 ...
 */
function Team_AddScore(team, arenaNum, origin, score) {
	var arena = GetArena(arenaNum);

	var tent = TempEntity(origin, EV.GLOBAL_TEAM_SOUND);
	tent.svFlags |= SVF.BROADCAST;

	if (team === TEAM.RED) {
		if (arena.teamScores[TEAM.RED] + score === arena.teamScores[TEAM.BLUE]) {
			// Teams are tied sound.
			tent.s.eventParm = GTS.TEAMS_ARE_TIED;
		} else if (arena.teamScores[TEAM.RED] <= arena.teamScores[TEAM.BLUE] && arena.teamScores[TEAM.RED] + score > arena.teamScores[TEAM.BLUE]) {
		...
		if (arena.teamScores[TEAM.BLUE] + score === arena.teamScores[TEAM.RED]) {
			// Teams are tied sound.
			tent.s.eventParm = GTS.TEAMS_ARE_TIED;
		} else if (arena.teamScores[TEAM.BLUE] <= arena.teamScores[TEAM.RED] && arena.teamScores[TEAM.BLUE] + score > arena.teamScores[TEAM.RED]) {
	...
	arena.teamScores[team] += score;
}

/**
 * SelectCTFSpawnPoint
 */
function SelectCTFSpawnPoint(team, teamstate, origin, angles, arenaNum) {
	var classname;
	...
	if (!classname) {
		return SelectSpawnPoint(QMath.vec3origin, origin, angles, arenaNum);
	}
