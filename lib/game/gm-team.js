...
function PickTeam(ignoreClientNum) {
	...
	if ((level.arena.gametype === GT.ROCKETARENA && ArenaCount(ignoreClientNum) >= playersPerTeam*2) ||
	    (level.arena.gametype === GT.TOURNAMENT && level.arena.numNonSpectatorClients >= 2)) {
		team = TEAM.SPECTATOR;
	}
...
function TeamCount(team, ignoreClientNum) {
	var gametype = level.arena.gametype;
	var count = 0;
	...
	for (var i = 0; i < level.maxclients; i++) {
		...
		if ((gametype === GT.ROCKETARENA && ent.client.sess.arenaTeam === team) ||
