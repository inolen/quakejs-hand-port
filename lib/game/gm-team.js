...
function TeamAliveCount(team, arenaNum) {
	...
	for (var i = 0; i < level.maxclients; i++) {
		...
		if ((ent.client.sess.spectatorState === SPECTATOR.NOT && ent.client.ps.pm_type !== PM.DEAD)) {
			count++;
		}
	}
...
function Team_TouchOurFlag(ent, other, team) {
	...
	Team_AddScore(other.client.sess.sessionTeam, other.s.arenaNum, ent.s.pos.trBase, 1);
...
// 		G_Printf ("Warning:  NULL passed to Team_ReturnFlagSound\n");
	...
	var temp = TempEntity(ent.s.arenaNum, ent.s.pos.trBase, EV.GLOBAL_TEAM_SOUND);
	if (team == TEAM.BLUE ) {
...
function Team_AddScore(team, arenaNum, origin, score) {
	var tent = TempEntity(arenaNum, origin, EV.GLOBAL_TEAM_SOUND);
...
function SelectCTFSpawnPoint(team, teamstate, origin, angles, arenaNum) {
	...
	for (var i = 0; i < spawnpoints.length; i++) {
		...
		if (spot.arena !== ARENANUM_NONE && spot.arena !== arenaNum) {
			continue;
		}
