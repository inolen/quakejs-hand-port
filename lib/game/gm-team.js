 ...
 * For gametype GT_TEAM the match.teamScores is updated in AddScore in g_combat.c
 */
function AddTeamScore(match, team, origin, score) {
	var tent = TempEntity(origin, EV.GLOBAL_TEAM_SOUND);
	tent.svFlags |= SVF.BROADCAST;

	if (team === TEAM.RED) {
		if (match.teamScores[TEAM.RED] + score === match.teamScores[TEAM.BLUE]) {
			// Teams are tied sound.
			tent.s.eventParm = GTS.TEAMS_ARE_TIED;
		} else if (match.teamScores[TEAM.RED] <= match.teamScores[TEAM.BLUE] && match.teamScores[TEAM.RED] + score > match.teamScores[TEAM.BLUE]) {
		...
		if (match.teamScores[TEAM.BLUE] + score === match.teamScores[TEAM.RED]) {
			// Teams are tied sound.
			tent.s.eventParm = GTS.TEAMS_ARE_TIED;
		} else if (match.teamScores[TEAM.BLUE] <= match.teamScores[TEAM.RED] && match.teamScores[TEAM.BLUE] + score > match.teamScores[TEAM.RED]) {
...
function Team_TouchOurFlag(ent, other, team) {
	...
	AddTeamScore(other.match, other.client.sess.sessionTeam, ent.s.pos.trBase, 1);
