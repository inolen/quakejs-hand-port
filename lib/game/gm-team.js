...
function PickTeam(arena) {
	...
	if (g_gametype() >= GT.TEAM) {
		...
		if (g_gametype() === GT.CLANARENA && arena === 0) {
			...
			else if (level.teamScores[TEAM.BLUE] > level.teamScores[TEAM.RED]) {
				team = TEAM.RED;
			} else {
				team = TEAM.BLUE;
			}
	...
	if (g_gametype() === GT.TOURNAMENT && level.numNonSpectatorClients >= 2) {
		team = TEAM.SPECTATOR;
	} else if (g_gametype() === GT.CLANARENA && TEAM_SIZE && level.numNonSpectatorClients >= TEAM_SIZE*2) {
		team = TEAM.SPECTATOR;
	} else if (g_maxGameClients() > 0 && level.numNonSpectatorClients >= g_maxGameClients()) {
		team = TEAM.SPECTATOR;
	}

	log('PickTeam', level.numNonSpectatorClients, team);
