...
function PickTeam(ignoreClientNum) {
	...
	if (g_gametype.getAsInt() >= GT.TEAM) {
		...
		// If we've exceeded the amount of players, kick to spectator.
		var total = counts[TEAM.RED] + counts[TEAM.BLUE];
		var playersPerTeam = GetArenaCvar('g_playersPerTeam');
		if (g_gametype.getAsInt() === GT.CLANARENA && playersPerTeam && total >= playersPerTeam*2) {
			team = TEAM.SPECTATOR;
		}
	}

	// Override decision if limiting the players.
	if (g_gametype.getAsInt() === GT.TOURNAMENT && level.arena.numNonSpectatorClients >= 2) {
		team = TEAM.SPECTATOR;
	}
...
function Team_ResetFlags() {
	...
	} else if (g_gametype.getAsInt() == GT.NFCTF) {
