...
function PickTeam(ignoreClientNum) {
	...
	if ((g_gametype.at(level.arena.arenaNum).get() === GT.ROCKETARENA && ArenaCount(ignoreClientNum) >= playersPerTeam*2) ||
	    (g_gametype.at(level.arena.arenaNum).get() === GT.TOURNAMENT && level.arena.numNonSpectatorClients >= 2)) {
		team = TEAM.SPECTATOR;
	}
...
function TeamCount(team, ignoreClientNum) {
	var gametype = g_gametype.at(level.arena.arenaNum).get();
	var count = 0;
	...
	for (var i = 0; i < level.maxclients; i++) {
		...
		if ((gametype === GT.ROCKETARENA && ent.client.sess.raTeam === team) ||
			(gametype !== GT.ROCKETARENA && ent.client.sess.team === team)) {
			count++;
		}
	}
...
function Team_InitGame() {
	...
	switch (g_gametype.at(level.arena.arenaNum).get()) {
		case GT.CTF:
...
function Team_ResetFlags() {
	...
	} else if (g_gametype.at(level.arena.arenaNum).get() == GT.NFCTF) {
