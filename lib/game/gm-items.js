...
function ItemTouch(ent, other) {
	...
	if (!BG.CanItemBeGrabbed(g_gametype.at(level.arena.arenaNum).get(), ent.s, other.client.ps)) {
		return;
	}
...
function PickupWeapon(ent, other) {
	...
	if (ent.count < 0) {
		...
		if (!(ent.flags & GFL.DROPPED_ITEM) && g_gametype.at(level.arena.arenaNum).get() !== GT.TEAM ) {
			// Respawning rules.
...
function PickupPowerup(ent, other) {
	...
	for (var i = 0; i < MAX_CLIENTS; i++) {
		...
		if (g_gametype.at(level.arena.arenaNum).get() >= GT.TEAM && other.client.sess.team === client.sess.team) {
			continue;
		}
...
function ItemDrop(ent, item, angle) {
	...
	if ((g_gametype.at(level.arena.arenaNum).get() == GT.CTF || g_gametype.at(level.arena.arenaNum).get() == GT.NFCTF) && item.giType == IT.TEAM) { // Special case for CTF flags
