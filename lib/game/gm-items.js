...
function RunItem(ent) {
	...
	var tr = sv.Trace(ent.r.currentOrigin, origin, ent.r.mins, ent.r.maxs, ent.r.ownerNum, mask);
...
function FinishSpawningItem(ent) {
	...
	if (ent.spawnflags & 1) {
		...
		var tr = sv.Trace(ent.s.origin, dest, ent.r.mins, ent.r.maxs, ent.s.number, MASK.SOLID);
		if (tr.startSolid) {
