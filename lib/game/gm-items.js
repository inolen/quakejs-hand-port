...
function RunItem(ent) {
	...
	var tr = sv.Trace(ent.r.currentOrigin, origin, ent.r.mins, ent.r.maxs, ent.s.arenaNum, ent.r.ownerNum, mask);
	...
	var contents = sv.PointContents(ent.r.currentOrigin, -1);
...
function FinishSpawningItem(ent) {
	...
	if (ent.spawnflags & 1) {
		...
		var tr = sv.Trace(ent.s.origin, dest, ent.r.mins, ent.r.maxs, ent.s.arenaNum, ent.s.number, MASK.SOLID);
		if (tr.startSolid) {
...
function RespawnItem(ent) {
	...
	if (ent.item.giType === IT.POWERUP) {
		...
		if (ent.speed) {
			tent = TempEntity(ent.s.arenaNum, ent.s.pos.trBase, EV.GENERAL_SOUND);
		}
		else {
			tent = TempEntity(ent.s.arenaNum, ent.s.pos.trBase, EV.GLOBAL_SOUND);
		}
...
//
	...
	if (ent.item.giType === IT.POWERUP || ent.item.giType === IT.TEAM) {
		...
		if (!ent.speed) {
			tent = TempEntity(ent.s.arenaNum, ent.s.pos.trBase, EV.GLOBAL_ITEM_PICKUP);
...
function PickupPowerup(ent, other) {
	...
	for (var i = 0; i < MAX_CLIENTS; i++) {
		...
		var tr = sv.Trace(client.ps.origin, ent.s.pos.trBase, null, null, ent.s.arenaNum, ENTITYNUM_NONE, CONTENTS.SOLID);
		if (tr.fraction !== 1.0) {
			continue;
		}
...
function ItemDrop(ent, item, angle) {
	...
	// Create dropped item.
	var dropped = SpawnEntity(ent.s.arenaNum);
