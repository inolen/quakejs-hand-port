...
function LaunchItem(item, origin, velocity) {
	...
	dropped.item = item;
	vec3.set(dropped.mins, -ITEM_RADIUS, -ITEM_RADIUS, -ITEM_RADIUS);
	vec3.set(dropped.maxs, ITEM_RADIUS, ITEM_RADIUS, ITEM_RADIUS);
 ...
 * Spawns an item and tosses it forward
 */
function Drop_Item (ent, item, angle) {
	var velocity = [0, 0, 0];
	var angles   = [0, 0, 0];
 ...
 * Respawn the item
 */
function Use_Item (ent, other, activator) {
	RespawnItem(ent);
}
