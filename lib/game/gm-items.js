...
function ItemTouch(ent, other) {
	...
	if (!bg.CanItemBeGrabbed(g_gametype.getAsInt(), ent.s, other.client.ps)) {
		return;
	}
...
function PickupWeapon(ent, other) {
	...
	if (ent.count < 0) {
		...
		if (!(ent.flags & GFL.DROPPED_ITEM) && g_gametype.getAsInt() !== GT.TEAM ) {
			// Respawning rules.
	...
	if (g_gametype.getAsInt() === GT.TEAM) {
		return g_weaponTeamRespawn.getAsInt();
	}

	return g_weaponRespawn.getAsInt();
}
...
function PickupPowerup(ent, other) {
	...
	for (var i = 0; i < MAX_CLIENTS; i++) {
		...
		if (g_gametype.getAsInt() >= GT.TEAM && other.client.sess.sessionTeam === client.sess.sessionTeam) {
			continue;
		}
		...
		Trace(trace, client.ps.origin, ent.s.pos.trBase, null, null, ENTITYNUM_NONE, CONTENTS.SOLID);
...
function ItemDrop(ent, item, angle) {
	...
	if ((g_gametype.getAsInt() == GT.CTF || g_gametype.getAsInt() == GT.NFCTF) && item.giType == IT.TEAM) { // Special case for CTF flags
