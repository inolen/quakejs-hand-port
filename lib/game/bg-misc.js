...
function AddPredictableEventToPlayerstate(ps, newEvent, eventParm) {
	ps.events[ps.eventSequence & (QS.MAX_PS_EVENTS - 1)] = newEvent;
	ps.eventParms[ps.eventSequence & (QS.MAX_PS_EVENTS - 1)] = eventParm;
	ps.eventSequence++;
}
...
function PlayerStateToEntityState(ps, es) {
	...
	if (ps.externalEvent) {
		...
		var seq = ps.entityEventSequence & (QS.MAX_PS_EVENTS - 1);
