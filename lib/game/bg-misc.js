 ...
 */
function GetWaterLevel(origin, viewheight, arenaNum, passEntityNum, pointContents) {
	var waterlevel = 0;

	var point = vec3.create(origin);
	point[2] += MINS_Z + 1;

	var contents = pointContents(point, arenaNum, passEntityNum);
	...
	if (contents & MASK.WATER) {
		...
		contents = pointContents(point, arenaNum, passEntityNum);
