...
function SpawnEntityFromDef(def) {
	...
	if (gametype !== null) {
		if (g_gametype.at(level.arena.arenaNum).get() >= GT.FFA && g_gametype.at(level.arena.arenaNum).get() < GT.MAX_GAME_TYPE) {
			var gametypeName = BG.GametypeNames[g_gametype.at(level.arena.arenaNum).get()];
