 ...
 */
function InitArenas() {
	var arenastr = g_arenas.getAsString();
	var arenas = arenastr ? arenastr.split(',') : null;
	...
	// Set arena string.
	arenastr = arenas.join(',');
	g_arenas.setValue(arenastr);
	...
	sv.SetConfigstring('arenaInfo', Cvar.GetCvarJSON(QShared.CVAR.ARENAINFO));
}
...
function SetArenaConfigstring(key, val) {
	var str = sv.GetConfigstring(key) || '';

	// We really should pass this as an array, however, we're matching what cvars do.
	var split = str.split(',');
	split[level.arena.arenaNum] = val;

	str = split.join(',');

	sv.SetConfigstring(key, str);
}
...
function CalculateRanks() {
	...
	if (g_gametype.getAsInt() >= GT.TEAM) {
		...
		SetArenaConfigstring('score1', { score: level.arena.teamScores[TEAM.RED],  num: count1 });
		SetArenaConfigstring('score2', { score: level.arena.teamScores[TEAM.BLUE], num: count2 });
	} else {
		var n1 = level.arena.sortedClients[0];
		var n2 = level.arena.sortedClients[1];

		if (level.arena.numConnectedClients === 0) {
			SetArenaConfigstring('score1', { score: SCORE_NOT_PRESENT, num: ENTITYNUM_NONE });
			SetArenaConfigstring('score2', { score: SCORE_NOT_PRESENT, num: ENTITYNUM_NONE });
		} else if (level.arena.numConnectedClients === 1) {
			SetArenaConfigstring('score1', { score: level.clients[n1].ps.persistant[PERS.SCORE], num: n1 });
			SetArenaConfigstring('score2', { score: SCORE_NOT_PRESENT,                           num: ENTITYNUM_NONE });
		} else {
			SetArenaConfigstring('score1', { score: level.clients[n1].ps.persistant[PERS.SCORE], num: n1 });
			SetArenaConfigstring('score2', { score: level.clients[n2].ps.persistant[PERS.SCORE], num: n2 });
		}
	}
