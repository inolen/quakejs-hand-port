...
function TeamInfoChanged() {
	for (var i = 0; i < MAX_CLIENTS; i++) {
		...
		var key = 'team:' + level.arena.arenaNum + ':' + i;
		var last = SV.GetConfigstring(key);

		// Don't set the configstring if there are no clients (unless we need to 0 it out).
		if (info.c === 0 && (!last || last.c === 0)) {
			continue;
		}

		SV.SetConfigstring(key, info);
	}
}
...
function RoundRunWaiting() {
	if (level.arena.gametype === GT.CLANARENA) {
		// Force red / blue in CA.
		level.arena.team1 = TEAM.RED;
		level.arena.team2 = TEAM.BLUE;

		var count1 = TeamCount(level.arena.team1, ENTITYNUM_NONE);
		var count2 = TeamCount(level.arena.team2, ENTITYNUM_NONE);

		if (count1 >= 1 && count2 >= 1) {
			level.arena.state.ready();
		}
	} else if (level.arena.gametype === GT.ROCKETARENA) {
		var count1 = function () { return TeamCount(level.arena.team1, ENTITYNUM_NONE); }
		var count2 = function () { return TeamCount(level.arena.team2, ENTITYNUM_NONE); }

		// Spawn in the next team.
		while (true) {
			// Free up the team if they all left.
			if (!count1()) {
				level.arena.team1 = TEAM.NONE;
			}
			if (!count2()) {
				level.arena.team2 = TEAM.NONE;
			}
			...
			var dequeuedTeam = DequeueNextTeam();
			if (dequeuedTeam === TEAM.NONE) {
				break;
			}

			if (!count1()) {
				level.arena.team1 = dequeuedTeam;
			} else {
				level.arena.team2 = dequeuedTeam;
			}
		}
...
function RoundRunActive() {
	var alive1 = TeamAliveCount(level.arena.team1);
	var alive2 = TeamAliveCount(level.arena.team2);

	if (!alive1 && !alive2) {
		level.arena.state.end(null);
	} else if (!alive1) {
		level.arena.state.end(level.arena.team2);
	} else if (!alive2) {
		level.arena.state.end(level.arena.team1);
	}
}
...
function RoundRestart() {
	...
	var loser = level.arena.team1 === level.arena.lastWinningTeam ? level.arena.team2 : level.arena.team2;

	// Queue losing team in RA.
	if (level.arena.gametype === GT.ROCKETARENA) {
		QueueTeam(loser);
	}
	// Always respawn losing team in CA.
	else if (level.arena.gametype === GT.CLANARENA) {
		RespawnTeam(loser);
	}

	RespawnTeam(level.arena.lastWinningTeam);
}

/**
 * DequeueNextTeam
 */
function DequeueNextTeam() {
	...
	for (var i = 0; i < level.maxclients; i++) {
		...
		if (ent.client.sess.arenaTeam === nextInLine.client.sess.arenaTeam) {
	...
	for (var i = 0; i < ents.length; i++) {
		...
		ent.client.sess.team = level.arena.team1 === null ? TEAM.RED : TEAM.BLUE;
	...
	return nextInLine.client.sess.arenaTeam;
}

/**
 * QueueTeam
 */
function QueueTeam(team) {
	for (var i = 0; i < level.maxclients; i++) {
		var ent = level.gentities[i];
...
function CalculateRanks() {
	...
	var score1 = level.arena.score1 = {};
	var score2 = level.arena.score2 = {};

	if (level.arena.gametype >= GT.CLANARENA) {
		score1.team = level.arena.team1;
		score1.count = TeamCount(level.arena.team1, ENTITYNUM_NONE);
		score1.alive = TeamAliveCount(level.arena.team1);

		score2.team = level.arena.team2;
		score2.count = TeamCount(level.arena.team2, ENTITYNUM_NONE);
		score2.alive = TeamAliveCount(level.arena.team2);
	}
	else if (level.arena.gametype >= GT.TEAM) {
		score1.team = TEAM.RED;
		score1.count = TeamCount(TEAM.RED, ENTITYNUM_NONE);
		score1.alive = TeamAliveCount(TEAM.RED);

		score2.team = TEAM.BLUE;
		score2.count = TeamCount(TEAM.BLUE, ENTITYNUM_NONE);
		score2.alive = TeamAliveCount(TEAM.BLUE);
	} else {
		...
		if (level.arena.numConnectedClients === 0) {
			...
			score1.clientNum = n1;
			score1.score = level.clients[n1].ps.persistant[PERS.SCORE];
			...
			score2.clientNum = n2;
			score2.score = level.clients[n2].ps.persistant[PERS.SCORE];
		}
	}
