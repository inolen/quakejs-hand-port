...
function InitArenas() {
	...
	for (var i = 0; i < arenas.length; i++) {
		...
		} else if (g_gametype.get() === GT.CLANARENA) {
			CreateRoundMachine();
		}
	}
...
function CheckMatchRules() {
	...
	// if (g_timelimit.get() && level.warmupTime === WARMUP_OVER) {
	// 	if (level.time - level.startTime >= g_timelimit.get() * 60000) {
	...
	// 	if (level.teams[TEAM.RED].score >= g_fraglimit.get()) {
	// 		SV.SendServerCommand(null, 'print', 'Red hit the fraglimit.');
	// 		QueueIntermission();
	// 		return;
	// 	}

	// 	if (level.teams[TEAM.BLUE].score >= g_fraglimit.get()) {
	...
	// if (g_gametype.get() >= GT.CTF && g_gametype.get() !== GT.CLANARENA && g_capturelimit.get()) {
	// 	if (level.teams[TEAM.RED].score >= g_capturelimit.get()) {
	// 		SV.SendServerCommand(null, 'print', 'Red hit the capturelimit.');
	// 		QueueIntermission();
	// 		return;
	// 	}

	// 	if (level.teams[TEAM.BLUE].score >= g_capturelimit.get()) {
