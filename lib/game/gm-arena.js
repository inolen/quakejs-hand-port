...
function ArenaInfoChanged() {
	var info = {
		...
		'c1': level.arena.count1,
		'c2': level.arena.count2,
		'a1': level.arena.alive1,
		'a2': level.arena.alive2,
		'gs': level.arena.gamestate,
...
function ChangeGameState(gamestate) {
	if (level.arena.gamestate === gamestate) {
		return;
	}

	level.arena.gamestate = gamestate;
...
function CheckTournamentWarmup() {
	...
	if (level.arena.gamestate > GAMESTATE.COUNTDOWN) {
		return;
	}
	...
	if (level.arena.gamestate === GAMESTATE.WAITING) {
		if (level.arena.numPlayingClients === 2) {
	...
	else if (level.arena.gamestate === GAMESTATE.COUNTDOWN && level.time > level.arena.warmupTime) {
...
function CheckTeamWarmup() {
	// CA doesn't go back to warmup when there are too few players,
	// the match just ends.
	if (level.arena.gamestate > GAMESTATE.COUNTDOWN) {
		return;
	}
...
function CheckRoundRules() {
	...
	if (level.arena.gamestate === GAMESTATE.OVER && level.time > level.arena.restartTime) {
		RestartRound();
		return;
	}

	if (level.arena.gamestate === GAMESTATE.ACTIVE) {
