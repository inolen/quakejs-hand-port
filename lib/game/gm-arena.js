...
function CheckArenaEnd(arena) {
	...
	if (!counts[TEAM.RED]) {
		EndArena(arena, TEAM.BLUE);
	} else if (!counts[TEAM.BLUE]) {
		EndArena(arena, TEAM.RED);
	}
}
 ...
 * RestartArena
 *
 * Called a few seconds after a arena has ended.
 */
function RestartArena(arena) {
	log('RestartArena');
	...
	if (TEAM_SIZE > 0) {
		for (var i = 0; i < level.maxclients; i++) {
			...
			if (client.arenaNum !== arena.number ||
				...
				client.sess.sessionTeam === arena.lastWinningTeam) {
				continue;
			}
...
function StartArena(arena) {
	...
	// Everybody should already have been spawned back in by RestartArena, but
 ...
 * EndArena
 */
function EndArena(arena, winningTeam) {
	log('EndArena');
