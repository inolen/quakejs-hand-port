...
function RunArenas() {
	...
	if (Cvar.Modified(QShared.CVAR.ARENAINFO)) {
		ArenaInfoChanged();
		Cvar.ClearModified(QShared.CVAR.ARENAINFO);
	}
...
function CalculateRanks() {
	...
	if (g_gametype.getAsInt() >= GT.TEAM) {
		...
		if (g_gametype.getAsInt() === GT.CLANARENA) {
			level.arena.count1 = TeamAliveCount(TEAM.RED);
			level.arena.count2 = TeamAliveCount(TEAM.BLUE);
		} else {
