...
function ArenaInfoChanged() {
	var info = {
		...
		'fraglimit': g_fraglimit.at(level.arena.arenaNum).getAsInt(),
		'capturelimit': g_capturelimit.at(level.arena.arenaNum).getAsInt(),
		'playersPerTeam': g_playersPerTeam.at(level.arena.arenaNum).getAsInt(),
		'numPlayingClients': level.arena.numPlayingClients,
		'score1': level.arena.score1,
		'score2': level.arena.score2,
		'count1': level.arena.count1,
		'count2': level.arena.count2,
		'alive1': level.arena.alive1,
		'alive2': level.arena.alive2,
		'warmup': level.arena.warmupTime
	};
...
function CheckWarmup() {
	...
	} else if (g_gametype.getAsInt() >= GT.TEAM) {
		CheckTeamWarmup();
	}
}
	...
	if (level.arena.warmupTime < 0) {
		if (level.arena.numPlayingClients === 2) {
			...
			if (g_warmup.getAsInt() > 1) {
				warmupTime = level.time + (g_warmup.getAsInt() - 1) * 1000;
			}
...
function CheckTeamWarmup() {
	...
	var notEnough = false;
	var islobby = g_gametype.getAsInt() === GT.CLANARENA && level.arena.arenaNum === 0;
	...
	// If the warmup time has counted down, restart.
	if (level.time > level.arena.warmupTime) {
		if (g_gametype.getAsInt() === GT.CLANARENA) {
			StartRound();
		} else {
...
function CheckMatchRules() {
	...
	// 	if (level.teamScores[TEAM.RED] >= g_fraglimit.getAsInt()) {
	// 		SV.SendServerCommand(null, 'print', 'Red hit the fraglimit.');
	// 		QueueIntermission();
	// 		return;
	// 	}

	// 	if (level.teamScores[TEAM.BLUE] >= g_fraglimit.getAsInt()) {
	...
	// 	if (level.teamScores[TEAM.RED] >= g_capturelimit.getAsInt()) {
	// 		SV.SendServerCommand(null, 'print', 'Red hit the capturelimit.');
	// 		QueueIntermission();
	// 		return;
	// 	}

	// 	if (level.teamScores[TEAM.BLUE] >= g_capturelimit.getAsInt()) {
...
function PopClientFromQueue() {
	var clientsNeeded = g_playersPerTeam.at(level.arena.arenaNum).getAsInt() * 2;
...
function CalculateRanks() {
	...
	if (g_gametype.getAsInt() >= GT.TEAM) {
		level.arena.score1 = level.arena.teamScores[TEAM.RED];
		level.arena.score2 = level.arena.teamScores[TEAM.BLUE];

		level.arena.count1 = TeamCount(TEAM.RED, ENTITYNUM_NONE);
		level.arena.count2 = TeamCount(TEAM.BLUE, ENTITYNUM_NONE);

		level.arena.alive1 = TeamAliveCount(TEAM.RED);
		level.arena.alive2 = TeamAliveCount(TEAM.BLUE);
	} else {
		var n1 = level.arena.sortedClients[0];
		var n2 = level.arena.sortedClients[1];

		if (level.arena.numConnectedClients === 0) {
			level.arena.score1 = SCORE_NOT_PRESENT;
			level.arena.score2 = SCORE_NOT_PRESENT;

			level.arena.count1 = ENTITYNUM_NONE;
			level.arena.count2 = ENTITYNUM_NONE;
		} else if (level.arena.numConnectedClients === 1) {
			level.arena.score1 = level.clients[n1].ps.persistant[PERS.SCORE];
			level.arena.count1 = n1;
			...
			level.arena.score2 = level.clients[n2].ps.persistant[PERS.SCORE];
			level.arena.count2 = n2;
		}
	}
