...
function InitArenas() {
	...
	for (var i = 0; i < arenas.length; i++) {
		...
		//
		if (g_gametype.at(level.arena.arenaNum).get() >= GT.CLANARENA) {
			CreateRoundMachine();
		} else {
	...
	if (g_gametype.at(level.arena.arenaNum).get() === GT.CTF) {
		Team_CheckItems();
	}
}
...
function ArenaInfoChanged() {
	var info = {
		'gt': g_gametype.at(level.arena.arenaNum).get(),
...
function TeamInfoChanged() {
	for (var i = 0; i < MAX_CLIENTS; i++) {
		...
		// log('TeamInfoChanged', JSON.stringify(info));
...
function CheckTournamentRules() {
	...
	var enough = function () {
		if (g_gametype.at(level.arena.arenaNum).get() >= GT.TEAM) {
	...
	if (state.current === GS.WAITING) {
		...
		if (g_gametype.at(level.arena.arenaNum).get() === GT.TOURNAMENT) {
			while (true) {
				if (enough()) {
					break;
				}
		...
		if (g_gametype.at(level.arena.arenaNum).get() < GT.CTF && g_fraglimit.get()) {
		...
		} else if (g_gametype.at(level.arena.arenaNum).get() >= GT.CTF && g_capturelimit.get()) {
...
function CalculateRanks() {
	...
	if (g_gametype.at(level.arena.arenaNum).get() === GT.ROCKETARENA) {
		for (var i = 0; i < MAX_CLIENTS; i++) {
			level.arena.teams[i].count = TeamCount(i, ENTITYNUM_NONE);
			level.arena.teams[i].alive = TeamAliveCount(i);
		}
	}
	else if (g_gametype.at(level.arena.arenaNum).get() >= GT.TEAM) {
