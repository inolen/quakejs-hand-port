...
function CalculateRanks() {
	...
	if (g_gametype.get() >= GT.CLANARENA) {
		score1.score = level.arena.teams[level.arena.team1].score;
		score1.count = TeamCount(level.arena.team1, ENTITYNUM_NONE);
		score1.alive = TeamAliveCount(level.arena.team1);

		score2.score = level.arena.teams[level.arena.team2].score;
		...
		score1.score = level.arena.teams[TEAM.RED].score;
		score1.count = TeamCount(TEAM.RED, ENTITYNUM_NONE);
		score1.alive = TeamAliveCount(TEAM.RED);

		score2.score = level.arena.teams[TEAM.BLUE].score;
		...
		if (level.arena.numConnectedClients === 0) {
			score1.score = SCORE_NOT_PRESENT;
			score1.clientNum = ENTITYNUM_NONE;

			score2.score = SCORE_NOT_PRESENT;
			score2.clientNum = ENTITYNUM_NONE;
