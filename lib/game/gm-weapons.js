...
function CheckGauntletAttack(ent) {
	...
	var tr = sv.Trace(muzzle, end, null, null, ent.s.arenaNum, ent.s.number, MASK.SHOT);
	...
	if (traceEnt.takeDamage && traceEnt.client) {
		var tent = TempEntity(traceEnt.s.arenaNum, tr.endPos, EV.MISSILE_HIT);
...
function BulletFire(ent, spread, damage, mod) {
	...
	for (var i = 0; i < 10; i++) {
		var tr = sv.Trace(muzzle, end, null, null, ent.s.arenaNum, passent, MASK.SHOT);
		...
		var tent;
		if (traceEnt.takeDamage && traceEnt.client) {
			tent = TempEntity(traceEnt.s.arenaNum, tr.endPos, EV.BULLET_HIT_FLESH);
			tent.s.eventParm = traceEnt.s.number;
			if (LogAccuracyHit(traceEnt, ent)) {
				ent.client.accuracy_hits++;
			}
		} else {
			tent = TempEntity(traceEnt.s.arenaNum, tr.endPos, EV.BULLET_HIT_WALL);
			tent.s.eventParm = QMath.DirToByte(tr.plane.normal);
		}
...
function ShotgunFire(ent) {
	...
	// Send shotgun blast.
	var tent = TempEntity(ent.s.arenaNum, muzzle, EV.SHOTGUN);
...
function ShotgunPellet(start, end, ent) {
	...
	for (var i = 0; i < 10; i++) {
		var tr = sv.Trace(start, end, null, null, ent.s.arenaNum, ent.s.number, MASK.SHOT);
		var traceEnt = level.gentities[tr.entityNum];
...
function LightningFire(ent) {
	...
	for (var i = 0; i < 10; i++) {
		...
		var tent;
		if (traceEnt.takeDamage && traceEnt.client) {
			tent = TempEntity(traceEnt.s.arenaNum, tr.endPos, EV.MISSILE_HIT);
			tent.s.otherEntityNum = traceEnt.s.number;
			tent.s.eventParm = QMath.DirToByte(tr.plane.normal);
			tent.s.weapon = ent.s.weapon;
			if (LogAccuracyHit(traceEnt, ent)) {
			...
			tent = TempEntity(traceEnt.s.arenaNum, tr.endPos, EV.MISSILE_MISS);
			tent.s.eventParm = QMath.DirToByte(tr.plane.normal);
			tent.s.weapon = ent.s.weapon;
		}
...
function RailgunFire(ent) {
	...
	while (unlinkedEntities.length < MAX_RAIL_HITS) {
		...
		trace = sv.Trace(muzzle, end, null, null, ent.s.arenaNum, passent, MASK.SHOT);
		...
		if (trace.r.contents & CONTENTS.SOLID) {
	...
	var tent = TempEntity(ent.s.arenaNum, trace.endPos, EV.RAILTRAIL);
