...
function FireWeapon(ent) {
	...
	var up = [0, 0, 0];
	var muzzle = [0, 0, 0];
	AnglesToVectors(client.ps.viewangles, forward, right, up);
...
function BulletFire(ent, muzzle, forward, right, up, spread, damage, mod) {
	...
	for (var i = 0; i < 10; i++) {
		// trap_Trace (&tr, muzzle, null, null, end, passent, ContentMasks.SHOT);

		// if (tr.surfaceFlags & SurfaceFlags.NOIMPACT) {
		// 	return;
		// }

		// var traceEnt = gm.entities[tr.entityNum];
		...
		// 	// }
		// } else {
			// var tent = G_TempEntity( tr.endpos, EntityEvent.BULLET_HIT_WALL);
