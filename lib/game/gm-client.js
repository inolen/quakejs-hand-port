...
function ClientSpawn(ent) {
	...
	// We may want to force the client into a specifc sub-arena in CA.
	if (g_gametype() === GT.CLANARENA && g_forcearena()) {
		ent.arena = g_forcearena();
	}
...
function PickTeam (ignoreClientNum) {
	...
	counts[TEAM.BLUE] = TeamCount(ignoreClientNum, TEAM.BLUE);
	counts[TEAM.RED] = TeamCount(ignoreClientNum, TEAM.RED);
 ...
 */
function TeamCount (ignoreClientNum, team) {
	var i;
	var count = 0;
