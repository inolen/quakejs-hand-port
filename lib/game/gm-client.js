...
function ClientSpawn(ent) {
	...
	if (client.sess.spectatorState !== SPECTATOR.NOT) {
		...
		spawnPoint = SelectCTFSpawnPoint(client.sess.sessionTeam, client.pers.teamState.state,
						spawnOrigin, spawnAngles, ent.s.arenaNum);
	} else {
		// Don't spawn near existing origin if possible.
		spawnPoint = SelectSpawnPoint(client.ps.origin, spawnOrigin, spawnAngles, ent.s.arenaNum);
	}
	...
	if (!level.intermissiontime) {
		if (ent.client.sess.spectatorState === SPECTATOR.NOT) {
			...
			var tent = TempEntity(ent.s.arenaNum, ent.client.ps.origin, EV.PLAYER_TELEPORT_IN);
...
function TouchTriggers(ent) {
	...
	var entityNums = sv.FindEntitiesInBox(mins, maxs, ent.s.arenaNum);
...
function SetTeam(client, teamName) {
	...
	if (teamName === 'scoreboard' || teamName === 'score') {
		...
		if (teamName === 'red' || teamName === 'r') {
			...
			team = PickTeam(ent.s.arenaNum, clientNum);
		}
...
function SpotWouldTelefrag(spot) {
	...
	var entityNums = sv.FindEntitiesInBox(mins, maxs, spot.s.arenaNum);
...
function SelectIntermissionSpawnPoint(origin, angles, arenaNum) {
	...
	if (!points.length) {
		...
		for (i = 0; i < points.length; i++) {
			...
			if (point.arena !== ARENANUM_NONE && point.arena !== arenaNum) {
				break;
			}
		}
...
function InitBodyQueue() {
	...
	for (var i = 0; i < BODY_QUEUE_SIZE; i++) {
		var ent = SpawnEntity(ARENANUM_NONE);
...
function TeleportPlayer(player, origin, angles) {
	...
	if (player.client.sess.spectatorState === SPECTATOR.NOT) {
		var tent = TempEntity(player.s.arenaNum, player.client.ps.origin, EV.PLAYER_TELEPORT_OUT);
		tent.s.clientNum = player.s.clientNum;

		tent = TempEntity(player.s.arenaNum, origin, EV.PLAYER_TELEPORT_IN);
		tent.s.clientNum = player.s.clientNum;
	}
