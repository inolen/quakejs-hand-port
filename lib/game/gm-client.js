...
function ClientBegin(clientNum) {
	...
	// HACK - SetArena calls ClientBegin itself - this is a bad.
	if (g_gametype() === GT.CLANARENA &&
		g_forcearena() && ent.arena !== g_forcearena()) {
		SetArena(ent, g_forcearena());
		return;
	}
...
function ClientSpawn(ent) {
	...
	var spawnPoint;
	if (client.sess.sessionTeam === TEAM.SPECTATOR) {
		spawnPoint = SelectSpectatorSpawnPoint(spawnOrigin, spawnAngles, ent.arena);
...
function SelectSpectatorSpawnPoint(origin, angles, arena) {
	return FindIntermissionPoint(origin, angles, arena);
}
