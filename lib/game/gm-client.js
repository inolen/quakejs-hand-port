 ...
 *
 * Called by CalculateRanks and directly by the server system when
 * the player updates a userinfo variable.
 *
...
function ClientUserinfoChanged(clientNum) {
	...
	// Update the game's internal name with the userinfo sent from the client.
	client.pers.name = userinfo['name'];

	// Set the internal CS for this client that is propogated out to all clients.
	var ping = -1;
