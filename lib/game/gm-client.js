...
function SetTeam(client, teamName, silent) {
	...
	if (teamName === 'scoreboard' || teamName === 'score') {
		...
		if (teamName === '<default>') {
			teamName = ent.client.pers.name + '\'s team';
		}

		// Try to find the team.
		for (var i = 0; i < MAX_CLIENTS; i++) {
			var arenaTeam = level.arena.teams[i];
			if (arenaTeam.name === teamName) {
				ent.client.sess.arenaTeam = i;
				break;
			}
		}
		...
		if (i === MAX_CLIENTS) {
			var arenaTeam = level.arena.teams[ent.client.ps.clientNum];
			arenaTeam.name = teamName;
