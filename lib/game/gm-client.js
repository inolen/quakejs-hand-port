...
function ClientConnect(clientNum, firstTime) {
	...
	ent.client = client;
	// Place in default arena at first.
	ent.match = level.matches[0];
...
function ClientSpawn(ent) {
	...
	if (g_gametype() === GT.CLANARENA && match.warmupTime === WARMUP_OVER) {
	...
	if (client.sess.spectatorState !== SPECTATOR.NOT) {
		spawnPoint = SelectSpectatorSpawnPoint(spawnOrigin, spawnAngles, match.arena);
	} else if (g_gametype() >= GT.CTF) {
		// All base oriented team games use the CTF spawn points.
		spawnPoint = SelectCTFSpawnPoint(client.sess.sessionTeam, client.pers.teamState.state,
						spawnOrigin, spawnAngles, match.arena);
	} else {
		// Don't spawn near existing origin if possible.
		spawnPoint = SelectSpawnPoint(client.ps.origin, spawnOrigin, spawnAngles, match.arena);
	}
...
function SetArena(ent, arena) {
	...
	// Currently, arenas and matches have a 1 <-> 1 mapping.
	var match;

	for (var i = 0; i < level.matches.length; i++) {
		if (level.matches[i].arena === arena) {
			match = level.matches[i];
			break;
		}
	}

	ent.match = match;
	client.sess.sessionTeam = PickTeam(ent.match);
