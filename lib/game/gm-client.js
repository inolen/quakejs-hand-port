...
function SetTeam(client, teamName, silent) {
	...
	if (teamName === 'scoreboard' || teamName === 'score') {
		...
		if (i === MAX_CLIENTS) {
			var at = level.arena.teams[ent.client.ps.clientNum];
			at.name = teamName;

			arenaTeam = ent.client.ps.clientNum;
		}
	...
	client.sess.arenaTeam = arenaTeam;
