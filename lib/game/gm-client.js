...
function ClientSpawn(ent) {
	...
	// Don't allow firing during CA warmups.
...
function SetArena(ent, arena) {
	...
	if (arena === 0) {
		client.sess.sessionTeam = TEAM.FREE;
	} else {
...
function SelectIntermissionSpawnPoint(origin, angles, arena) {
	...
	var point;
	if (!points.length) {
		point = SelectSpawnPoint(QMath.vec3_origin, origin, angles, arena);
	} else {
		var i;
		...
		if (point.target) {
			...
			if (target) {
				var dir = vec3.subtract(target.s.origin, origin, [0, 0, 0]);
...
function CopyToBodyQueue(ent) {
	...
	// var contents = trap_PointContents(ent.s.origin, -1);
