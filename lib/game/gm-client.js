...
function ClientConnect(clientNum, firstTime) {
	...
	if (g_gametype.getAsInt() >= GT.TEAM && client.sess.sessionTeam !== TEAM.SPECTATOR) {
...
function ClientSpawn(ent) {
	...
	if (client.sess.sessionTeam === TEAM.SPECTATOR ||
		...
		(g_gametype.getAsInt() === GT.CLANARENA && client.pers.teamState.state === TEAM_STATE.ELIMINATED)) {
		spawnPoint = SelectSpectatorSpawnPoint(spawnOrigin, spawnAngles);
	} else {
		if (g_gametype.getAsInt() >= GT.CTF) {
	...
	client.inactivityTime = level.time + g_inactivity.getAsInt() * 1000;
...
function RunClient(ent) {
	if (!g_synchronousClients.getAsInt()) {
		return;
	}
...
function ClientThink_real(ent) {
	...
	if (pmove_msec.getAsInt() < 8) {
		pmove_msec.setValue(8);
	} else if (pmove_msec.getAsInt() > 33) {
		pmove_msec.setValue(33);
	}

	if (pmove_fixed.getAsInt()) {
		cmd.serverTime = ((cmd.serverTime + pmove_msec.getAsInt() - 1) / pmove_msec.getAsInt()) * pmove_msec.getAsInt();
	}
	...
	client.ps.gravity = g_gravity.getAsInt();
	client.ps.speed = g_speed.getAsInt();
	...
	pm.pmove_fixed = pmove_fixed.getAsInt();
	pm.pmove_msec = pmove_msec.getAsInt();
	...
	if (client.ps.pm_type === PM.DEAD) {
		...
		if (level.time > client.respawnTime) {
			...
			if (g_forceRespawn.getAsInt() > 0 &&
				(level.time - client.respawnTime) > g_forceRespawn.getAsInt() * 1000) {
				ClientRespawn(ent);
				return;
			}
