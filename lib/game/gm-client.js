...
function ClientSpawn(ent) {
	...
	if (g_gametype() === GT.CLANARENA && level.warmupTime === WARMUP_OVER) {
	...
	if (client.sess.spectatorState !== SPECTATOR.NOT) {
		spawnPoint = SelectSpectatorSpawnPoint(spawnOrigin, spawnAngles, ent.arenaNum);
	} else if (g_gametype() >= GT.CTF) {
		// All base oriented team games use the CTF spawn points.
		spawnPoint = SelectCTFSpawnPoint(client.sess.sessionTeam, client.pers.teamState.state,
						spawnOrigin, spawnAngles, ent.arenaNum);
	} else {
		// Don't spawn near existing origin if possible.
		spawnPoint = SelectSpawnPoint(client.ps.origin, spawnOrigin, spawnAngles, ent.arenaNum);
	}
...
function SetArena(ent, arenaNum) {
	...
	ent.arenaNum = arenaNum;
	client.sess.sessionTeam = PickTeam(ent.arenaNum);
