...
function ClientConnect(clientNum, firstTime) {
	...
	if (g_gametype.at(level.arena.arenaNum).get() >= GT.TEAM && client.sess.team !== TEAM.SPECTATOR) {
...
function ClientSpawn(ent) {
	...
	if (client.sess.team === TEAM.SPECTATOR ||
		...
		(g_gametype.at(level.arena.arenaNum).get() >= GT.CLANARENA && client.pers.teamState.state === TEAM_STATE.ELIMINATED)) {
