...
function Player_Die(self, inflictor, attacker, damage, meansOfDeath) {
	...
	// } else {
		...
		self.client.ps.legsAnim = ((self.client.ps.legsAnim & bg.ANIM_TOGGLEBIT) ^ bg.ANIM_TOGGLEBIT) | anim;
		self.client.ps.torsoAnim =  ((self.client.ps.torsoAnim & bg.ANIM_TOGGLEBIT) ^ bg.ANIM_TOGGLEBIT ) | anim;
...
function CheckArmor(ent, damage, dflags) {
	...
	var save = Math.ceil(damage * bg.ARMOR_PROTECTION);
	if (save >= count) {
		save = count;
	}
