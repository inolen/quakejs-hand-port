...
function Damage(targ, inflictor, attacker, dir, point, damage, dflags, mod) {
	...
	if (knockback && targ.client) {
		var mass = 200;
		var kvel = vec3.scale(dir, g_knockback.getAsInt() * knockback / mass, vec3.create());
	...
	// No damage during CA warmup.
	if (g_gametype.getAsInt() === GT.CLANARENA && level.arena.gamestate <= GAMESTATE.COUNTDOWN) {
		return;
	}
