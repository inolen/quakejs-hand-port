...
function RadiusDamage(origin, inflictor, attacker, damage, radius, ignore, mod) {
	...
	var entityNums = sv.FindEntitiesInBox(mins, maxs, inflictor.s.arenaNum);
...
function CanDamage(targ, origin) {
	...
	var tr = sv.Trace(origin, dest, QMath.vec3origin, QMath.vec3origin, targ.s.arenaNum, ENTITYNUM_NONE, MASK.SOLID);
	...
	dest[0] += 15.0;
	dest[1] += 15.0;
	tr = sv.Trace(origin, dest, QMath.vec3origin, QMath.vec3origin, targ.s.arenaNum, ENTITYNUM_NONE, MASK.SOLID);
	if (tr.fraction === 1.0) {
		return true;
	}
...
function PlayerDie(self, inflictor, attacker, damage, meansOfDeath) {
	...
	var ent = TempEntity(self.s.arenaNum, self.r.currentOrigin, EV.OBITUARY);
	...
	var contents = sv.PointContents(self.r.currentOrigin, -1);
...
function AddScore(ent, origin, score) {
	...
	if (g_gametype() === GT.TEAM) {
		arena.teamScores[client.ps.persistant[PERS.TEAM]] += score;
	}
...
function ScorePlum(ent, origin, score) {
	var plum = TempEntity(ent.s.arenaNum, origin, EV.SCOREPLUM);
