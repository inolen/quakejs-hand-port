...
function ExecuteBuffer(buffer) {
	...
	// Try to look up the cmd in the register cmds.
	...
	// Otherwise, forward it to the server.
	// Ignore +/- commands, those are often unmapped.
	if (args[0].charAt(0) === '+' ||
		args[0].charAt(0) === '-' ||
		(!dedicated && !cl.ForwardCommandToServer(args))) {
