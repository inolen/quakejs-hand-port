...
function Init(inSYS, inDedicated, callback) {
	...
	//
	// FIXME Should bind be moved to common so we LoadConfig -> cl.Init -> sv.Init ?
	//
	LoadConfig(function () {
		// Don't initialize the server until the configs have been
		// loaded (we want the correct sv_port value).
		sv.Init(cl);
