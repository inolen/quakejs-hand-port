...
// Generate as few of programs as possible.
var programCache = {};
 ...
 *
 **********************************************************/
function compileScript(gl, script, flags, numLights) {
	var builder = buildProgram(script, flags, numLights);
	var fragmentSrc = builder.getFragmentSource();
	var vertexSrc = builder.getVertexSource();

	// FIXME We should compare the script + flags to other scripts
	// in the cache to avoid having to re-build the program each time.
	var key = fragmentSrc + vertexSrc;
	if (programCache[key]) {
	...
	return (programCache[key] = shaderProgram);
}

function buildProgram(script, flags, numLights) {
