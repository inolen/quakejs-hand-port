...
var dmodel_t = function () {
	this.mins         = vec3.create();                     // float32 * 3
	this.maxs         = vec3.create();                     // float32 * 3
...
function ReadFile(path, encoding, callback) {
	...
	path = 'http://localhost:9000/assets/' + path;

	http.get(path, function (res) {
		if (res.statusCode !== 200) {
			return callback(new Error('ReadFile ' + path + ' returned HTTP status code ' + res.statusCode));
		}
...
function LoadWorld(mapName) {
	...
	log('Loading world for', mapName);

	ReadFile('maps/' + mapName + '.bsp', 'binary', function (err, data) {
		if (err) throw err;
...
function LoadBrushModels(world, buffer, lump) {
	...
	for (var i = 0; i < models.length; i++) {
		...
		model.mins[0] = bb.readFloat();
		model.mins[1] = bb.readFloat();
		model.mins[2] = bb.readFloat();

		model.maxs[0] = bb.readFloat();
		model.maxs[1] = bb.readFloat();
		model.maxs[2] = bb.readFloat();
...
function LoadLightGrid(world, buffer, lump) {
	...
	var wMins = world.bmodels[0].mins;
	var wMaxs = world.bmodels[0].maxs;
