 ...
 * NetchanSetup
 */
function NetchanSetup(src, addr) {
	...
	var onerror = function () {
		NetchanDestroy(netchan);
	};

	// Go ahead and initiate our socket connection.
	if (src === NS.CLIENT && addr.type !== NA.LOOPBACK) {
		sys.NetConnectToServer(addr, onopen, onerror);
	} else {
		onopen();
	}
...
function NetchanDestroy(netchan) {
	...
	if (netchan.remoteAddress.type === NA.LOOPBACK) {
		...
		QueueEvent({ type: SE.SVNETCLOSE, addr: netchan.remoteAddress });
		return;
	}

	sys.NetClose(netchan.remoteAddress);
}
...
function NetchanSendLoopPacket(netchan, buffer, length) {
	...
	QueueEvent({
		type: netchan.src === NS.CLIENT ? SE.SVNETMSG : SE.CLNETMSG,
...
function NetchanSend(netchan, buffer, length) {
	...
	sys.NetSend(netchan.remoteAddress, msg.buffer, msg.index);
}
