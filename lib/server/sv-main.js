...
function SendConfigstring(client, key) {
	SendServerCommand(client, 'cs', sv.configstrings[key]);
}
...
function AddServerCommand(client, type, msg) {
	...
	var serialized = JSON.stringify({ k: type, v: msg });
	...
	if (client.reliableSequence - client.reliableAcknowledge === MAX_RELIABLE_COMMANDS + 1 ) {
		...
		log('cmd', i, serialized);
		DropClient(client, 'Server command overflow');
		return;
	}

	// Copy the command off.
	client.reliableCommands[client.reliableSequence % MAX_RELIABLE_COMMANDS] = serialized;
}