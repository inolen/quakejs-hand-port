...
function CreateListenServer() {
	...
	SYS.NetListen(sv_port.getAsInt(), {
...
function CreateRemoteConsole() {
	...
	SYS.CreateRemoteConsole(sv_rconPort.getAsInt());
}
...
function CheckTimeouts() {
	var droppoint = svs.time - 1000 * sv_timeout.getAsInt();
	var zombiepoint = svs.time - 1000 * sv_zombietime.getAsInt();
...
function SendMasterHeartbeat() {
	...
	var url = sv_master.getAsString() + '/heartbeat';

	SYS.HttpPost(url, {
		port: sv_port.getAsInt()
	}, function (err, data) {
		if (err) {
			log('Failed to send master server heartbeat: ' + err.message);
		}
	});
}
...
function ServerInfo(netchan) {
	...
	info.sv_mapname = sv_mapname.getAsString();
	info.sv_maxClients = sv_maxClients.getAsInt();
	info.g_gametype = g_gametype.getAsInt();
