...
function Frame(msec) {
	...
	if (Cvar.cvarModifiedFlags & QS.CVF.SERVERINFO) {
		SetConfigstring('serverInfo', Cvar.GetCvarsByFlag(QS.CVF.SERVERINFO));
		Cvar.cvarModifiedFlags &= ~QS.CVF.SERVERINFO;
	}
	if (Cvar.cvarModifiedFlags & QS.CVF.SYSTEMINFO ) {
		SetConfigstring('systemInfo', Cvar.GetCvarsByFlag(QS.CVF.SYSTEMINFO));
		Cvar.cvarModifiedFlags &= ~QS.CVF.SYSTEMINFO;
	}
