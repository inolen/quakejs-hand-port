...
function ClientEnterServer(netchan, data) {
	...
	if (clientNum === undefined) {
		COM.NetchanOutOfBandPrint(netchan, JSON.stringify({
			type: 'print',
			data: 'Server is full.'
		}));
	...
	COM.NetchanOutOfBandPrint(newcl.netchan, JSON.stringify({ type: 'connectResponse' }));
...
function DropClient(client, reason) {
	...
	// Kill the netchan.
	COM.NetClose(client.netchan.msocket);
