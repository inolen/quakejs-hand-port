 ...
 * AcceptClient
 */
function AcceptClient(addr, infostr) {
	if (!Running()) {
		return;
	}
	...
	newcl.netchan = com.NetchanSetup(NS.SERVER, addr);
 ...
 * Called when a
 */
function ClientDisconnected(addr) {
	for (var i = 0; i < svs.clients.length; i++) {
		...
		if (client.netchan.remoteAddress.equals(addr)) {
