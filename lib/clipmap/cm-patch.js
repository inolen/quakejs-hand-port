...
function ValidateFacet(pc, facet) {
	...
	var w = QPoly.BaseWindingForPlane(plane.normal, plane.dist);
	...
	for (var i = 0; i < facet.numBorders; i++) {
		...
		if (!QPoly.ChopWindingInPlace(w, plane.normal, plane.dist, 0.1)) {
			return false;  // winding was completely chopped away
		}
	}

	// See if the facet is unreasonably large.
	QPoly.WindingBounds(w, bounds[0], bounds[1]);

	// TODO Move to qpoly.
	for (var i = 0; i < 3; i++) {
		if (bounds[1][i] - bounds[0][i] > QPoly.MAX_MAP_BOUNDS) {
			return false;  // we must be missing a plane
		}
		if (bounds[0][i] >= QPoly.MAX_MAP_BOUNDS) {
			return false;
		}
		if (bounds[1][i] <= -QPoly.MAX_MAP_BOUNDS) {
			return false;
		}
	}
...
function AddFacetBevels(pc, facet) {
	...
	QPoly.WindingBounds(w, mins, maxs);
		...
		for (axis = 0; axis < 3; axis++) {
			for (dir = -1; dir <= 1; dir += 2) {
				...
				if (i === facet.numBorders) {
					...
					if (!QPoly.ChopWindingInPlace(w2, newplane.normal, newplane.dist, 0.1)) {
