...
function TraceThroughPatchCollide(tw, pc) {
	...
	for (var i = 0; i < pc.facets.length; i++) {
		...
		for (j = 0; j < facet.numBorders; j++) {
			...
			if (j == 4 && plane[0] == -1 && plane[3] > -242 && plane[3] < -240) {
				console.log('TraceThroughPatchCollide HIT', cw.hit, '[', plane[0], plane[1], plane[2], plane[3], ']', startp[0], endp[0]);
				debug = true;
				window.bgdebug = true;
			}
			if (cw.hit) {
		...
		if (j < facet.numBorders) {
			if (debug) {
				// console.log('i < facet.numBorders');
			}
			continue;
		}
		...
		if (hitnum === facet.numBorders - 1) {
			if (debug) {
				// console.log('Never clip against the back side');
			}
			continue;
		}
		...
		if (cw.enterFrac < cw.leaveFrac && cw.enterFrac >= 0) {
			if (cw.enterFrac < tw.trace.fraction) {
				...
				// console.log('TraceThroughPatchCollide', i, hitnum, bestplane[0], bestplane[1], bestplane[2]);
	...
	if (debug) {
		console.log('TraceThroughPatchCollide FINAL', tw.trace.fraction);
	}
}
