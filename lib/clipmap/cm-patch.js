...
function EdgePlaneNum(pc, grid, gridPlanes, i, j, k) {
	...
	switch (k) {
		case 0:  // top border
			...
			vec3.add(p1, vec3.scale(pp.plane, 4, vec3.create()), up);
...
function ValidateFacet(pc, facet) {
	...
	vec4.set(pc.planes[facet.surfacePlane].plane, plane);
	...
	for (j = 0; j < facet.numBorders; j++) {
		...
		vec4.set(pc.planes[facet.borderPlanes[j]].plane, plane);
		if (!facet.borderInward[j]) {
			vec3.subtract(vec3.create(), plane, plane);
			plane[3] = -plane[3];
		}
		...
		for (axis = 0; axis < 3; axis++) {
			for (dir = -1; dir <= 1; dir += 2) {
				...
				if (i === facet.numBorders) {
					...
					vec4.set(pc.planes[facet.borderPlanes[facet.numBorders]].plane, newplane);
...
function EmitCollisionSurfaces(tessFn) {
	var plane = vec4.create();
	var mins = vec3.createFrom(-15, -15, -28);
	var maxs = vec3.createFrom(15, 15, 28);
	var v1 = vec3.create();
	var v2 = vec3.create();
	var debugSize = 2;
	...
	for (var pcnum = 0; pcnum < debugPatchCollides.length; pcnum++) {
		...
		for (var i = 0; i < pc.facets.length; i++) {
			...
			for (var k = 0; k < facet.numBorders + 1; k++) {
				...
				vec4.set(pc.planes[planenum].plane, plane);
				if (inward) {
					vec3.subtract(vec3.create(), plane, plane);
					plane[3] = -plane[3];
				}
				...
				for (var j = 0; j < facet.numBorders + 1; j++) {
					...
					vec4.set(pc.planes[curplanenum].plane, plane);
					if (!curinward) {
						vec3.subtract(vec3.create(), plane, plane);
						plane[3] = -plane[3];
					}
