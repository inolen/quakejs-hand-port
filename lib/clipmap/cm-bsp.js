...
function LoadMap(mapName, callback) {
	...
	sys.ReadFile('maps/' + mapName + '.bsp', 'binary', function (err, data) {
		...
		for (var i = 0; i < sh.LUMP.NUM_LUMPS; i++) {
		...
		LoadShaders(data, header.lumps[sh.LUMP.SHADERS]);
		LoadLeafs(data, header.lumps[sh.LUMP.LEAFS]);
		LoadLeafBrushes(data, header.lumps[sh.LUMP.LEAFBRUSHES]);
		LoadLeafSurfaces(data, header.lumps[sh.LUMP.LEAFSURFACES]);
		LoadPlanes(data, header.lumps[sh.LUMP.PLANES]);
		LoadBrushSides(data, header.lumps[sh.LUMP.BRUSHSIDES]);
		LoadBrushes(data, header.lumps[sh.LUMP.BRUSHES]);
		LoadSubmodels(data, header.lumps[sh.LUMP.MODELS]);
		LoadNodes(data, header.lumps[sh.LUMP.NODES]);
		LoadEntities(data, header.lumps[sh.LUMP.ENTITIES]);
		LoadVisibility(data, header.lumps[sh.LUMP.VISIBILITY]);
		LoadPatches(data, header.lumps[sh.LUMP.SURFACES], header.lumps[sh.LUMP.DRAWVERTS]);
...
function LoadPatches(buffer, surfsLump, vertsLump) {
	...
	for (var i = 0; i < count; i++) {
		...
		dface.lightmapX = bb.readInt();
		dface.lightmapY = bb.readInt();
		dface.lightmapWidth = bb.readInt();
		dface.lightmapHeight = bb.readInt();
		dface.lightmapOrigin[0] = bb.readFloat();
		dface.lightmapOrigin[1] = bb.readFloat();
		dface.lightmapOrigin[2] = bb.readFloat();
		dface.lightmapVecs[0][0] = bb.readFloat();
		dface.lightmapVecs[0][1] = bb.readFloat();
		dface.lightmapVecs[0][2] = bb.readFloat();
		dface.lightmapVecs[1][0] = bb.readFloat();
		dface.lightmapVecs[1][1] = bb.readFloat();
		dface.lightmapVecs[1][2] = bb.readFloat();
		dface.lightmapVecs[2][0] = bb.readFloat();
		dface.lightmapVecs[2][1] = bb.readFloat();
		dface.lightmapVecs[2][2] = bb.readFloat();
		dface.patchWidth = bb.readInt();
		dface.patchHeight = bb.readInt();

		if (dface.surfaceType !== sh.MST.PATCH) {
