...
function init(gl, canvas) {
    ...
    playerMover = new Q3GMove();
    Q3RMain.loadShaders(gl, mapShaders);
...
function drawFrame(gl, event) {
    ...
    Q3RMain.draw(gl, cameraPosition, modelViewMat, projectionMat);
}
...
function updateInput(frameTime) {
    ...
    playerMover.move(bsp, dir, frameTime);
}
...
function initEvents() {
    ...
    function moveUpdate(x, y, frameTime) {
        ...
        // Send desired movement direction to the player mover for collision detection against the map
        playerMover.move(bsp, dir, frameTime*2);
    }
