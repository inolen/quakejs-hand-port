...
define('common/com-trace', [], function () {
	var q3bsptree_trace_offset = 0.03125;

	function traceNode(bsp, nodeIdx, startFraction, endFraction, start, end, radius, output) {
		...
		if (nodeIdx < 0) { // Leaf node?
			...
			for (var i = 0; i < leaf.leafBrushCount; i++) {
				...
				if (brush.brushSideCount > 0 && shader.contents & 1) {
					traceBrush(bsp, brush, start, end, radius, output);
				}
			}
			return;
		}
		...
		if (startDist >= radius && endDist >= radius) {
			traceNode(bsp, node.children[0], startFraction, endFraction, start, end, radius, output );
		} else if (startDist < -radius && endDist < -radius) {
			traceNode(bsp, node.children[1], startFraction, endFraction, start, end, radius, output );
		} else {
			var side;
			...
			traceNode(bsp, node.children[side], startFraction, middleFraction, start, middle, radius, output );
			...
			traceNode(bsp, node.children[side===0?1:0], middleFraction, endFraction, middle, end, radius, output );
		}
	}

	function traceBrush(bsp, brush, start, end, radius, output) {
		var startFraction = -1;
		var endFraction = 1;
		var startsOut = false;
		var endsOut = false;
	...
	return {
		Trace: function(bsp, start, end, radius) {
			var output = {
				allSolid: false,
				startSolid: false,
				fraction: 1.0,
			...
			traceNode(bsp, 0, 0, 1, start, end, radius, output);
