...
Q3GMove.prototype._applyFriction = function() {
...
Q3GMove.prototype._groundCheck = function (bsp) {
...
Q3GMove.prototype._clipVelocity = function(velIn, normal) {
...
Q3GMove.prototype._accelerate = function(dir, speed, accel) {
...
Q3GMove.prototype._slideMove = function(bsp, gravity) {
    var bumpcount;
    var numbumps = 4;
    var planes = [];
    ...
    if ( gravity ) {
        ...
        if ( this.groundTrace && this.groundTrace.plane ) {
            ...
            this.velocity = this._clipVelocity(this.velocity, this.groundTrace.plane.normal);
        }
    }
    ...
    for(bumpcount=0; bumpcount < numbumps; ++bumpcount) {
        ...
        var trace = Q3Trace.trace(bsp, this.position, end, q3movement_playerRadius);
        ...
        for(var i = 0; i < planes.length; ++i) {
            ...
            var clipVelocity = this._clipVelocity(this.velocity, planes[i]);
            var endClipVelocity = this._clipVelocity(endVelocity, planes[i]);
            ...
            for (var j = 0; j < planes.length; j++) {
                ...
                clipVelocity = this._clipVelocity( clipVelocity, planes[j] );
                endClipVelocity = this._clipVelocity( endClipVelocity, planes[j] );
...
Q3GMove.prototype._stepSlideMove = function (bsp, gravity) {
    var start_o = vec3.set(this.position, [0,0,0]);
    var start_v = vec3.set(this.velocity, [0,0,0]);

    if (this._slideMove(bsp, gravity) === 0) { return; } // we got exactly where we wanted to go first try

    var down = vec3.set(start_o, [0,0,0]);
    down[2] -= q3movement_stepsize;
    var trace = Q3Trace.trace(bsp, start_o, down, q3movement_playerRadius);
    ...
    trace = Q3Trace.trace(bsp, start_o, up, q3movement_playerRadius);
    ...
    this._slideMove(bsp, gravity);

    // push down the final amount
    vec3.set(this.position, down);
    down[2] -= stepSize;
    trace = Q3Trace.trace(bsp, this.position, down, q3movement_playerRadius);
    if ( !trace.allSolid ) {
        vec3.set(trace.endPos, this.position);
    }
    if ( trace.fraction < 1.0 ) {
        this.velocity = this._clipVelocity( this.velocity, trace.plane.normal );
    }
};

Q3GMove.prototype._airMove = function (bsp, dir) {
    var speed = vec3.length(dir) * q3movement_scale;

    this._accelerate(dir, speed, q3movement_airaccelerate);

    this._stepSlideMove(bsp, true);
};

Q3GMove.prototype._walkMove = function (bsp, dir) {
    this._applyFriction();

    var speed = vec3.length(dir) * q3movement_scale;

    this._accelerate(dir, speed, q3movement_accelerate);

    this.velocity = this._clipVelocity(this.velocity, this.groundTrace.plane.normal);

    if(!this.velocity[0] && !this.velocity[1]) { return; }

    this._stepSlideMove(bsp, false);
};

Q3GMove.prototype.jump = function(bsp) {
...
Q3GMove.prototype.move = function(bsp, dir, frameTime) {
    q3movement_frameTime = frameTime*0.0075;

    this._groundCheck(bsp);

    vec3.normalize(dir);

    if (this.onGround) {
        this._walkMove(bsp, dir);
    } else {
        this._airMove(bsp, dir);
    }
