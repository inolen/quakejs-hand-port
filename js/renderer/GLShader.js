define('renderer/GLShader', [], function () {
	...
	function GLShaderFromTexture(gl, map, texture) {
		if (!defaultProgram) defaultProgram = CompileShaderProgram(gl, defaultVertexShaderSrc, defaultFragmentShaderSrc);
		if (!modelProgram) modelProgram = CompileShaderProgram(gl, defaultVertexShaderSrc, modelFragmnetShaderSrc);

		var glshader = Object.create(GLShader);

		glshader.cull = gl.FRONT;

		glshader.stages = [];
		glshader.stages.push({
			map: map,
			isLightmap: false,
			blendSrc: gl.ONE,
			blendDest: gl.ZERO,
	...
	function GLShaderFromShader(gl, shader) {
		var glshader = Object.create(GLShader);
		...
		for (var i = 0; i < shader.stages.length; i++) {
			var stage = shader.stages[i],
				vertexSrc = GenerateVertexShader(shader, stage),
				fragmentSrc = GenerateFragmentShader(shader, stage);
	...
	return {
		FromTexture: GLShaderFromTexture,
		FromShader: GLShaderFromShader
	};
});